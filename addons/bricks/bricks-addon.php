    ...
    public static function render_admin_page()
    {
        ...
        if (isset($tabs['packages'])) {
            ...
            if ($role_mode === 'mothership') {
                echo '<h3>' . esc_html__('Protected Variant Visibility', 'dbvc') . '</h3>';
                echo '<p>' . esc_html__('Review client-protected Bricks artifacts from the latest package metadata.', 'dbvc') . ' ';
                echo '<button type="button" class="button" id="dbvc-bricks-refresh-protected-fleet">' . esc_html__('Refresh Protected Visibility', 'dbvc') . '</button></p>';
                ...
                echo '<thead><tr><th>' . esc_html__('Site UID', 'dbvc') . '</th><th>' . esc_html__('Site', 'dbvc') . '</th><th>' . esc_html__('Protected', 'dbvc') . '</th><th>' . esc_html__('Types', 'dbvc') . '</th><th>' . esc_html__('Freshness', 'dbvc') . '</th><th>' . esc_html__('Actions', 'dbvc') . '</th></tr></thead>';
            ...
            if ($role_mode === 'client') {
                echo '<h3>' . esc_html__('Mothership Publish Runs', 'dbvc') . '</h3>';
                echo '<p>' . esc_html__('Recent package publish attempts from this client to the configured mothership.', 'dbvc') . ' ';
                ...
                echo '<thead><tr><th>' . esc_html__('At', 'dbvc') . '</th><th>' . esc_html__('Package', 'dbvc') . '</th><th>' . esc_html__('Targeting', 'dbvc') . '</th><th>' . esc_html__('Result', 'dbvc') . '</th><th>' . esc_html__('Verify', 'dbvc') . '</th><th>' . esc_html__('Receipt', 'dbvc') . '</th></tr></thead>';
        ...
        $status_config = [
            ...
            'messages' => [
                ...
                'protectedVariantFleetLoaded' => esc_html__('Protected visibility loaded.', 'dbvc'),
                ...
                'publishRunsLoaded' => esc_html__('Publish runs loaded.', 'dbvc'),
        ...
        echo 'function renderPublishRunsSummary(items){ if(!publishRunsSummary){ return; } const rows = Array.isArray(items) ? items : []; if(rows.length === 0){ publishRunsSummary.textContent = "No publish runs loaded."; return; } let successCount = 0; let failedCount = 0; let verifyOkCount = 0; rows.forEach(function(run){ if(run && String(run.result || "") === "success"){ successCount++; } else { failedCount++; } const verify = run && run.remote_package_visibility && typeof run.remote_package_visibility === "object" ? run.remote_package_visibility : null; if(verify && verify.ok === true){ verifyOkCount++; } }); const latest = rows[0] && rows[0].created_at ? String(rows[0].created_at) : ""; let text = "Recent runs: " + rows.length + " | Success: " + successCount + " | Failed: " + failedCount + " | Verified: " + verifyOkCount; if(latest){ text += " | Latest: " + latest; } publishRunsSummary.textContent = text; }';
        echo 'function renderPublishRuns(items){ if(!publishRunsBody){ return; } const rows = Array.isArray(items) ? items : []; renderPublishRunsSummary(rows); if(rows.length === 0){ state.selectedPublishRunId = null; publishRunsBody.innerHTML = "<tr><td colspan=\\"6\\">No publish runs loaded.</td></tr>"; if(publishRunsDetail){ publishRunsDetail.textContent = ""; } return; } let selectedRunId = String(state.selectedPublishRunId || ""); if(!selectedRunId){ selectedRunId = String(rows[0] && rows[0].run_id || ""); } if(!rows.some(function(run){ return String(run && run.run_id || "") === selectedRunId; })){ selectedRunId = String(rows[0] && rows[0].run_id || ""); } state.selectedPublishRunId = selectedRunId; publishRunsBody.innerHTML = rows.map(function(run){ const runId = String(run.run_id || ""); const createdAt = String(run.created_at || ""); const packageId = String(run.package_id || ""); const channel = String(run.channel || ""); const targeting = run.targeting && typeof run.targeting === "object" ? run.targeting : {mode:"all",site_uids:[]}; const targetMode = String(targeting.mode || "all"); const siteCount = Array.isArray(targeting.site_uids) ? targeting.site_uids.length : 0; const targetText = targetMode === "selected" ? ("selected (" + siteCount + ")") : "all"; const result = String(run.result || "failed"); const status = Number(run.http_status || 0); const verify = run.remote_package_visibility && typeof run.remote_package_visibility === "object" ? run.remote_package_visibility : null; const verifyText = verify ? (verify.ok === true ? "ok" : ("fail" + (verify.status ? " (" + String(verify.status) + ")" : ""))) : "n/a"; const receiptId = String(run.receipt_id || ""); const selectedStyle = runId === selectedRunId ? " style=\\"background:#f0f6fc;\\"" : ""; const packageBits = "<code>" + esc(packageId || "unknown") + "</code>" + (channel ? "<br><small>" + esc(channel) + "</small>" : ""); const resultText = esc(result + (status ? " (" + String(status) + ")" : "")); return "<tr data-publish-run-id=\\"" + esc(runId) + "\\"" + selectedStyle + "><td>" + esc(createdAt) + "</td><td>" + packageBits + "</td><td>" + esc(targetText) + "</td><td>" + resultText + "</td><td>" + esc(verifyText) + "</td><td><code>" + esc(receiptId) + "</code></td></tr>"; }).join(""); Array.prototype.forEach.call(publishRunsBody.querySelectorAll("tr[data-publish-run-id]"), function(row){ row.addEventListener("click", function(){ const runId = String(row.getAttribute("data-publish-run-id") || ""); selectPublishRun(runId, true); renderPublishRuns(state.publishRuns); }); }); selectPublishRun(selectedRunId, false); }';
        ...
        echo 'function createBootstrapPackage(){ if(!cfg.bootstrapPackageEndpoint){ return; } const selectedChannel = packagesChannelFilter && packagesChannelFilter.value ? String(packagesChannelFilter.value) : "stable"; setLoading(true); fetch(cfg.bootstrapPackageEndpoint, {method:"POST", headers: {"X-WP-Nonce": cfg.nonce, "X-DBVC-Correlation-ID": genCorrelationId(), "Idempotency-Key": "pkg-bootstrap-" + Date.now(), "Content-Type":"application/json"}, body: JSON.stringify({channel: selectedChannel})}).then(function(r){ return r.json().catch(function(){ return null; }).then(function(data){ if(!r.ok){ const err = new Error(data && data.message ? data.message : "Bootstrap package create failed."); err.details = data; throw err; } return data; }); }).then(function(data){ if(packageDetail){ packageDetail.textContent = JSON.stringify(data, null, 2); } setSuccess((cfg.messages && cfg.messages.packageBootstrapped) || "Package created from current site.", "package_bootstrapped"); return loadPackages().then(function(){ if(packagesSelect && data && data.package_id){ packagesSelect.value = data.package_id; state.selectedPackageId = data.package_id; return loadPackageDetail(data.package_id); } return null; }); }).catch(function(err){ if(packageDetail){ packageDetail.textContent = JSON.stringify({ok:false, error: err && err.message ? err.message : "Bootstrap package create failed.", details: err && err.details ? err.details : null}, null, 2); } setError(err.message || "Failed to create package from current site.", createBootstrapPackage, "package_bootstrap"); }).finally(function(){ setLoading(false); }); }';
        ...
        echo 'function renderProtectedFleetSummary(summary){ if(!protectedFleetSummary){ return; } const normalized = summary && typeof summary === "object" ? summary : {}; const types = countBits(normalized.by_artifact_type); const scopes = countBits(normalized.by_scope); let text = "Sites: " + Number(normalized.site_count || 0) + " | Protected sites: " + Number(normalized.protected_site_count || 0) + " | Protected variants: " + Number(normalized.total_protected_variants || 0) + " | Artifacts: " + Number(normalized.unique_artifact_uids || 0); if(types){ text += " | Types: " + types; } if(scopes){ text += " | Scope: " + scopes; } if(normalized.latest_sync_at){ text += " | Latest sync: " + String(normalized.latest_sync_at); } protectedFleetSummary.textContent = text; }';
