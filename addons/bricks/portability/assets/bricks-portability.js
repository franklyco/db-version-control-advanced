( function () {
	...
	const els = {
		domainList: document.getElementById(
			'dbvc-bricks-portability-domain-list'
		),
		exportNotes: document.getElementById(
			'dbvc-bricks-portability-export-notes'
		),
		exportButton: document.getElementById(
			'dbvc-bricks-portability-export-button'
		),
		exportsBody: document.getElementById(
			'dbvc-bricks-portability-exports-body'
		),
		importFile: document.getElementById(
			'dbvc-bricks-portability-import-file'
		),
		importButton: document.getElementById(
			'dbvc-bricks-portability-import-button'
		),
		sessionMeta: document.getElementById(
			'dbvc-bricks-portability-session-meta'
		),
		sessionsBody: document.getElementById(
			'dbvc-bricks-portability-sessions-body'
		),
		summary: document.getElementById( 'dbvc-bricks-portability-summary' ),
		reviewState: document.getElementById(
			'dbvc-bricks-portability-review-state'
		),
		refreshSession: document.getElementById(
			'dbvc-bricks-portability-refresh-session'
		),
		appliedSummary: document.getElementById(
			'dbvc-bricks-portability-applied-summary'
		),
		stats: document.getElementById( 'dbvc-bricks-portability-stats' ),
		pagination: document.getElementById(
			'dbvc-bricks-portability-pagination'
		),
		pageSize: document.getElementById(
			'dbvc-bricks-portability-page-size'
		),
		pageSummary: document.getElementById(
			'dbvc-bricks-portability-page-summary'
		),
		pagePrev: document.getElementById(
			'dbvc-bricks-portability-page-prev'
		),
		pageNext: document.getElementById(
			'dbvc-bricks-portability-page-next'
		),
		filterDomain: document.getElementById(
			'dbvc-bricks-portability-filter-domain'
		),
		filterStatus: document.getElementById(
			'dbvc-bricks-portability-filter-status'
		),
		filterDecision: document.getElementById(
			'dbvc-bricks-portability-filter-decision'
		),
		filterWarning: document.getElementById(
			'dbvc-bricks-portability-filter-warning'
		),
		filterSearch: document.getElementById(
			'dbvc-bricks-portability-filter-search'
		),
		hideIdentical: document.getElementById(
			'dbvc-bricks-portability-hide-identical'
		),
		bulkAction: document.getElementById(
			'dbvc-bricks-portability-bulk-action'
		),
		bulkApply: document.getElementById(
			'dbvc-bricks-portability-apply-bulk'
		),
		tableBody: document.getElementById(
			'dbvc-bricks-portability-table-body'
		),
		detail: document.getElementById( 'dbvc-bricks-portability-detail' ),
		domainSummary: document.getElementById(
			'dbvc-bricks-portability-domain-summary'
		),
		confirmApplyBoxes: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-confirm-apply]' )
		),
		saveDraftButtons: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-save-draft]' )
		),
		applyButtons: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-apply-button]' )
		),
		backupsBody: document.getElementById(
			'dbvc-bricks-portability-backups-body'
		),
		...
		subtabButtons: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-tab]' )
		),
		subtabPanels: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-panel]' )
		),
		rowModal: document.getElementById(
			'dbvc-bricks-portability-row-modal'
		),
		rowModalClose: document.getElementById(
			'dbvc-bricks-portability-row-modal-close'
		),
		rowModalSubtitle: document.getElementById(
			'dbvc-bricks-portability-row-modal-subtitle'
		),
		sortButtons: Array.prototype.slice.call(
			panel.querySelectorAll( '[data-portability-sort]' )
		),
	};
	...
	const STATUS_META = {
		identical: {
			...
			description:
				'The transfer package version and this site match after DBVC compares them.',
		},
		new_in_source: {
			label: 'Only in Transfer Package',
			description:
				'This item exists in the Transfer Package but not on This Site.',
		},
		missing_from_source: {
			label: 'Only on This Site',
			description:
				'This item exists on This Site but is not present in the Transfer Package.',
		},
		...
		value_changed: {
			...
			description:
				'The same item exists on both sides, but one or more values differ.',
		},
		changed_props: {
			label: 'Changed Settings Structure',
			description:
				'The same item exists on both sides, with changed values plus added or removed fields.',
		},
		added_props: {
			label: 'Transfer Package Adds Details',
			description:
				'The Transfer Package adds fields that are not on This Site.',
		},
		removed_props: {
			label: 'This Site Has Extra Details',
			description:
				'This Site contains fields that are not present in the Transfer Package.',
		},
	};
	...
	function activateSubtab( tabKey ) {
		...
		els.subtabButtons.forEach( function ( button ) {
			const isActive =
				String(
					button.getAttribute( 'data-portability-tab' ) || ''
				) === nextTab;
		...
		els.subtabPanels.forEach( function ( tabPanel ) {
			const isActive =
				String(
					tabPanel.getAttribute( 'data-portability-panel' ) || ''
				) === nextTab;
	...
	function getRowApproval( row ) {
		return {
			decision: String(
				row && row.approved_action ? row.approved_action : ''
			),
			approvedAt: String(
				row && row.approved_at_gmt ? row.approved_at_gmt : ''
			),
			status: String(
				row && row.approval_status ? row.approval_status : ''
			),
		};
	}
		...
		return Array.prototype.slice
			.call(
				panel.querySelectorAll(
					'input[name="dbvc-bricks-portability-domain"]:checked'
				)
			)
			.map( function ( node ) {
	...
	function renderDomains() {
		...
		if ( ! Array.isArray( state.domains ) || state.domains.length === 0 ) {
			els.domainList.textContent =
				'No Bricks settings groups are available to export.';
			return;
		}
		els.domainList.innerHTML = state.domains
			.map( function ( domain ) {
				const domainKey = String( domain.domain_key || '' );
				const disabled = domain.available ? '' : ' disabled="disabled"';
				const note = domain.available
					? ''
					: ' <em>(not available on this site)</em>';
				const risk = domain.high_risk
					? ' <span class="dbvc-bricks-portability-badge">review carefully</span>'
					: '';
				const verification = domain.verification
					? ' <span class="dbvc-bricks-portability-badge">needs check</span>'
					: '';
				return (
					'<label class="dbvc-bricks-portability-domain-option"><input type="checkbox" name="dbvc-bricks-portability-domain" value="' +
					esc( domainKey ) +
					'"' +
					disabled +
					' /> <strong>' +
					esc( domain.label || domainKey ) +
					'</strong>' +
					risk +
					verification +
					note +
	...
	function renderExports() {
		...
		if ( ! Array.isArray( state.exports ) || state.exports.length === 0 ) {
			els.exportsBody.innerHTML =
				'<tr><td colspan="4">No transfer packages yet.</td></tr>';
			return;
		}
		els.exportsBody.innerHTML = state.exports
			.map( function ( item ) {
				const domains = Array.isArray( item.selected_domains )
					? item.selected_domains.join( ', ' )
					: '';
				...
				return (
					'<tr><td><code>' +
					esc( item.package_id || item.export_id || '' ) +
					'</code></td><td>' +
					esc( item.created_at_gmt || '' ) +
					'</td><td>' +
					esc( domains ) +
					'</td><td>' +
					download +
	...
	function renderSessions() {
		...
		if (
			...
			els.sessionsBody.innerHTML =
				'<tr><td colspan="4">No review sessions yet.</td></tr>';
			return;
		}
		els.sessionsBody.innerHTML = state.sessions
			.map( function ( item ) {
				const summary =
					item.summary && typeof item.summary === 'object'
						...
						: {};
				const draft =
					item.draft && typeof item.draft === 'object'
						? item.draft
						: {};
				const approval =
					item.approval && typeof item.approval === 'object'
						...
						: {};
				const rollback =
					item.rollback && typeof item.rollback === 'object'
						? item.rollback
						: {};
				const summaryParts = [
					'<div class="dbvc-bricks-portability-session-summary-line">' +
						esc(
							'Rows: ' +
								String( summary.total_rows || 0 ) +
								' | Actionable: ' +
								String( summary.actionable_rows || 0 )
						) +
						'</div>',
				];
				if ( item.refreshed_at_gmt ) {
					summaryParts.push(
						'<div class="description">' +
							esc(
								'Last compared: ' +
									formatTimestamp( item.refreshed_at_gmt )
							) +
							'</div>'
					);
				}
				if ( draft.saved_at_gmt ) {
					summaryParts.push(
						'<div class="description">' +
							esc(
								'Draft saved: ' +
									formatTimestamp( draft.saved_at_gmt ) +
									' | Decisions: ' +
									String( draft.decision_count || 0 )
							) +
							'</div>'
					);
				}
				if ( approval.approved_at_gmt ) {
					summaryParts.push(
						'<div class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--approved">' +
							esc(
								'Applied to This Site: ' +
									formatTimestamp( approval.approved_at_gmt )
							) +
							'</div>'
					);
				}
				if ( rollback.rolled_back_at_gmt ) {
					summaryParts.push(
						'<div class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--rollback">' +
							esc(
								'Restored on This Site: ' +
									formatTimestamp(
										rollback.rolled_back_at_gmt
									)
							) +
							'</div>'
					);
				...
				return (
					...
					'</code></td><td>' +
					summaryParts.join( '' ) +
					'</td><td><button type="button" class="button button-small dbvc-bricks-portability-open-session" data-session-id="' +
					esc( item.session_id || '' ) +
					'">Open</button></td></tr>'
				);
			} )
		...
		Array.prototype.forEach.call(
			els.sessionsBody.querySelectorAll(
				'.dbvc-bricks-portability-open-session'
			),
			function ( button ) {
				button.addEventListener( 'click', function () {
					openSession(
						String( button.getAttribute( 'data-session-id' ) || '' )
					);
				} );
			}
		);
	...
	function renderBackups() {
		...
		if ( ! Array.isArray( state.backups ) || state.backups.length === 0 ) {
			els.backupsBody.innerHTML =
				'<tr><td colspan="4">No backups yet.</td></tr>';
			return;
		}
		els.backupsBody.innerHTML = state.backups
			.map( function ( item ) {
				const optionNames = Array.isArray( item.option_names )
					? item.option_names.join( ', ' )
					: '';
				const receipt = renderReceiptInline(
					item.reference_receipt || {}
				);
				const detail = [ optionNames, receipt ]
					.filter( Boolean )
					.join( ' | ' );
				return (
					'<tr><td><code>' +
					...
					'</td><td>' +
					esc( detail ) +
					'</td><td><button type="button" class="button button-small dbvc-bricks-portability-rollback" data-backup-id="' +
					esc( item.backup_id || '' ) +
			...
			.join( '' );
		Array.prototype.forEach.call(
			els.backupsBody.querySelectorAll(
				'.dbvc-bricks-portability-rollback'
			),
			function ( button ) {
				button.addEventListener( 'click', function () {
					rollbackBackup(
						String( button.getAttribute( 'data-backup-id' ) || '' )
					);
				} );
			}
		);
	...
	function renderSessionMeta() {
		...
		const site =
			state.currentSession.site &&
			typeof state.currentSession.site === 'object'
				? state.currentSession.site
				: {};
		const draft =
			state.currentSession.draft &&
			typeof state.currentSession.draft === 'object'
				? state.currentSession.draft
				: {};
		const approval =
			state.currentSession.approval &&
			typeof state.currentSession.approval === 'object'
				? state.currentSession.approval
				: {};
		const rollback =
			state.currentSession.rollback &&
			typeof state.currentSession.rollback === 'object'
				? state.currentSession.rollback
				: {};
		const incomingSite = [ site.site_name, site.home_url ]
			.filter( Boolean )
			.join( ' | ' );
		const lines = [
			'Session: ' + String( state.currentSession.session_id || '' ),
			'Transfer Package: ' +
				String( state.currentSession.package_id || '' ),
			'Last Compared: ' +
				String(
					state.currentSession.refreshed_at_gmt ||
						state.currentSession.created_at_gmt ||
						''
				),
		...
		if ( draft.saved_at_gmt ) {
			...
			lines.push(
				'Draft Decisions: ' + String( draft.decision_count || 0 )
			);
		}
		if ( approval.approved_at_gmt ) {
			lines.push(
				'Applied to This Site: ' + String( approval.approved_at_gmt )
			);
			lines.push(
				'Reviewed Decisions: ' + String( approval.decision_count || 0 )
			);
		}
		if ( rollback.rolled_back_at_gmt ) {
			lines.push(
				'Restored on This Site: ' +
					String( rollback.rolled_back_at_gmt )
			);
			lines.push(
				'Restore Backup: ' + String( rollback.backup_id || '' )
			);
		}
	...
	function renderAppliedSummary() {
		...
		if ( ! state.currentSession ) {
			els.appliedSummary.textContent =
				'Apply selected changes to record review timestamps for this transfer package on this site.';
			return;
		}
		...
		const rollback =
			...
			typeof state.currentSession.rollback === 'object'
				...
				: {};
		if ( ! approval.approved_at_gmt ) {
			els.appliedSummary.innerHTML =
				'<div class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--pending">Applied to This Site: Not applied yet for this transfer package.</div>';
			return;
		}

		const detailBits = [
			'Reviewed rows: ' + String( approval.decision_count || 0 ),
			'Transfer package changes applied: ' +
				String( approval.mutating_decision_count || 0 ),
		];
		if ( approval.backup_id ) {
			detailBits.push( 'Backup: ' + String( approval.backup_id || '' ) );
		}
		const receiptLine = renderReceiptInline(
			approval.reference_receipt || {}
		);
		if ( receiptLine ) {
		...
		if ( rollback.rolled_back_at_gmt ) {
			parts.push(
				'<div class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--rollback">' +
					esc(
						'Restored on This Site: ' +
							formatTimestamp( rollback.rolled_back_at_gmt )
					) +
					'</div>'
			);
		}
	...
	function renderReceiptInline( receipt ) {
		...
		const references =
			receipt.references && typeof receipt.references === 'object'
				? receipt.references
				: {};
		const media =
			receipt.media && typeof receipt.media === 'object'
				? receipt.media
				: {};
		const entities =
			receipt.entities && typeof receipt.entities === 'object'
				? receipt.entities
				: {};
		const parts = [];
		...
		const mediaChanged =
			Number( media.created_attachments || 0 ) +
			Number( media.reused_attachments || 0 );
		const templatesChanged =
			Number( entities.created_posts || 0 ) +
			Number( entities.updated_posts || 0 );
		if ( remapped > 0 ) {
	...
	function renderReviewState() {
		...
		const freshness =
			state.currentSession.freshness &&
			typeof state.currentSession.freshness === 'object'
				? state.currentSession.freshness
				: {};
		const rollback =
			state.currentSession.rollback &&
		...
		const changedDomains = Array.isArray( freshness.changed_domains )
			? freshness.changed_domains
			: [];
		const freshnessBadge =
			'<span class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--' +
			esc( isFresh ? 'fresh' : 'stale' ) +
			'">' +
			esc(
				isFresh
					? 'This Site Check: Current'
					: 'This Site Check: Needs Refresh'
			) +
			'</span>';
		const parts = [
			freshnessBadge,
			'<span class="description">' +
				esc(
					'Last compared: ' +
						formatTimestamp(
							freshness.last_compared_at_gmt ||
								state.currentSession.refreshed_at_gmt ||
								state.currentSession.created_at_gmt ||
								''
						)
		...
		if ( ! isFresh && changedDomains.length ) {
			parts.push(
				'<span class="description">' +
					esc(
						'Changed settings groups since comparison: ' +
							changedDomains.join( ', ' )
					) +
					'</span>'
			);
		...
		if ( rollback.rolled_back_at_gmt ) {
			parts.push(
				'<span class="dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--rollback">' +
					esc(
						'Restored: ' +
							formatTimestamp( rollback.rolled_back_at_gmt )
					) +
		...
		els.reviewState.innerHTML = [
			'<div class="dbvc-bricks-portability-review-state__meta">' +
				parts.join( ' ' ) +
				'</div>',
		...
		els.refreshSession = document.getElementById(
			'dbvc-bricks-portability-refresh-session'
		);
		if ( els.refreshSession ) {
			els.refreshSession.addEventListener(
				'click',
	...
	function hydrateDraftDecisions( session ) {
		const draft =
			session && session.draft && typeof session.draft === 'object'
				? session.draft
				: {};
		const approval =
			session && session.approval && typeof session.approval === 'object'
				? session.approval
				: {};
		const decisions =
			draft.decisions && typeof draft.decisions === 'object'
				? draft.decisions
				: {};
		const approvalRows =
			approval.rows && typeof approval.rows === 'object'
				? approval.rows
				: {};
		const hydrated = {};
		...
		( Array.isArray( session && session.rows )
			...
			const allowed = Array.isArray( row.available_actions )
				? row.available_actions
				: [];
			const approvalDecision =
				approvalRows[ rowId ] &&
				typeof approvalRows[ rowId ] === 'object'
					? String( approvalRows[ rowId ].decision || '' )
					: '';
			const savedDecision = String(
				decisions[ rowId ] || approvalDecision || ''
			);
	...
	function hydrateManualDecisionRowIds( session ) {
		...
		[]
			.concat(
				...
				Array.isArray( approval.manual_rows )
					? approval.manual_rows
					: []
			)
	...
	function isManualDecision( row ) {
		...
		return Boolean(
			state.manualDecisionRowIds[ rowId ] ||
				( row && row.manual_decision )
		);
	}
	...
	function currentRows() {
		return state.currentSession &&
			Array.isArray( state.currentSession.rows )
			? state.currentSession.rows
			: [];
	}

	function filteredRows() {
		const selectedDomain = els.filterDomain
			? String( els.filterDomain.value || '' )
			: '';
		const selectedStatus = els.filterStatus
			? String( els.filterStatus.value || '' )
			: '';
		const selectedDecision = els.filterDecision
			? String( els.filterDecision.value || '' )
			: '';
		const selectedWarning = els.filterWarning
			? String( els.filterWarning.value || '' )
			: '';
		const hideIdentical =
			! els.hideIdentical || Boolean( els.hideIdentical.checked );
		const search = els.filterSearch
			? String( els.filterSearch.value || '' ).toLowerCase()
			: '';
		return currentRows().filter( function ( row ) {
			if (
				selectedDomain &&
				String( row.domain_key || '' ) !== selectedDomain
			) {
				return false;
			}
			if (
				selectedStatus &&
				String( row.status || '' ) !== selectedStatus
			) {
				return false;
			}
			if (
				hideIdentical &&
				selectedStatus !== 'identical' &&
				String( row.status || '' ) === 'identical'
			) {
				return false;
			}
			if ( selectedDecision ) {
			...
			if (
				selectedWarning === 'with_warnings' &&
				( ! Array.isArray( row.warnings ) || row.warnings.length === 0 )
			) {
				return false;
			}
			if (
				selectedWarning === 'without_warnings' &&
	...
	function compareRowIdentity( left, right ) {
		const leftKey = [
			String(
				left && left.domain_label
					? left.domain_label
					: left && left.domain_key
					? left.domain_key
					: ''
			),
		...
		const rightKey = [
			String(
				right && right.domain_label
					? right.domain_label
					: right && right.domain_key
					? right.domain_key
					: ''
			),
	...
	function sortedRows( rows ) {
		...
		list.sort( function ( left, right ) {
			const leftStamp = timestampValue(
				getRowApproval( left ).approvedAt
			);
			const rightStamp = timestampValue(
				getRowApproval( right ).approvedAt
			);
	...
	function totalPagesForCount( totalCount ) {
		...
		return Math.max(
			1,
			Math.ceil( Math.max( 0, Number( totalCount || 0 ) ) / pageSize )
		);
	}
	...
	function renderFilters() {
				...
				domains
					.map( function ( value ) {
						return (
							...
							esc( value ) +
							'">' +
							esc( domainOptions[ value ] || value ) +
							'</option>'
						);
					} )
					.join( '' );
			els.filterDomain.value =
				currentValue && domains.indexOf( currentValue ) !== -1
					? currentValue
					: '';
		}
		if ( els.filterStatus ) {
		...
		if ( els.filterDecision ) {
			...
			const observedDecisions = Array.from(
				new Set(
					currentRows()
						.map( function ( row ) {
			...
			els.filterDecision.innerHTML =
				'<option value="">All</option>' +
				'<option value="' +
				esc( STAGED_DECISIONS_FILTER ) +
				'">Staged Decisions</option>' +
				'<option value="' +
				esc( MANUAL_DECISIONS_FILTER ) +
				'">Manual Decisions</option>' +
				decisions
					.map( function ( value ) {
						return (
							'<option value="' +
					...
					.join( '' );
			const allowedDecisionValues = decisions.concat( [
				STAGED_DECISIONS_FILTER,
				MANUAL_DECISIONS_FILTER,
			] );
			els.filterDecision.value =
				currentDecision &&
				allowedDecisionValues.indexOf( currentDecision ) !== -1
					? currentDecision
					: '';
		}
	}
	...
	function renderPagination( rows ) {
		...
		if ( ! state.currentSession ) {
			els.pageSummary.textContent =
				'Upload a transfer package to page through review rows.';
		...
		const end =
			totalRows > 0 ? Math.min( totalRows, start + pageSize - 1 ) : 0;
	...
	function ensureDecision( row ) {
		...
		if ( ! state.decisions[ rowId ] ) {
			state.decisions[ rowId ] = String(
				row.suggested_action || 'keep_current'
			);
		}
	...
	function renderApprovalCell( row ) {
		...
		const badgeClass =
			'dbvc-bricks-portability-inline-badge dbvc-bricks-portability-inline-badge--' +
			safeKey( approval.status || 'approved' );
		return [
			'<div class="dbvc-bricks-portability-approval-cell">',
			'<span class="' +
				esc( badgeClass ) +
				'">' +
				esc( approvalStatusLabel( approval.status || 'approved' ) ) +
				'</span>',
			'<div class="dbvc-bricks-portability-approval-cell__time">' +
				esc( formatTimestamp( approval.approvedAt ) ) +
				'</div>',
			'</div>',
		].join( '' );
	}
	...
	function renderTable( rows ) {
		...
		const sortedFilteredRows = Array.isArray( rows )
			? rows
			: sortedRows( filteredRows() );
		const pageRows = paginatedRows( sortedFilteredRows );
		if ( pageRows.length === 0 ) {
			els.tableBody.innerHTML =
				'<tr><td colspan="9">No review rows match the current filters.</td></tr>';
			return;
		}
		els.tableBody.innerHTML = pageRows
			.map( function ( row ) {
				...
				const options = Array.isArray( row.available_actions )
					? row.available_actions
					: [];
				const selectedClass =
					rowId === state.selectedRowId ? ' is-selected' : '';
				const statusClass =
					'dbvc-bricks-portability-status dbvc-bricks-portability-status--' +
					safeKey( row.status || '' );
				const actionClass =
					'dbvc-bricks-portability-action-pill dbvc-bricks-portability-action-pill--' +
					safeKey( decision );
				return (
					'<tr class="dbvc-bricks-portability-row' +
					selectedClass +
					'" data-row-id="' +
					esc( rowId ) +
					'">' +
					'<td>' +
					esc( row.domain_label || row.domain_key || '' ) +
					'</td>' +
					'<td>' +
					esc( row.object_label || 'Untitled item' ) +
					'</td>' +
					'<td><code>' +
					esc( row.object_id || '' ) +
					'</code></td>' +
					'<td>' +
					esc( matchLabel( row.match_status || '' ) ) +
					'</td>' +
					'<td><span class="' +
					esc( statusClass ) +
					'">' +
					...
					esc( String( warnings ) ) +
					'</td>' +
					'<td><select class="dbvc-bricks-portability-decision" data-row-id="' +
					esc( rowId ) +
					'">' +
					options
						.map( function ( option ) {
		...
		Array.prototype.forEach.call(
			els.tableBody.querySelectorAll( '.dbvc-bricks-portability-row' ),
			function ( rowNode ) {
				rowNode.addEventListener( 'click', function ( event ) {
					...
					selectRow(
						String( rowNode.getAttribute( 'data-row-id' ) || '' )
					);
				} );
			}
		);
		...
		Array.prototype.forEach.call(
			els.tableBody.querySelectorAll(
				'.dbvc-bricks-portability-decision'
			),
			function ( select ) {
				select.addEventListener( 'change', function () {
					const rowId = String(
						select.getAttribute( 'data-row-id' ) || ''
					);
					state.decisions[ rowId ] = String(
						select.value || 'keep_current'
					);
	...
	function getRowById( rowId ) {
		return (
			currentRows().find( function ( candidate ) {
				return (
					String( candidate.row_id || '' ) === String( rowId || '' )
				);
			} ) || null
		);
	}
	...
	function openRowModal( row ) {
		...
		if (
			els.rowModalClose &&
			typeof els.rowModalClose.focus === 'function'
		) {
	...
	function closeRowModal( restoreFocus ) {
		...
		if ( document.body && document.body.classList ) {
			document.body.classList.remove(
				'dbvc-bricks-portability-modal-open'
			);
		}
		if (
			shouldRestoreFocus &&
	...
	function renderRowDetail( row ) {
		...
		if ( ! row ) {
			if ( els.rowModalSubtitle ) {
				els.rowModalSubtitle.textContent =
					'Select a row to inspect its exact transfer package versus this site comparison.';
			}
			els.detail.innerHTML =
				'<p class="dbvc-bricks-portability-empty">Select a row to inspect its exact transfer package versus this site comparison.</p>';
			return;
		}

		const sourcePayload = getNormalizedPayload( row.source );
		const targetPayload = getNormalizedPayload( row.target );
		const diffEntries = buildDiffEntries(
			row,
			sourcePayload,
			targetPayload
		);
		const warnings = uniqueStrings(
			[].concat(
				row.warnings || [],
				( row.references && row.references.missing_dependencies ) || []
			)
		);
		...
		const verification = renderVerificationSummary(
			row.verification || {}
		);
		if ( els.rowModalSubtitle ) {
			els.rowModalSubtitle.textContent =
				String( row.domain_label || row.domain_key || 'Row' ) +
				' | ' +
				String( row.object_label || 'Untitled item' ) +
				' | ID: ' +
				String( row.object_id || 'n/a' );
		}

		const headerHtml = [
			'<div class="dbvc-bricks-portability-detail-header">',
			'<div class="dbvc-bricks-portability-detail-meta">',
			'<div class="dbvc-bricks-portability-detail-title">' +
				esc( row.object_label || 'Untitled item' ) +
				'</div>',
			'<div class="dbvc-bricks-portability-detail-subtitle">' +
				esc( String( row.domain_label || row.domain_key || '' ) ) +
				' | ID: ' +
				esc( String( row.object_id || 'n/a' ) ) +
				'</div>',
			'</div>',
			'<div class="dbvc-bricks-portability-detail-badges">',
			'<span class="dbvc-bricks-portability-chip">' +
			...
			'<span class="dbvc-bricks-portability-chip">' +
				...
				'</span>',
			'<span class="dbvc-bricks-portability-chip">' +
				esc( 'Selected Action: ' + decision ) +
				'</span>',
			'</div>',
			'</div>',
			'<p class="dbvc-bricks-portability-detail-note">' +
				esc(
					statusInfo.description ||
						'Review the exact differences below.'
				) +
				'</p>',
			references,
			verification,
		...
		const diffHtml = diffEntries.length
			? '<div class="dbvc-bricks-portability-diff-list">' +
			  diffEntries.map( renderDiffEntry ).join( '' ) +
			  '</div>'
	...
	function collectDependencyRowCandidates( row, domainKey ) {
		...
		sides.forEach( function ( side ) {
			...
			const matchKeys =
				side.match_keys && typeof side.match_keys === 'object'
					? side.match_keys
					: {};
			[ 'token', 'name', 'slug', 'id', 'selector', 'map_key' ].forEach(
				function ( key ) {
	...
	function findDependencyRows( domainKey, values ) {
		const wanted = uniqueStrings(
			( Array.isArray( values ) ? values : [] ).map( function ( value ) {
		...
		return currentRows().filter( function ( row ) {
			if (
				! row ||
				String( row.domain_key || '' ) !== String( domainKey || '' ) ||
				! row.source
			) {
				return false;
			}
	...
	function findMetadataRows( domainKey, optionName ) {
		...
		return currentRows().filter( function ( row ) {
			if (
				! row ||
				String( row.domain_key || '' ) !== normalizedDomainKey ||
				String( row.row_type || '' ) !== 'meta'
			) {
				return false;
			}
	...
	function preferredIncomingDecision( row ) {
		const options = Array.isArray( row && row.available_actions )
			? row.available_actions
			: [];
		const suggested = String(
			row && row.suggested_action ? row.suggested_action : ''
		);
		if (
			( suggested === 'add_incoming' ||
				suggested === 'replace_with_incoming' ) &&
			options.indexOf( suggested ) !== -1
		) {
			return suggested;
		}
	...
	function renderDependencyAction( actionConfig ) {
		const domainKey = String(
			actionConfig && actionConfig.domainKey ? actionConfig.domainKey : ''
		);
		const values = Array.isArray( actionConfig && actionConfig.values )
			? actionConfig.values
			: [];
		const metadataOptionName = String(
			actionConfig && actionConfig.metadataOptionName
				? actionConfig.metadataOptionName
				: ''
		);
		...
		const allApproved =
			...
			approvedCount === actionableRowIds.length;
		...
		const feedback = allApproved
			? state.dependencyActionFeedback[
					dependencyActionKey( domainKey, actionableRowIds )
			  ] || null
			: null;
		const wrapperClass =
			'dbvc-bricks-portability-reference-actions' +
			( feedback && feedback.status === 'success' ? ' is-success' : '' );
		const statusMessage =
			feedback && feedback.message
				...
				? 'All linked ' +
				  nounLabel +
				  ' rows are marked to apply from the transfer package.'
				: 'Click to mark linked ' +
				  nounLabel +
				  ' rows to apply from the transfer package.';

		return [
			'<div class="' + esc( wrapperClass ) + '">',
			actionableRowIds.length
				? '<button type="button" class="button button-small dbvc-bricks-portability-reference-action' +
			...
			'<span class="description">' +
				esc(
					String( rowIds.length ) +
						...
						( allApproved
							? ' Already marked to apply from the transfer package.'
							: approvedCount > 0
							? ' ' + String( approvedCount ) + ' already marked.'
							: '' )
				) +
				'</span>',
			'<span class="dbvc-bricks-portability-reference-action-status" role="status" aria-live="polite">' +
				esc( statusMessage ) +
				'</span>',
			'</div>',
		].join( '' );
	...
	function renderDependencyGroup( groupConfig ) {
		const values = Array.isArray( groupConfig && groupConfig.values )
			? groupConfig.values
			: [];
		if ( ! values.length ) {
			return '';
		...
		const actionHtml =
			groupConfig && groupConfig.action
				? renderDependencyAction( groupConfig.action )
				: '';
		return [
			'<li>',
			'<strong>' +
				esc( String( groupConfig.label || '' ) ) +
				':</strong> ',
			'<span class="dbvc-bricks-portability-reference-analysis__tokens">' +
				values
					.map( function ( value ) {
						return (
							'<code>' + esc( String( value || '' ) ) + '</code>'
						);
					} )
					.join( ' ' ) +
				'</span>',
	...
	function renderReferenceSummary( references ) {
		const cssVariables = Array.isArray( references.css_variables )
			? references.css_variables
			: [];
		const classNames = Array.isArray( references.class_names )
			? references.class_names
			: [];
		const categoryValues = Array.isArray( references.category_values )
			? references.category_values
			: [];
		const cssVariableDependencies =
			references.css_variable_dependencies &&
			typeof references.css_variable_dependencies === 'object'
				? references.css_variable_dependencies
				: {};
		const categoryDependencies =
			references.category_dependencies &&
			typeof references.category_dependencies === 'object'
				? references.category_dependencies
				: {};
		const classDependencyWarnings = Array.isArray(
			references.class_dependencies_missing_on_current
		)
			? references.class_dependencies_missing_on_current
			: [];
		const templateReferenceSummary =
			references.template_reference_summary &&
			typeof references.template_reference_summary === 'object'
				? references.template_reference_summary
				: {};
		...
		const hasDependencyAnalysis = Boolean(
			classDependencyWarnings.length ||
				( Array.isArray(
					categoryDependencies.missing_on_current_supplied_by_incoming
				) &&
					categoryDependencies.missing_on_current_supplied_by_incoming
						.length ) ||
				( Array.isArray( categoryDependencies.missing_on_both ) &&
					categoryDependencies.missing_on_both.length ) ||
				( Array.isArray(
					cssVariableDependencies.missing_on_current_supplied_by_incoming
				) &&
					cssVariableDependencies
						.missing_on_current_supplied_by_incoming.length ) ||
				( Array.isArray( cssVariableDependencies.missing_on_both ) &&
					cssVariableDependencies.missing_on_both.length ) ||
				( Array.isArray( cssVariableDependencies.possibly_external ) &&
					cssVariableDependencies.possibly_external.length )
		);
		if (
		...
		if ( hasTemplateReferenceSummary ) {
			...
			[
				...
				const count = Number(
					templateReferenceSummary[ item[ 1 ] ] || 0
				);
				if ( count > 0 ) {
		...
		const dependencyGroups = [
			{
				...
				action: {
					...
					nounLabel: 'variable',
					helperText:
						'Marks the linked Bricks Global CSS Variable rows to apply from the transfer package.',
				},
			},
			{
				label: 'Missing from this site and the transfer package',
				values: Array.isArray( cssVariableDependencies.missing_on_both )
					? cssVariableDependencies.missing_on_both
					: [],
			},
			{
				label: 'May come from another plugin or custom code',
				values: Array.isArray(
					cssVariableDependencies.possibly_external
				)
					? cssVariableDependencies.possibly_external
					: [],
			},
		...
		if ( categoryDependencies.option_name ) {
			dependencyGroups.push( {
				...
				values: Array.isArray(
					...
					: [],
				action: {
					domainKey: categoryDomainKeyFromOption(
						categoryDependencies.option_name
					),
					metadataOptionName: String(
						categoryDependencies.option_name || ''
					),
					buttonLabel: 'Include These Missing Categories',
					nounLabel: 'category metadata',
					helperText:
						'Marks the related Bricks categories metadata row to apply from the transfer package.',
				},
			} );
			dependencyGroups.push( {
				label: 'Missing categories from this site and the transfer package',
				values: Array.isArray( categoryDependencies.missing_on_both )
					? categoryDependencies.missing_on_both
					: [],
			} );
		}
			...
			dependencyHtml.push(
				renderDependencyGroup( {
					...
					action: {
						...
						nounLabel: 'class',
						helperText:
							'Marks the linked Bricks Global Classes rows to apply from the transfer package when available.',
					},
				} )
			);
		}
		...
		return [
			parts.length
				? '<div class="dbvc-bricks-portability-reference-summary">' +
				  esc( parts.join( ' | ' ) ) +
				  '</div>'
				: '',
			dependencyHtml.length
				? '<div class="dbvc-bricks-portability-reference-analysis"><strong>Linked Items Check</strong><ul>' +
				  dependencyHtml.join( '' ) +
				  '</ul></div>'
				: '',
		].join( '' );
	...
	function hasVerificationReport( report ) {
		return Boolean(
			report &&
				...
				report.status &&
				String( report.status || '' ) !== 'not_required'
		);
	}
	...
	function renderVerificationBlock( title, report ) {
		const detailBits = [];
		if (
			Array.isArray( report.detected_path ) &&
			report.detected_path.length
		) {
		...
		if ( report.recognized_count || report.unrecognized_count ) {
			detailBits.push(
				'Recognized: ' +
					String( report.recognized_count || 0 ) +
					' / ' +
					String( report.entry_count || 0 )
			);
		}
		return [
			'<div class="dbvc-bricks-portability-verification-item">',
			'<div class="dbvc-bricks-portability-verification-item__title">' +
				esc( title ) +
				'</div>',
			'<div class="dbvc-bricks-portability-verification-item__status">' +
				esc( verificationStatusLabel( report.status || '' ) ) +
				'</div>',
			detailBits.length
				? '<div class="description">' +
				  esc( detailBits.join( ' | ' ) ) +
			...
			Array.isArray( report.warnings ) && report.warnings.length
				? '<ul class="dbvc-bricks-portability-verification-item__warnings">' +
				  report.warnings
	...
	function renderVerificationSummary( verification ) {
		const source =
			verification &&
			verification.source &&
			typeof verification.source === 'object'
				? verification.source
				: null;
		const target =
			verification &&
			verification.target &&
			typeof verification.target === 'object'
				? verification.target
				: null;
		const blocks = [];
		...
		if ( hasVerificationReport( source ) ) {
			blocks.push(
				renderVerificationBlock( 'Transfer Package Check', source )
			);
		}
		...
		return (
			'<div class="dbvc-bricks-portability-verification-summary"><strong>Storage Check</strong>' +
			blocks.join( '' ) +
			'</div>'
		);
	}
	...
	function renderWarningList( warnings ) {
		return (
			'<div class="dbvc-bricks-portability-warning-box"><strong>Warnings</strong><ul>' +
			warnings
				.map( function ( warning ) {
	...
	function applyDependencyDecision( domainKey, rowIds, nounLabel ) {
		...
		if ( ! targets.length ) {
			showNotice(
				'error',
				'No linked dependency rows were available to update.'
			);
			return;
		}
		...
		if ( appliedCount > 0 ) {
			state.dependencyActionFeedback[
				dependencyActionKey( domainKey, targets )
			] = {
				status: 'success',
				message:
					'Included ' +
					...
					' row' +
					( appliedCount === 1 ? '' : 's' ) +
					'. Ready to apply selected changes.',
			};
		}
		...
		if ( ! appliedCount ) {
			showNotice(
				'error',
				'No linked dependency rows could be marked to apply from the transfer package.'
			);
			return;
		}
		...
		showNotice(
			...
			'Marked ' +
				String( appliedCount ) +
				' linked dependency row' +
				( appliedCount === 1 ? '' : 's' ) +
				' to apply from the transfer package' +
				( skippedCount
					? '. ' +
					  String( skippedCount ) +
					  ' row' +
	...
	function bindRowDetailActions() {
		...
		Array.prototype.forEach.call(
			els.detail.querySelectorAll(
				'.dbvc-bricks-portability-reference-action'
			),
			function ( button ) {
				button.addEventListener( 'click', function ( event ) {
					...
					applyDependencyDecision(
						String(
							button.getAttribute( 'data-domain-key' ) || ''
						),
						String( button.getAttribute( 'data-row-ids' ) || '' ),
						String(
							button.getAttribute( 'data-noun-label' ) ||
								'dependency'
						)
					);
				} );
	...
	function buildDiffEntries( row, sourcePayload, targetPayload ) {
		const pathSummary =
			row && row.path_summary && typeof row.path_summary === 'object'
				? row.path_summary
				: {};
		const entries = [];

		[ 'changed', 'added', 'removed' ].forEach( function ( bucket ) {
			const paths = Array.isArray( pathSummary[ bucket ] )
				? pathSummary[ bucket ]
				: [];
			paths.forEach( function ( path ) {
				...
				entries.push( {
					...
					currentValue:
						bucket === 'added'
							? undefined
							: getValueAtPath( targetPayload, safePath ),
					incomingValue:
						bucket === 'removed'
							? undefined
							: getValueAtPath( sourcePayload, safePath ),
				} );
			} );
		} );
	...
	function renderDiffEntry( entry ) {
		const bucketClass =
			'dbvc-bricks-portability-diff-entry dbvc-bricks-portability-diff-entry--' +
			safeKey( entry.bucket || '' );
		return [
			'<div class="' + esc( bucketClass ) + '">',
			'<div class="dbvc-bricks-portability-diff-path">',
			'<span class="dbvc-bricks-portability-diff-label">' +
				esc( entry.label || '' ) +
				'</span>',
	...
	function renderValuePreview( value ) {
		...
		return (
			'<pre class="dbvc-bricks-portability-value-preview">' +
			esc( stringifyValue( value ) ) +
			'</pre>'
		);
	}
	...
	function getNormalizedPayload( side ) {
		...
		return Object.prototype.hasOwnProperty.call( side, 'normalized' )
			? side.normalized
			: null;
	}
	...
	function renderDomainSummary() {
		...
		const summaries =
			...
			Array.isArray( state.currentSession.domain_summaries )
				? state.currentSession.domain_summaries
				: [];
		if ( summaries.length === 0 ) {
			els.domainSummary.textContent =
				'Upload a transfer package to load settings group summaries.';
			return;
		}
		...
		summaries.forEach( function ( summary ) {
			const statuses =
				summary && typeof summary.statuses === 'object'
					? summary.statuses
					: {};
			lines.push(
				String(
					...
					' [' +
					domainStatusLabel( summary.domain_status || '' ) +
					']'
			);
			lines.push(
				'Rows: ' +
					...
					String( summary.actionable_rows || 0 ) +
					' | Warnings: ' +
					String( summary.warning_rows || 0 )
			);
			Object.keys( statuses )
				.sort()
				.forEach( function ( statusKey ) {
					lines.push(
						'  - ' +
							statusLabel( statusKey ) +
							': ' +
							String( statuses[ statusKey ] || 0 )
					);
				} );
			lines.push( '' );
		} );
	...
	function renderSummary() {
		...
		if ( ! state.currentSession ) {
			els.summary.textContent =
				'Upload a transfer package to load changes.';
			return;
		}
		const summary = state.currentSession.summary || {};
		els.summary.textContent =
	...
	function buildStatCard( label, value, detail, modifier ) {
		const className =
			'dbvc-bricks-portability-stat-card' +
			( modifier
				? ' dbvc-bricks-portability-stat-card--' + safeKey( modifier )
				: '' );
		return [
			'<div class="' + esc( className ) + '">',
			'<div class="dbvc-bricks-portability-stat-card__label">' +
				esc( label ) +
				'</div>',
			'<div class="dbvc-bricks-portability-stat-card__value">' +
				esc( String( value ) ) +
				'</div>',
			detail
				? '<div class="dbvc-bricks-portability-stat-card__detail">' +
				  esc( detail ) +
				  '</div>'
				: '',
			'</div>',
	...
	function renderStats() {
		...
		if ( ! state.currentSession ) {
			els.stats.textContent =
				'Upload a transfer package to load review totals.';
			return;
		}
		...
		allRows.forEach( function ( row ) {
			...
			const decision = ensureDecision( row || {} );
			if (
				! Object.prototype.hasOwnProperty.call( actionCounts, decision )
			) {
		...
		const cards = [
			buildStatCard(
				'Total Review Rows',
				totalRows,
				visibleRows + ' visible with current filters',
				'rows'
			),
			buildStatCard(
				'Transfer Package',
				incomingCount,
				'rows present in uploaded package',
				'incoming'
			),
			buildStatCard(
				'This Site',
				currentCount,
				'rows present on this site',
				'current'
			),
		];
	...
	function handleFilterChange() {
		const selectedRow = state.selectedRowId
			? getRowById( state.selectedRowId )
			: null;
		const filtered = filteredRows();
		const sortedFiltered = sortedRows( filtered );
		const stillVisible = selectedRow
	...
	function exportSelectedDomains() {
		...
		api( cfg.exportEndpoint, {
			...
			body: JSON.stringify( {
				domains,
				notes: els.exportNotes
					? String( els.exportNotes.value || '' )
					: '',
			} ),
		} )
			.then( function () {
				showNotice(
					'success',
					( cfg.messages && cfg.messages.exported ) ||
			...
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Transfer package creation failed.'
				);
			} );
	}
	...
	function importPackage() {
		const file =
			els.importFile && els.importFile.files
				? els.importFile.files[ 0 ]
				: null;
		if ( ! file ) {
			showNotice(
				'error',
				'Choose a Bricks settings transfer ZIP file to upload.'
			);
			return;
		}
		...
		api( cfg.importEndpoint, {
			...
			.then( function ( session ) {
				showNotice(
					'success',
					( cfg.messages && cfg.messages.imported ) ||
						'Transfer package uploaded.'
				);
			...
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Transfer package upload failed.'
				);
			} );
	}
	...
	function openSession( sessionId ) {
		...
		api(
			String( cfg.sessionEndpointBase || '' ) +
			...
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Failed to load review session.'
				);
			} );
	}
	...
	function refreshCurrentSession() {
		if ( ! state.currentSession || ! state.currentSession.session_id ) {
			showNotice(
				'error',
				'Open a review session before refreshing the comparison.'
			);
			return;
		}

		api(
			String( cfg.sessionRefreshBase || '' ) +
				encodeURIComponent(
					String( state.currentSession.session_id || '' )
				) +
				'/refresh',
			{
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'Idempotency-Key':
						'bricks-portability-refresh-' + Date.now(),
				},
				body: JSON.stringify( {} ),
			}
		)
			.then( function ( session ) {
				showNotice(
					'success',
					( cfg.messages && cfg.messages.refreshed ) ||
			...
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Failed to refresh the comparison.'
				);
			} );
	}
	...
	function saveDraftDecisions() {
		if ( ! state.currentSession ) {
			showNotice(
				'error',
				'Upload a transfer package and review rows before saving a draft.'
			);
			return;
		}

		api(
			String( cfg.sessionDraftBase || '' ) +
				encodeURIComponent(
					String( state.currentSession.session_id || '' )
				) +
				'/draft',
			{
				...
				body: JSON.stringify( {
					...
					manual_decisions: Object.keys(
						state.manualDecisionRowIds || {}
					),
				} ),
			}
		)
			.then( function ( session ) {
				...
				state.decisions = hydrateDraftDecisions( session );
				state.manualDecisionRowIds =
					hydrateManualDecisionRowIds( session );
				state.sessions = (
					Array.isArray( state.sessions ) ? state.sessions : []
				).map( function ( item ) {
					if (
						String(
							item && item.session_id ? item.session_id : ''
				...
				showNotice(
					'success',
					( cfg.messages && cfg.messages.draftSaved ) ||
						'Draft saved.'
				);
			} )
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Failed to save review draft.'
				);
			} );
	}

	function applyBulkAction() {
		const action = els.bulkAction
			? String( els.bulkAction.value || '' )
			: '';
		if ( ! action ) {
			return;
		}
			...
			'Applied ' +
				...
				( skippedCount > 0
					? '. ' +
					  ...
					  ' row' +
					  ( skippedCount === 1 ? ' was' : 's were' ) +
					  ' skipped because that action is not allowed.'
					: '.' )
		);
	}
	...
	function applyApprovedChanges() {
		if ( ! state.currentSession ) {
			showNotice(
				'error',
				'Upload a transfer package and review rows before applying changes.'
			);
			return;
		}
		if ( ! isApplyConfirmed() ) {
			showNotice(
				'error',
				'Confirm that you want to apply these changes before continuing.'
			);
			return;
		}
			...
			.then( function ( result ) {
				showNotice(
					...
					( result && result.message ) ||
						( cfg.messages && cfg.messages.applied ) ||
		...
		api(
				...
				'/rollback',
			{
				...
				headers: {
					'Content-Type': 'application/json',
					'Idempotency-Key':
						'bricks-portability-rollback-' + Date.now(),
				},
				body: JSON.stringify( {
			...
			.then( function () {
				showNotice(
					'success',
					( cfg.messages && cfg.messages.rolledBack ) ||
						'Restore completed.'
				);
				return loadStatus();
			} )
	...
	function loadStatus() {
		return api( cfg.statusEndpoint, { method: 'GET' } )
			.then( function ( data ) {
				state.domains = Array.isArray( data.domains )
					? data.domains
					: [];
				state.exports = Array.isArray( data.recent_exports )
					? data.recent_exports
					: [];
				state.sessions = Array.isArray( data.recent_sessions )
					? data.recent_sessions
					: [];
				state.backups = Array.isArray( data.recent_backups )
					? data.recent_backups
					: [];
				state.jobs = Array.isArray( data.recent_jobs )
					? data.recent_jobs
					: [];
				renderDomains();
				renderExports();
			...
			.catch( function ( error ) {
				showNotice(
					'error',
					error.message || 'Failed to load settings transfer status.'
				);
			} );
	}
	...
	function bindNoticeDismissHandlers() {
		document.addEventListener( 'click', function ( event ) {
			...
			if (
				target.closest( '#dbvc-bricks-notice-success .notice-dismiss' )
			) {
				setNoticeState( els.successNotice, false, '' );
			} else if (
				target.closest( '#dbvc-bricks-notice-error .notice-dismiss' )
			) {
	...
	if ( els.filterDomain ) {
		els.filterDomain.addEventListener(
			'change',
			resetToFirstPageAndRender
		);
	}
	if ( els.filterStatus ) {
		els.filterStatus.addEventListener(
			'change',
			resetToFirstPageAndRender
		);
	}
	if ( els.filterDecision ) {
		els.filterDecision.addEventListener(
			'change',
			resetToFirstPageAndRender
		);
	}
	if ( els.filterWarning ) {
		els.filterWarning.addEventListener(
			'change',
			resetToFirstPageAndRender
		);
	}
	...
	if ( els.hideIdentical ) {
		els.hideIdentical.addEventListener(
			'change',
			resetToFirstPageAndRender
		);
	}
	...
	els.sortButtons.forEach( function ( button ) {
		button.addEventListener( 'click', function () {
			toggleSort(
				String( button.getAttribute( 'data-portability-sort' ) || '' )
			);
		} );
	} );
	...
	if ( els.pageSize ) {
		state.pageSize =
			Number( els.pageSize.value || 50 ) > 0
				? Number( els.pageSize.value || 50 )
				: 50;
	}
	renderReviewState();
	renderAppliedSummary();
