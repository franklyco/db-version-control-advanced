...
### Tasks
- [x] P15-T1 Staging workflow drill execution (Status: DONE)
  - [x] P15-T1-S1 Lock validation dataset + package manifest fixtures used for staging drills (Status: DONE)
  - [x] P15-T1-S2 Execute apply/restore/rollback drill and capture timestamps + operator IDs (Status: DONE)
  - [x] P15-T1-S3 Execute proposal submit/review/transition drill and capture full audit trail (Status: DONE)
...
- [x] P15-T3 Live Bricks schema verification (Status: DONE)
...
- [x] P15-T4 Go/no-go decision package (Status: DONE)
  - [x] P15-T4-S1 Update progress tracker statuses and attach command/output evidence (Status: DONE)
  - [x] P15-T4-S2 Produce release decision summary (`GO` or `NO_GO`) with explicit blocker list (Status: DONE)
  - [x] P15-T4-S3 Open follow-on phases/tasks for non-blocking enhancements discovered in validation (Status: DONE)
    - [x] Queue `DBVC_USER_DOCUMENTATION_LIBRARY` rollout using `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md` as the seed source for future in-plugin documentation library UI (Status: DONE)
    - [x] Queue drift-noise masking/ignore rules enhancement:
      - [x] meta-level masking/ignore support for selected artifact payload paths.
      - [x] option and nested option-object ignore-path support (example: `bricks_color_palette` value paths that differ across sites).
    - [x] Queue UX clarity enhancement for operators:
      - [x] add template title in Differences table rows for template artifacts.
      - [x] surface source site domain metadata in Packages table/detail for direct site/package mapping.
...
### Test Evidence
- P15-TEST-01: PASS (2026-02-15 20:33:16Z) - live apply/restore/rollback drill on `https://dbvc-codexchanges.local` (`operator_id=1`, `user=FranklyCo`):
  - Fixture lock (`P15-T1-S1`): `manifest.package_id=pkg_p15_t1_applydry_20260215T2033Z`, `artifact_uid=option:dbvc_p15_live_probe`, receipt IDs:
    - `pkg_rcpt_p15_t1_applydry_20260215T2033Z` (apply dry-run plan),
    - `pkg_rcpt_p15_t1_restore_20260215T2033Z` (restore create),
    - `pkg_rcpt_p15_t1_applylive_20260215T2033Z` (apply live),
    - `pkg_rcpt_p15_t1_rollback_20260215T2033Z` (rollback).
  - `POST /dbvc/v1/bricks/apply` (`dry_run=true`) -> `200`; plan generated (`option_artifacts=1`).
  - `POST /dbvc/v1/bricks/restore-points` (plan from dry-run) -> `200`; `restore_id=bricks-restore-IxHFPWxCOiE0`.
  - `POST /dbvc/v1/bricks/apply` (`dry_run=false`) -> `200`; verification hash match for `option:dbvc_p15_live_probe`, `restore_id=bricks-restore-130Z3y2K14EY`.
  - `POST /dbvc/v1/bricks/restore-points/bricks-restore-IxHFPWxCOiE0/rollback` -> `200`; `rolled_back=true`.
- P15-TEST-02: PASS (2026-02-15 20:33:50Z) - live proposal submit/review/transition drill on `https://dbvc-codexchanges.local` (`operator_id=1`, `user=FranklyCo`):
  - `POST /dbvc/v1/bricks/proposals` -> `200`; `proposal_id=prop_A8Lu5Rn0Hj`, initial `status=RECEIVED`.
  - Transition sequence (`PATCH /dbvc/v1/bricks/proposals/prop_A8Lu5Rn0Hj`) all returned `200`:
    - `RECEIVED -> NEEDS_CHANGES` (2026-02-15T20:33:46+00:00),
    - `NEEDS_CHANGES -> SUBMITTED` (2026-02-15T20:33:47+00:00),
    - `SUBMITTED -> RECEIVED` (2026-02-15T20:33:49+00:00),
    - `RECEIVED -> APPROVED` (2026-02-15T20:33:50+00:00).
  - `GET /dbvc/v1/bricks/proposals` confirmed final `status=APPROVED` and full `history[]` audit trail with `actor_id=1` on each transition.
...
- P15-TEST-05: PASS (2026-02-15 19:09:02Z) - live compatibility drills:
  ...
  - `POST /dbvc/v1/bricks/packages/publish-remote` dry-run with `schema_version=0.9.0` on `https://dbvc-codexchanges.local`:
...
- P15-TEST-06: PASS (2026-02-15 19:07:30Z) - live schema verification (`GET /dbvc/v1/bricks/schema-verify`) now returns `200` across all three updated sites:
  - `https://dbvc-codexchanges.local` (`generated_at=2026-02-15T19:07:22+00:00`): `bricks_theme_styles` present/array (`entry_count=2`), `bricks_components` count `2`, `label_coverage=100`, `slug_coverage=100`, dominant paths `elements.0.label` + `id`.
  - `https://vf-pluginmediaimporter.local` (`generated_at=2026-02-15T19:07:28+00:00`): `bricks_theme_styles` present/array (`entry_count=2`), `bricks_components` count `2`, `label_coverage=100`, `slug_coverage=100`, dominant paths `elements.0.label` + `id`.
  - `https://flourishweb.co` (`generated_at=2026-02-15T19:07:30+00:00`): `bricks_theme_styles` present/array (`entry_count=2`), `bricks_components` count `3`, `label_coverage=100`, `slug_coverage=100`, dominant paths `elements.0.label` + `id`.
- P15-TEST-07: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase14Test.php` covers force-channel policy behavior:
  - `test_publish_remote_force_stable_requires_confirmation_and_records_audit` validates forced stable confirmation + channel-force audit metadata.
  - `test_promote_requires_stable_confirmation_and_forward_progression` validates stable confirmation and channel progression guards.
  - `test_promote_and_revoke_success_with_confirmations` validates confirmed promote/revoke governance path.
...
- P15-ENH-02: PASS - `vendor/bin/phpunit --filter BricksAddon tests/phpunit` -> `OK (101 tests, 522 assertions)`.
...
### Exit Criteria Check
- [x] All required tests pass with evidence captured in tracker.
- [x] Final go/no-go gate conditions satisfied.
- [x] Any unresolved blocker explicitly tracked with `BLOCKED` status.

### Release Decision (`P15-T4-S2`)
- Decision: `GO`
- Decision timestamp: 2026-02-15 20:38:00Z
- Explicit blockers: none.
- Non-blocking follow-on items (already queued in `P15-T4-S3`):
  - Documentation library rollout (`DBVC_USER_DOCUMENTATION_LIBRARY` seed).
  - Drift-noise masking/ignore-path enhancements.
  - UX clarity improvements (template titles in diffs; source-site metadata in packages UI).

## Phase 16 - Drift masking/ignore rules + raw compare visibility
Status: DONE
Owner: Codex
Started: 2026-02-15
Completed: 2026-02-15
...
### Tasks
- [x] P16-T1 Comparison-rule contract + enforcement (Status: DONE)
  - [x] P16-T1-S1 Add settings contract for path-level ignore/mask rules (`dbvc_bricks_diff_path_rules`) (Status: DONE)
  - [x] P16-T1-S2 Apply ignore/mask rules in drift comparison + summary generation (Status: DONE)
  - [x] P16-T1-S3 Return applied-rule metadata in compare responses (Status: DONE)
- [x] P16-T2 Raw comparison visibility in Differences detail (Status: DONE)
  - [x] P16-T2-S1 Add raw compare payload support in drift scan rows (`options.include_raw_compare`) (Status: DONE)
  - [x] P16-T2-S2 Add raw payload rendering in diff detail panel (local vs golden) (Status: DONE)
  - [x] P16-T2-S3 Add targeted compare endpoint (`POST /dbvc/v1/bricks/drift-compare`) for selected artifact deep inspection (Status: DONE)
- [x] P16-T3 Validation + docs update (Status: DONE)
  - [x] P16-T3-S1 Add automated tests for ignore/mask path rules and drift compare endpoint (Status: DONE)
  - [x] P16-T3-S2 Run regression suite for Differences UX paths (Status: DONE)
  - [x] P16-T3-S3 Capture live UI evidence from staging sites (Status: DONE)

### Test Evidence
- P16-TEST-01: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase16Test.php`:
  - `test_drift_scan_applies_ignore_and_mask_path_rules`
  - `test_drift_compare_endpoint_returns_raw_compare_payload`
  - `test_drift_rules_option_accepts_array_storage_shape`
- P16-TEST-02: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase9Test.php` (differences UX regression remains green).
- P16-TEST-03: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase15Test.php` (phase 15 integration regression remains green).
- P16-TEST-04: PASS - `vendor/bin/phpunit --filter BricksAddon tests/phpunit` -> `OK (101 tests, 522 assertions)`.
- P16-TEST-05: PASS (2026-02-15 21:33:09Z) - live staging evidence capture across all three updated sites:
  - `https://dbvc-codexchanges.local`:
    - `POST /dbvc/v1/bricks/apply` set/reset for `option:dbvc_bricks_diff_path_rules` returned `200`.
    - `POST /dbvc/v1/bricks/drift-compare` -> `200`; `diff_summary.total=0`.
    - `POST /dbvc/v1/bricks/drift-scan` (`include_raw_compare=true`) -> `200`; `status=CLEAN`, `diff_summary.total=0`, mask/ignore paths applied.
  - `https://vf-pluginmediaimporter.local`:
    - `POST /dbvc/v1/bricks/apply` set/reset returned `200`.
    - `POST /dbvc/v1/bricks/drift-compare` -> `200`; `diff_summary.total=0`.
    - `POST /dbvc/v1/bricks/drift-scan` (`include_raw_compare=true`) -> `200`; `status=CLEAN`, `diff_summary.total=0`, mask/ignore paths applied.
  - `https://flourishweb.co`:
    - `POST /dbvc/v1/bricks/apply` set/reset returned `200`.
    - `POST /dbvc/v1/bricks/drift-compare` -> `200`; `diff_summary.total=0`.
    - `POST /dbvc/v1/bricks/drift-scan` (`include_raw_compare=true`) -> `200`; `status=CLEAN`, `diff_summary.total=0`, mask/ignore paths applied.
  - Cross-site assertion: masked token markers (`[masked]`) visible and ignored paths (`last_seen`, `updated_at`) omitted in compare payloads on all sites.
- P16-ENH-01: PASS - UX metadata extension for upcoming clarity tasks:
  - drift rows now include `artifact_label` (template title for `bricks_template`, option key for `option:*` artifacts).
  - package responses now include `source_site_domain` derived from `source_site.base_url`.
  - coverage added in `tests/phpunit/BricksAddonPhase16Test.php`:
    - `test_drift_scan_returns_artifact_label_for_template_and_option`
    - `test_packages_list_and_get_include_source_site_domain`

### Exit Criteria Check
- [x] Operators can configure ignore/mask path rules from Configure.
- [x] Differences detail can display side-by-side raw payload views (masked/ignored rules applied).
- [x] Live staging evidence captured for rules + raw compare behavior on all staging sites.

## Phase 17 - User documentation library rollout
Status: DONE
Owner: Codex
Started: 2026-02-15
Completed: 2026-02-15
...
### Tasks
- [x] P17-T1 Seed-file integration in Bricks Documentation tab (Status: DONE)
  - [x] P17-T1-S1 Add runtime loader for `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md` (Status: DONE)
  - [x] P17-T1-S2 Render parsed user-doc sections in the Bricks Documentation tab (Status: DONE)
  - [x] P17-T1-S3 Keep fallback messaging when seed file is missing/unreadable (Status: DONE)
- [x] P17-T2 Parser and output safety hardening (Status: DONE)
  - [x] P17-T2-S1 Add constrained markdown subset renderer (paragraphs, bullets, inline code) (Status: DONE)
  - [x] P17-T2-S2 Sanitize rendered HTML via allowlist before output (Status: DONE)
- [x] P17-T3 Validation + docs updates (Status: DONE)
  - [x] P17-T3-S1 Extend admin-page tests for documentation library rendering (Status: DONE)
  - [x] P17-T3-S2 Add parser payload test coverage for seed sections (Status: DONE)
  - [x] P17-T3-S3 Update tracker/checklist evidence (Status: DONE)

### Test Evidence
- P17-TEST-01: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase8Test.php`:
  - `test_role_based_tab_visibility_for_client_and_mothership` confirms docs library panel renders for client and mothership views.
  - `test_user_documentation_library_payload_parses_seed_sections` validates seed title/section parsing and inline code list rendering.
- P17-TEST-02: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase13Test.php`.
- P17-TEST-03: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase14Test.php`.
- P17-TEST-04: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase16Test.php`.

### Exit Criteria Check
- [x] Bricks Documentation tab reads from `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md` seed content.
- [x] Seed content renders with safe HTML and preserves operator-facing semantics.
- [x] Regression tests for admin tab rendering remain green.

## Phase 18 - Addon Configure tab + artifact/meta rule expansion
Status: DONE
Owner: Codex
Started: 2026-02-16
...
### Tasks
- [x] P18-T1 Add Configure tab to Bricks submenu page (Status: DONE)
  - [x] P18-T1-S1 Add role-aware `Configure` tab to Bricks addon navigation (Status: DONE)
  - [x] P18-T1-S2 Add submenu-scoped rules form with nonce-protected save flow (Status: DONE)
  - [x] P18-T1-S3 Keep save behavior scoped to rules fields only (no unrelated setting mutation) (Status: DONE)
  - [x] P18-T1-S4 Add Configure sub-tabs: `Basic Settings`, `Advanced Settings`, `Artifacts/Mask + Ignore Rules` and keep rules editor in artifacts/rules section (Status: DONE)
- [x] P18-T2 Expand rules settings contract for artifact/meta masking and ignore (Status: DONE)
  - [x] P18-T2-S1 Add options: `dbvc_bricks_artifact_ignore_rules`, `dbvc_bricks_artifact_mask_rules`, `dbvc_bricks_meta_ignore_rules`, `dbvc_bricks_meta_mask_rules` (Status: DONE)
  - [x] P18-T2-S2 Add field metadata/help text + policy-group exposure for new options (Status: DONE)
- [x] P18-T3 Drift engine rule merge implementation (Status: DONE)
  - [x] P18-T3-S1 Merge base path rules with artifact rules in effective compare contract (Status: DONE)
  - [x] P18-T3-S2 Merge and normalize meta rules (`meta.` prefix normalization) in effective compare contract (Status: DONE)
- [x] P18-T4 Validation + regression updates (Status: DONE)
  - [x] P18-T4-S1 Extend Phase 8 admin tests for Configure tab + rules-save behavior (Status: DONE)
  - [x] P18-T4-S2 Extend Phase 16 drift tests for merged artifact/meta rule behavior (Status: DONE)
  - [x] P18-T4-S3 Run phase regression suites (Status: DONE)
  - [x] P18-T4-S4 Add authenticated REST read/update contract for rules configuration to support cross-site automation/evidence capture (Status: DONE)
- [x] P18-T5 Live staging evidence + docs closure (Status: DONE)
  - [x] P18-T5-S1 Capture live UI evidence for Configure tab and saved rule behavior on staging (Status: DONE)
  - [x] P18-T5-S2 Capture live drift compare evidence showing merged artifact/meta rules (Status: DONE)
  - [x] P18-T5-S3 Mark phase DONE with completion note after live evidence capture (Status: DONE)

### Test Evidence
- P18-TEST-01: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase8Test.php` (`test_role_based_tab_visibility_for_client_and_mothership` now asserts Configure sub-tab IDs, `test_save_rules_configuration_updates_only_rule_fields`).
- P18-TEST-02: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase16Test.php` (`test_expanded_artifact_and_meta_rules_merge_into_compare_payload` + existing rule regression tests).
- P18-TEST-04: PASS - `vendor/bin/phpunit tests/phpunit/BricksAddonPhase16Test.php` (`test_rules_configuration_rest_get_and_partial_post`, `test_rules_configuration_rest_post_rejects_invalid_json_string`).
- P18-REG-01: PASS - `php -l addons/bricks/bricks-addon.php` and `php -l addons/bricks/bricks-drift.php`.
- P18-LIVE-01: PASS (2026-02-16 03:08:26Z-03:14:46Z) - live staging/API evidence for rules save + merged compare behavior on client + mothership sites.
  - Client A (`https://dbvc-codexchanges.local`) save/read checks:
    - `POST /wp-json/dbvc/v1/bricks/configure/rules` -> `200`, `ok=true`, all 5 rule fields persisted in response.
    - `GET /wp-json/dbvc/v1/bricks/configure/rules` -> `200`, returned `fields` + `values` for all 5 rule keys.
  - Client B (`https://vf-pluginmediaimporter.local`) parity checks:
    - `POST /wp-json/dbvc/v1/bricks/configure/rules` -> `200`, `ok=true`.
    - `GET /wp-json/dbvc/v1/bricks/configure/rules` -> `200`.
  - Mothership (`https://flourishweb.co`) parity checks:
    - `POST /wp-json/dbvc/v1/bricks/configure/rules` -> `200`, `ok=true`.
    - `GET /wp-json/dbvc/v1/bricks/configure/rules` -> `200`, returned `fields` + `values` for all 5 rule keys.
  - Live merged-rule drift evidence:
    - `POST /wp-json/dbvc/v1/bricks/drift-scan` on clientA, clientB, and mothership with deterministic manifest/local payload returned `status=CLEAN`.
    - `rules_applied.ignore_paths` included `meta.runtime.last_seen`, `settings.unused`, `meta.runtime.generated_at`.
    - `rules_applied.mask_paths` included `meta.auth.token`, `settings.secret`, `meta.auth.api_key`.

### Exit Criteria Check
- [x] Configure tab exists in Bricks submenu for both client and mothership roles.
- [x] Artifact/meta ignore/mask settings are writable from addon Configure tab.
- [x] Drift compare applies merged base + artifact + meta rule sets.
- [x] Live staging evidence captured and linked before phase close.

### Completion Note (2026-02-16)
- Phase 18 is complete with:
  - addon Configure tab + in-page sub-tabs (`Basic Settings`, `Advanced Settings`, `Artifacts/Mask + Ignore Rules`),
  - expanded artifact/meta ignore+mask rule settings contract,
  - merged effective rule application in drift compare/raw compare output,
  - authenticated REST rules configuration read/update endpoint for automation and evidence capture.
- Live evidence captured across all three sites (`dbvc-codexchanges.local`, `vf-pluginmediaimporter.local`, `flourishweb.co`) with successful save/read + merged-rule drift validation.

## Phase 19 - Network rule distribution + protected artifact variants
Status: NOT_STARTED
Owner: Codex
Started: n/a
Completed: n/a
...
### Tasks
- [ ] P19-T1 Mothership-managed shared rules profile contract (Status: NOT_STARTED)
  - [ ] P19-T1-S1 Define shared rule profile schema and storage contract (`version`, `updated_at`, 5 rule maps, notes) (Status: NOT_STARTED)
  - [ ] P19-T1-S2 Add mothership validation/persistence controls for the shared profile (Status: NOT_STARTED)
  - [ ] P19-T1-S3 Add distribution request/receipt contract (`all` vs `selected`, idempotency, per-site outcomes) (Status: NOT_STARTED)
- [ ] P19-T2 Shared-rules transport and client apply flow (Status: NOT_STARTED)
  - [ ] P19-T2-S1 Add mothership distribute endpoint(s) and operator action wiring (Status: NOT_STARTED)
  - [ ] P19-T2-S2 Add client receive/apply endpoint(s) with signed-command verification and audit events (Status: NOT_STARTED)
  - [ ] P19-T2-S3 Add per-site distribution diagnostics timeline (`queued|sent|applied|failed`) (Status: NOT_STARTED)
- [ ] P19-T3 Client protected artifact variants model (Status: NOT_STARTED)
  - [ ] P19-T3-S1 Define protected variant schema (`artifact_uid`, `artifact_type`, `label`, `reason`, actor/timestamps, scope) (Status: NOT_STARTED)
  - [ ] P19-T3-S2 Add protected variant CRUD endpoints with authorization guards (Status: NOT_STARTED)
  - [ ] P19-T3-S3 Add drift/apply/package payload annotations for protected variant visibility (Status: NOT_STARTED)
- [ ] P19-T4 Client UI: Protected Artifacts tab (Status: NOT_STARTED)
  - [ ] P19-T4-S1 Add `Protected Artifacts` tab in Bricks submenu for clients (Status: NOT_STARTED)
  - [ ] P19-T4-S2 Add list/create/remove controls with audit attribution display (Status: NOT_STARTED)
  - [ ] P19-T4-S3 Add Differences panel actions to mark/unmark selected artifacts as protected variants (Status: NOT_STARTED)
- [ ] P19-T5 Mothership visibility for client protected variants (Status: NOT_STARTED)
  - [ ] P19-T5-S1 Show connected client sites with protected variant counts by artifact class (Status: NOT_STARTED)
  - [ ] P19-T5-S2 Add drill-down listing for client protected variants (`bricks_template`, `global_classes`, etc) (Status: NOT_STARTED)
  - [ ] P19-T5-S3 Add deep-link helper to client `DBVC -> Bricks -> Protected Artifacts` tab (Status: NOT_STARTED)
- [ ] P19-T6 Validation + staging evidence + closure (Status: NOT_STARTED)
  - [ ] P19-T6-S1 Add automated coverage for shared-rule distribution + signed apply flows (Status: NOT_STARTED)
  - [ ] P19-T6-S2 Add automated coverage for protected variant CRUD + UI + mothership visibility (Status: NOT_STARTED)
  - [ ] P19-T6-S3 Capture live staging evidence (`mothership -> clientA/clientB`) and close phase (Status: NOT_STARTED)

### Test Evidence
- P19-TEST-01: NOT_RUN - shared rule profile schema/persistence validation tests.
- P19-TEST-02: NOT_RUN - shared rule distribution transport tests (`all` and `selected`) with idempotency replay checks.
- P19-TEST-03: NOT_RUN - client signed-command verification tests for shared rule apply endpoint.
- P19-TEST-04: NOT_RUN - protected variant CRUD + authorization tests.
- P19-TEST-05: NOT_RUN - client Protected Artifacts tab rendering/interaction tests.
- P19-TEST-06: NOT_RUN - mothership protected-variant aggregation and deep-link rendering tests.
- P19-TEST-07: NOT_RUN - live staging drill evidence across mothership + at least two clients.

### Exit Criteria Check
- [ ] Shared rule profile can be defined once on mothership and distributed to all or selected connected clients with auditable per-site receipts.
- [ ] Client Protected Artifacts tab supports create/list/remove workflows for protected variants.
- [ ] Mothership can identify clients with protected variants and navigate to the client Protected Artifacts view.
- [ ] Required tests are PASS with evidence logged.
