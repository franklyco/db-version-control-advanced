...
- package/schema verification slice implemented:
  ...
  - live schema verification parity now confirmed across all three environments (`https://dbvc-codexchanges.local`, `https://vf-pluginmediaimporter.local`, `https://flourishweb.co`) with `200` responses from `GET /dbvc/v1/bricks/schema-verify`.
  - package policy regression coverage reconfirmed via `vendor/bin/phpunit tests/phpunit/BricksAddonPhase14Test.php` (force-stable confirmation + channel-force audit metadata path).
  - `P15-T1` staging workflow drill executed on `https://dbvc-codexchanges.local` with locked fixtures:
    - apply/restore/rollback flow passed with receipt chain (`pkg_rcpt_p15_t1_applydry_20260215T2033Z`, `pkg_rcpt_p15_t1_restore_20260215T2033Z`, `pkg_rcpt_p15_t1_applylive_20260215T2033Z`, `pkg_rcpt_p15_t1_rollback_20260215T2033Z`) and rollback success (`rolled_back=true`).
    - proposal submit/review/transition flow passed (`proposal_id=prop_A8Lu5Rn0Hj`) with full transition history and `actor_id=1` audit trail.
  - `P15-T4-S2` release decision produced: `GO` with no explicit blockers; follow-on non-blocking enhancements remain queued.
  - next-phase implementation started for drift masking/ignore + raw compare visibility:
    - added settings contract `dbvc_bricks_diff_path_rules` (`ignore`/`mask` path maps),
    - drift scan now applies rules and can return `raw_compare` payloads (`local` vs `golden`) for detail inspection,
    - new endpoint `POST /dbvc/v1/bricks/drift-compare` returns targeted artifact compare payload with applied-rule metadata.
    - live parity probe now passes on all three updated environments (`https://dbvc-codexchanges.local`, `https://vf-pluginmediaimporter.local`, `https://flourishweb.co`): `drift-compare` + `drift-scan` return `200`, `diff_summary.total=0`, `[masked]` tokens present, and ignored paths omitted.
    - metadata contract extended for next UX slice:
      - drift rows now include `artifact_label` (template title/option key friendly label),
      - package list/get payloads include `source_site_domain` for direct site/package mapping.
...
### Entry criteria
- Phase 15 follow-on enhancement queue (`P15-T4-S3`) includes documentation library rollout.
- Seed source file exists: `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md`.

### Tasks / Sub-tasks
- `P17-T1` Seed-file integration in Bricks Documentation tab
  - `P17-T1-S1` Add runtime loader for `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md`.
  - `P17-T1-S2` Render parsed sections in Bricks Documentation tab for both `client` and `mothership`.
  - `P17-T1-S3` Add fallback message when seed file is unavailable.
- `P17-T2` Parser/output safety hardening
  - `P17-T2-S1` Implement constrained markdown subset rendering (paragraphs, bullet lists, inline code).
  - `P17-T2-S2` Apply strict HTML allowlist sanitization before output.
- `P17-T3` Validation + tracker updates
  - `P17-T3-S1` Extend Bricks admin page tests to assert docs library container and seeded section labels.
  - `P17-T3-S2` Add parser payload test for title/section extraction.
  - `P17-T3-S3` Update progress tracker with implementation and test evidence.

### Required tests
- `P17-TEST-01` Bricks Documentation tab rendering includes docs library panel and seeded section content.
- `P17-TEST-02` Docs-library parser extracts title + sections and preserves inline code/list semantics.
- `P17-TEST-03` Regression suite for package/drift phases remains green after docs integration.

### Exit criteria
- Documentation tab is backed by `docs/DBVC_USER_DOCUMENTATION_LIBRARY.md` seed content.
- Output remains safely sanitized with deterministic fallback behavior.
- Required tests pass and are logged in tracker.

## Phase 18: Addon Configure tab + artifact/meta masking and ignore-rule expansion

### Entry criteria
- Phase 17 implementation merged.
- Existing `dbvc_bricks_diff_path_rules` baseline behavior remains green.

### Tasks / Sub-tasks
- `P18-T1` Bricks submenu Configure tab
  - `P18-T1-S1` Add `Configure` tab to Bricks submenu role-aware tab map.
  - `P18-T1-S2` Add nonce-protected rules form in submenu Configure panel.
  - `P18-T1-S3` Keep submenu save path scoped to rules-only fields.
  - `P18-T1-S4` Add Configure sub-tabs (`Basic Settings`, `Advanced Settings`, `Artifacts/Mask + Ignore Rules`) and keep rules form under the artifacts/rules section.
- `P18-T2` Rules settings contract expansion
  - `P18-T2-S1` Add settings keys:
    - `dbvc_bricks_artifact_ignore_rules`
    - `dbvc_bricks_artifact_mask_rules`
    - `dbvc_bricks_meta_ignore_rules`
    - `dbvc_bricks_meta_mask_rules`
  - `P18-T2-S2` Add labels/help text and include fields in policies-group contract.
- `P18-T3` Drift engine effective-rule merge
  - `P18-T3-S1` Merge base `dbvc_bricks_diff_path_rules` with artifact ignore/mask rules.
  - `P18-T3-S2` Merge and normalize meta ignore/mask rules (prefix `meta.` where omitted).
  - `P18-T3-S3` Ensure `rules_applied` payload reflects merged effective paths.
- `P18-T4` Validation and regression
  - `P18-T4-S1` Add/extend tests for Configure tab rendering and rules-only save behavior.
  - `P18-T4-S2` Add/extend tests for merged artifact/meta rule behavior in drift compare output.
  - `P18-T4-S3` Run regression suites covering phase 8 and phase 16.
  - `P18-T4-S4` Add authenticated REST contract for rules configuration read/update to support cross-site automation and evidence capture.
- `P18-T5` Live staging evidence + closure
  - `P18-T5-S1` Capture live Configure-tab save evidence for all new rule fields.
  - `P18-T5-S2` Capture live drift-compare evidence showing merged rule application.
  - `P18-T5-S3` Update tracker to `DONE` with completion note.

### Required tests
- `P18-TEST-01` Configure tab exists for client and mothership and rules save is nonce-protected.
- `P18-TEST-02` Rules save updates only rule fields and preserves unrelated settings.
- `P18-TEST-03` Drift compare returns merged `rules_applied` paths from base + artifact + meta rule sources.
- `P18-TEST-04` Rules configuration REST endpoint supports GET + partial POST update and rejects invalid JSON map payloads.
- `P18-REG-01` Regression suites remain green for previously completed phases.

### Exit criteria
- Bricks submenu has an operator-facing Configure panel for expanded rules.
- Artifact/meta ignore/mask rules are persisted and applied in drift compare output.
- Live staging evidence captured and tracker updated before phase closure.

## Phase 19: Network rule distribution + protected artifact variants

### Entry criteria
- Phase 18 complete.
- Connected-site registry and onboarding states are operational (`P15-T5`, `P13-T4`).

### Tasks / Sub-tasks
- `P19-T1` Mothership-managed shared rules profile contract
  - `P19-T1-S1` Define shared rule profile schema (`version`, `updated_at`, 5 rule maps, optional notes).
  - `P19-T1-S2` Add mothership persistence + validation for shared profile.
  - `P19-T1-S3` Add distribution operation contract (`all` vs `selected` connected sites, idempotent request key, per-site result receipts).
- `P19-T2` Shared-rules transport and apply flow
  - `P19-T2-S1` Add mothership endpoint(s) to publish/distribute shared rule profile to clients.
  - `P19-T2-S2` Add client endpoint(s) to receive/apply shared profile with signed-command verification and audit event.
  - `P19-T2-S3` Add diagnostics timeline entries for distribution (`queued|sent|applied|failed`) per site.
- `P19-T3` Client protected artifact variants data model
  - `P19-T3-S1` Define protected-variant record schema (`artifact_uid`, `artifact_type`, `label`, `reason`, `created_by`, `created_at`, `scope`).
  - `P19-T3-S2` Add CRUD endpoints for protected variants (client-side manage; mothership read aggregation).
  - `P19-T3-S3` Integrate protected-variant state into drift/apply/package context payloads (read-only annotations in this phase).
- `P19-T4` Client UI: Protected Artifacts tab
  - `P19-T4-S1` Add new Bricks submenu tab `Protected Artifacts` (client role required).
  - `P19-T4-S2` Add list/create/remove controls for protected variants with audit attribution.
  - `P19-T4-S3` Add entrypoint actions from Differences detail/table to mark/unmark selected artifact as protected.
- `P19-T5` Mothership visibility for client protected variants
  - `P19-T5-S1` Add mothership panel showing connected clients with protected variant counts by artifact class.
  - `P19-T5-S2` Add drill-down of client protected variants (`bricks_template`, `global_classes`, etc).
  - `P19-T5-S3` Add deep-link helper to client `DBVC -> Bricks -> Protected Artifacts` tab.
- `P19-T6` Validation + staging evidence + closure
  - `P19-T6-S1` Add unit/integration tests for shared-rule distribution contract and signed apply verification.
  - `P19-T6-S2` Add tests for protected-variant CRUD, UI rendering, and mothership aggregation visibility.
  - `P19-T6-S3` Capture live staging drill evidence across mothership + at least two clients and close tracker.

### Required tests
- `P19-TEST-01` Shared-rule profile persistence and validation tests.
- `P19-TEST-02` Shared-rule distribution transport tests (`all` + `selected`) with idempotency replay behavior.
- `P19-TEST-03` Client signed-command verification tests for shared-rule apply endpoint.
- `P19-TEST-04` Protected-variant CRUD tests + authorization checks.
- `P19-TEST-05` Client Protected Artifacts tab render/interaction tests.
- `P19-TEST-06` Mothership aggregation visibility tests + client deep-link rendering tests.
- `P19-TEST-07` Live staging drill (`mothership -> clientA/clientB`) with receipts, diagnostics, and resulting protected-variant visibility evidence.

### Exit criteria
- Mothership can define one shared masking/ignore rules profile and distribute it to all or selected connected sites with auditable receipts.
- Client operators can manage Protected Artifact Variants in a dedicated Bricks tab.
- Mothership operators can see which client sites have protected variants and can navigate directly to each client Protected Artifacts view.
- Required tests pass and tracker evidence is complete.
...
- Network-wide shared rules distribution:
  - owner: `P19-T1` + `P19-T2`.
  - define once on mothership; distribute to all/selected connected clients with signed delivery receipts.
- Protected Artifact Variants:
  - owner: `P19-T3` + `P19-T4` + `P19-T5`.
  - client-managed protected variants registry and tab, plus mothership cross-client visibility and deep-link navigation.
