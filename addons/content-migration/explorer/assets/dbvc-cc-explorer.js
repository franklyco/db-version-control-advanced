jQuery( function ( $ ) {
	...
	const mappingBridgeEnabled =
		mappingCatalogBridgeEnabled && mediaMappingBridgeEnabled;
	...
	const $contentContextScrubPreviewBtn = $(
		'#cc-content-context-scrub-preview'
	);
	const $contentContextScrubApproveBtn = $(
		'#cc-content-context-scrub-approve'
	);
	const $contentContextScrubStatusBtn = $(
		'#cc-content-context-scrub-status'
	);
	...
	function formatAiStatus( status, mode ) {
		...
		const modeLabel =
			modeLabelMap[ String( mode || '' ).toLowerCase() ] || '';
	...
	function dbvc_cc_get_domain_ai_health( domainKey ) {
		const dbvc_cc_key = String( domainKey || '' )
			.trim()
			.toLowerCase();
		if (
			! dbvc_cc_key ||
			! state.dbvc_cc_domain_ai_health_by_key ||
			typeof state.dbvc_cc_domain_ai_health_by_key !== 'object'
		) {
			return null;
		}
	...
	function dbvc_cc_build_domain_option_label( domainRecord ) {
		const dbvc_cc_domain_key =
			domainRecord && domainRecord.key ? String( domainRecord.key ) : '';
		const dbvc_cc_domain_label =
			domainRecord && domainRecord.label
				? String( domainRecord.label )
				: dbvc_cc_domain_key;
	...
	function dbvc_cc_render_domain_ai_warning( domainKey ) {
		...
		$domainAiWarning
			...
			.attr(
				'title',
				`${ dbvc_cc_message } Click to run a full-domain AI refresh.`
			);
	}

	async function dbvc_cc_run_domain_ai_refresh_from_warning() {
		const dbvc_cc_domain_key = String( $domain.val() || '' )
			.trim()
			.toLowerCase();
		...
		if ( ! dbvc_cc_domain_has_warning( dbvc_cc_domain_key ) ) {
			setStatus(
				`No AI warning is currently flagged for ${ dbvc_cc_domain_key }.`
			);
			return;
		}
		...
		setStatus(
			`Queueing full-domain AI refresh for ${ dbvc_cc_domain_key }...`
		);
		...
		try {
			...
			const dbvc_cc_batch_id =
				dbvc_cc_response && dbvc_cc_response.batch_id
					? String( dbvc_cc_response.batch_id )
					: '';
			const dbvc_cc_total_jobs = Number(
				dbvc_cc_response && dbvc_cc_response.total_jobs
					? dbvc_cc_response.total_jobs
					: 0
			);
			setStatus(
				dbvc_cc_batch_id
			...
			setStatus(
				error && error.message
					? error.message
					: 'Failed to queue domain AI refresh.',
				true
			);
			dbvc_cc_render_domain_ai_warning( dbvc_cc_domain_key );
		}
	...
	async function dbvc_cc_load_content_context( pathOverride = '' ) {
		...
		if ( ! state.domain || ! targetPath ) {
			dbvc_cc_set_content_context_status(
				'Select a page or section node to inspect content context.',
				true
			);
		...
		const viewModel = {
			domain:
				response && response.domain ? response.domain : state.domain,
			path: response && response.path ? response.path : targetPath,
			artifact:
				response && response.artifact ? response.artifact : artifact,
		...
		const payloadCount =
			viewModel.payload && typeof viewModel.payload === 'object'
				? Object.keys( viewModel.payload ).length
				: 0;
		dbvc_cc_set_content_context_status(
			`Loaded ${ payloadCount } context payload block(s) for ${ targetPath }.`
		);
	}
	...
	async function dbvc_cc_load_scrub_policy_preview( pathOverride = '' ) {
		...
		if ( ! state.domain || ! targetPath ) {
			dbvc_cc_set_content_context_status(
				'Select a page node to preview scrub policy suggestions.',
				true
			);
			$contentContextJson.text( '' );
			return;
		...
		const sampleSize = dbvc_cc_normalize_limit(
			$contentContextLimit.val(),
			40
		);
		$contentContextLimit.val( sampleSize );
		dbvc_cc_set_content_context_status(
			'Loading deterministic scrub preview...'
		);
		...
		dbvc_cc_set_content_context_status(
			`Loaded scrub preview for ${ targetPath }.`
		);
	}

	async function dbvc_cc_load_scrub_policy_approval_status(
		pathOverride = ''
	) {
		const targetPath = String( pathOverride || state.selectedPath || '' );
		const params = {};
	...
	async function dbvc_cc_approve_scrub_policy_suggestions(
		pathOverride = ''
	) {
		const targetPath = String( pathOverride || state.selectedPath || '' );
		if ( ! state.domain || ! targetPath ) {
			dbvc_cc_set_content_context_status(
				'Select a page node to approve scrub suggestions.',
				true
			);
			return;
		}
		...
		$contentContextLimit.val( sampleSize );
		dbvc_cc_set_content_context_status(
			'Applying approved scrub suggestions to Configure defaults...'
		);
		...
		$contentContextJson.text( JSON.stringify( response || {}, null, 2 ) );
		dbvc_cc_set_content_context_status(
			'Applied scrub suggestions to Configure defaults.'
		);
	}
	...
	function getNodeByPath( path ) {
		...
		const node = state.cy
			.nodes()
			.filter( ( item ) => ( item.data( 'path' ) || '' ) === path )
			.first();
		if ( node && node.length ) {
			return node;
		}
	...
	function runLayout( layoutMode ) {
		...
		if ( 'linear_grid' === mode ) {
			const visibleNodes = sortNodesByDepthAndPath(
				state.cy
					.nodes()
	...
	function refreshCptFilterOptions() {
		...
		const sorted = Array.from( cpts ).sort( ( a, b ) =>
			a.localeCompare( b )
		);
		$viewCpt.empty();
		...
		sorted.forEach( ( cpt ) => {
			$viewCpt.append(
				`<option value="${ escapeHtml( cpt ) }">${ escapeHtml(
					cpt
				) }</option>`
			);
		} );
	...
	function applyViewState( runLayoutAfter = true ) {
		...
		if ( state.isolatedRootId ) {
			const isolatedNode = state.cy.getElementById(
				state.isolatedRootId
			);
			if ( isolatedNode && isolatedNode.length ) {
				isolatedNodeIds = collectBranchViewNodeIds(
					state.isolatedRootId
				);
			} else {
				state.isolatedRootId = '';
		...
		state.cy.nodes().forEach( ( node ) => {
			...
			const passesSearch =
				! searchQuery ||
				( searchableVisibleIds && searchableVisibleIds.has( id ) );
	...
	function syncViewStateFromControls() {
		...
		state.view.collapseLevel = Number.isNaN( collapseLevelRaw )
			? 0
			: Math.max( 0, Math.min( 12, collapseLevelRaw ) );
	...
	function updateNodeActionButtons() {
		...
		$nodeExpandBranchBtn.prop(
			'disabled',
			! ( navigableType && hasChildren )
		);
		$nodeToggleBranchBtn.prop(
			'disabled',
			! ( navigableType && hasChildren )
		);
		$nodeOpenSourceBtn.prop(
			'disabled',
			! normalizeUrl( state.selectedSourceUrl )
		);
		$nodeOpenCanonicalBtn.prop(
			'disabled',
			! normalizeUrl( state.selectedCanonicalUrl )
		);
	...
	function updateInspectorMeta( nodeData, detailData ) {
		...
		const sourceUrl =
			detailNode && detailNode.artifact
				? normalizeUrl( detailNode.artifact.source_url )
				: '';
		const canonicalUrl =
			detailNode && detailNode.artifact
				? normalizeUrl( detailNode.artifact.canonical_source_url )
				: '';
		...
		state.selectedNodeId =
			nodeData.id ||
			( selectedGraphNode && selectedGraphNode.length
		...
		$exportSelectedBtn.prop(
			'disabled',
			! exportEnabled || ! state.selectedPath
		);
		$rerunAiBtn.prop(
			'disabled',
			! aiEnabled || ! ( isPageLike && hasJson )
		);
		...
		$( '#cc-node-images' ).text(
			String( detailNode.image_count || nodeData.image_count || 0 )
		);
		$( '#cc-node-crawl-status' ).text(
			detailNode.status && detailNode.status.crawl
				? detailNode.status.crawl
				: 'unknown'
		);
		const aiStatus =
			detailNode.status && detailNode.status.analysis
				? detailNode.status.analysis
				: 'not_started';
		const aiMode =
			detailNode.status && detailNode.status.ai_mode
				? detailNode.status.ai_mode
				: '';
		...
		dbvc_cc_render_badges( '#cc-node-mode-badges', [
			{
				key: 'capture_mode',
				label:
					detailNode && detailNode.status
						? detailNode.status.capture_mode
						: '',
			},
			{
				key: 'section_typing_mode',
				label:
					detailNode && detailNode.status
						? detailNode.status.section_typing_mode
						: '',
			},
			{
				key: 'scrub_profile',
				label:
					detailNode && detailNode.status
						? detailNode.status.scrub_profile
						: '',
			},
		] );
	...
	function updateAuditTrail( auditData ) {
		...
		const stageCounts =
			summary.stage_counts && typeof summary.stage_counts === 'object'
		...
		const pipelineCounts =
			summary.pipeline_counts &&
			typeof summary.pipeline_counts === 'object'
				? summary.pipeline_counts
				: {};
		const stageSummary =
		...
		$( '#cc-content-audit-summary' ).text(
			`Events: ${ total } | Stages: ${ stageSummary } | Status: ${ statusSummary } | Pipelines: ${ pipelineSummary }`
		);

		const events = Array.isArray( auditData && auditData.events )
			? auditData.events
			: [];
		...
		events.forEach( ( event ) => {
			const timestamp =
				event && event.timestamp
					...
					: 'unknown-time';
			const stage =
				event && event.stage ? String( event.stage ) : 'unknown';
			const status =
				event && event.status ? String( event.status ) : 'unknown';
			const path = event && event.path ? String( event.path ) : '';
			const pipelineId =
				event && event.pipeline_id ? String( event.pipeline_id ) : '';
			const message =
				event && event.message ? String( event.message ) : '';
			const failureCode =
				event && event.failure_code ? String( event.failure_code ) : '';
			const summaryLine = [
		...
		if ( events.length === 0 ) {
			$auditEvents.append(
				$( '<li></li>' ).text( 'No audit events for selected scope.' )
			);
		}
	}
	...
	function updateContentPreview( contentData ) {
		const content =
			contentData && contentData.content ? contentData.content : {};
		const pii =
			contentData && contentData.pii_flags ? contentData.pii_flags : {};
		const analysis =
			contentData && contentData.analysis ? contentData.analysis : {};
		const metrics =
			contentData && contentData.metrics ? contentData.metrics : {};
		const readiness =
			contentData && contentData.readiness ? contentData.readiness : {};
		const comparison =
			contentData && contentData.comparison ? contentData.comparison : {};
		const phase36 =
			contentData && contentData.phase36 ? contentData.phase36 : {};
		...
		const headings = Array.isArray( content.headings )
			? content.headings
			: [];
		const excerpt = Array.isArray( content.text_excerpt )
		...
		sections.slice( 0, 12 ).forEach( ( section, index ) => {
			const level = Number(
				section && section.level ? section.level : 0
			);
			const heading =
				section && section.heading
					...
					: '(intro section)';
			const tag =
				section && section.heading_tag
					? String( section.heading_tag ).toUpperCase()
					: '';
			const textBlocks = Array.isArray( section && section.text_blocks )
				? section.text_blocks
				: [];
			const links = Array.isArray( section && section.links )
				? section.links
				: [];
			const ctas = Array.isArray( section && section.ctas )
				? section.ctas
				: [];
			const images = Array.isArray( section && section.images )
				? section.images
				: [];

			const summaryPrefix = tag
				? `${ tag }: `
				: level > 0
				? `H${ level }: `
				: 'Intro: ';
			...
			const $details = $(
				'<details class="cc-section-group"></details>'
			);
			if ( index === 0 ) {
			...
			if ( links.length > 0 ) {
				...
				links.forEach( ( link ) => {
					const text =
						link && link.text ? String( link.text ) : 'Link';
			...
			if ( images.length > 0 ) {
				...
				images.forEach( ( image ) => {
					const name =
						image && image.local_filename
							? String( image.local_filename )
							: '';
					const source = normalizeUrl(
						image && image.source_url ? image.source_url : ''
					);
			...
			if (
				...
				$body.append(
					$( '<p></p>' ).text(
						'No content captured in this section.'
					)
				);
			}
		...
		if ( sections.length === 0 ) {
			$sectionsRoot.append(
				$( '<p></p>' ).text( 'No grouped sections available.' )
			);
		}
		$( '#cc-content-section-count' ).text(
		...
		const piiMessage = `Emails: ${ pii.emails_count || 0 }, Phones: ${
			pii.phones_count || 0
		}, Forms: ${ pii.forms_count || 0 }, Legal review: ${
			pii.requires_legal_review ? 'Yes' : 'No'
		}`;
		$( '#cc-content-pii' ).text( piiMessage );

		const categoriesRaw = Array.isArray( analysis.categories )
			? analysis.categories
			: [];
		const categories = categoriesRaw
			...
			.filter( Boolean );
		$( '#cc-content-ai-status' ).text(
			`Status: ${ formatAiStatus(
				analysis.status || 'not_started',
				''
			) }`
		);
		$( '#cc-content-ai-post-type' ).text(
			`Post Type: ${ analysis.post_type || 'n/a' }${
				analysis.post_type_confidence != null
					? ` (${ analysis.post_type_confidence })`
					: ''
			}`
		);
		$( '#cc-content-ai-categories' ).text(
			`Categories: ${
				categories.length ? categories.join( ', ' ) : 'n/a'
			}`
		);
		$( '#cc-content-ai-summary-text' ).text(
		...
		const signals = [
			...
			`Headings: ${
				metrics.headings_count != null
					? metrics.headings_count
					: headings.length
			} (H1: ${ metrics.h1_count != null ? metrics.h1_count : 0 })`,
			`Text Blocks: ${
				metrics.text_blocks_count != null
					? metrics.text_blocks_count
					: excerpt.length
			} | Words: ${
				metrics.word_count != null ? metrics.word_count : 'n/a'
			}`,
			`Sections: ${
				metrics.section_count != null
					? metrics.section_count
					: sectionCount
			}`,
			`Links/CTAs: ${
				metrics.links_count != null ? metrics.links_count : 0
			}/${ metrics.ctas_count != null ? metrics.ctas_count : 0 }`,
			`Images: ${
		...
		const phase36Badges = Array.isArray( phase36.badges )
			? phase36.badges
			: [];
		dbvc_cc_render_badges( '#cc-content-phase36-badges', phase36Badges );

		const phase36Available =
			phase36.available && typeof phase36.available === 'object'
				? phase36.available
				: {};
		const availableSummary = Object.keys( phase36Available )
			.map(
				( key ) =>
					`${ key }: ${ phase36Available[ key ] ? 'yes' : 'no' }`
			)
			.join( ' | ' );
		$( '#cc-content-phase36-availability' ).text(
			`Artifacts: ${ availableSummary || 'none' }`
		);

		const phase36SummaryLines = [];
		const phase36Elements =
			phase36.elements && typeof phase36.elements === 'object'
				? phase36.elements
				: {};
		const phase36Sections =
			phase36.sections && typeof phase36.sections === 'object'
				? phase36.sections
				: {};
		const phase36Context =
			phase36.context_bundle && typeof phase36.context_bundle === 'object'
				? phase36.context_bundle
				: {};
		const phase36Ingestion =
			phase36.ingestion_package &&
			typeof phase36.ingestion_package === 'object'
				? phase36.ingestion_package
				: {};
		const phase36ElementsProcessing =
			phase36Elements.processing &&
			typeof phase36Elements.processing === 'object'
				? phase36Elements.processing
				: {};
		const phase36SectionsProcessing =
			phase36Sections.processing &&
			typeof phase36Sections.processing === 'object'
				? phase36Sections.processing
				: {};
		phase36SummaryLines.push(
			`Elements: ${ Number( phase36Elements.element_count || 0 ) }${
				phase36Elements.truncated ? ' (truncated)' : ''
			}`
		);
		phase36SummaryLines.push(
			`Sections v2: ${ Number( phase36Sections.section_count || 0 ) }`
		);
		if ( phase36ElementsProcessing.is_partial ) {
			phase36SummaryLines.push(
				`Extract partial: ${ String(
					phase36ElementsProcessing.partial_reason || 'unknown'
				) }`
			);
		}
		if ( phase36SectionsProcessing.is_partial ) {
			phase36SummaryLines.push(
				`Segment partial: ${ String(
					phase36SectionsProcessing.partial_reason || 'unknown'
				) }`
			);
		}
		phase36SummaryLines.push(
			`Context bundle: outlines ${ Number(
				phase36Context.outline_count || 0
			) }, sections ${ Number( phase36Context.section_count || 0 ) }`
		);
		phase36SummaryLines.push(
			`Ingestion package: sections ${ Number(
				phase36Ingestion.section_count || 0
			) }, elements ${ Number( phase36Ingestion.element_count || 0 ) }`
		);
		...
		const typeCounts =
			phase36.section_typing &&
			phase36.section_typing.type_counts &&
			typeof phase36.section_typing.type_counts === 'object'
				? phase36.section_typing.type_counts
				: {};
		...
		Object.keys( typeCounts ).forEach( ( key ) => {
			$phase36Types.append(
				$( '<li></li>' ).text(
					`Type ${ key }: ${ Number( typeCounts[ key ] || 0 ) }`
				)
			);
		} );
		if ( $phase36Types.children().length === 0 ) {
			$phase36Types.append(
				$( '<li></li>' ).text( 'No section-typing totals available.' )
			);
		}

		const scrubTotals =
			phase36.scrub_report &&
			phase36.scrub_report.totals &&
			typeof phase36.scrub_report.totals === 'object'
				? phase36.scrub_report.totals
				: {};
		const scrubProfile =
			phase36.scrub_report && phase36.scrub_report.profile
				? String( phase36.scrub_report.profile )
				: 'n/a';
		const $phase36Scrub = $( '#cc-content-phase36-scrub-totals' ).empty();
		$phase36Scrub.append(
			$( '<li></li>' ).text(
				`Scrub profile: ${ dbvc_cc_normalize_mode_label(
					scrubProfile
				) }`
			)
		);
		Object.keys( scrubTotals ).forEach( ( key ) => {
			$phase36Scrub.append(
				$( '<li></li>' ).text(
					`${ key }: ${ Number( scrubTotals[ key ] || 0 ) }`
				)
			);
		} );
		if ( Object.keys( scrubTotals ).length === 0 ) {
			$phase36Scrub.append(
				$( '<li></li>' ).text( 'No scrub totals available.' )
			);
		}
		...
		const readinessStatus =
			readinessStatusMap[
				String( readiness.status || '' ).toLowerCase()
			] || 'Needs Review';
		...
		readinessChecks.forEach( ( check ) => {
			...
			const label =
				check && check.label ? String( check.label ) : 'Check';
			$readinessChecks.append(
				$( '<li></li>' ).text(
					`${ passed ? 'PASS' : 'REVIEW' }: ${ label }`
				)
			);
		} );
		if ( readinessChecks.length === 0 ) {
			$readinessChecks.append(
				$( '<li></li>' ).text( 'No readiness checks available.' )
			);
		}

		const readinessNotes = Array.isArray( readiness.notes )
			? readiness.notes
			: [];
		...
		if ( readinessNotes.length === 0 ) {
			$readinessNotes.append(
				$( '<li></li>' ).text( 'No additional migration notes.' )
			);
		}

		const rawExcerpt = Array.isArray( comparison.raw_excerpt )
			? comparison.raw_excerpt
			: [];
		const sanitizedExcerpt = Array.isArray( comparison.sanitized_excerpt )
			? comparison.sanitized_excerpt
			: [];
		const totalLines = Number(
			comparison.total_lines ||
				Math.max( rawExcerpt.length, sanitizedExcerpt.length )
		);
		const changedLines = Number( comparison.changed_lines || 0 );
		const changedPercent =
			totalLines > 0
				? Math.round( ( changedLines / totalLines ) * 100 )
				: 0;
		$( '#cc-content-diff-summary' ).text(
			`Changed lines: ${ changedLines }/${ totalLines } (${ changedPercent }%)`
		);

		const compareLen = Math.max(
			rawExcerpt.length,
			sanitizedExcerpt.length
		);
		...
		if ( compareLen === 0 ) {
			$rawDiff.append(
				$( '<li></li>' ).text( 'No raw excerpt available.' )
			);
			$sanitizedDiff.append(
				$( '<li></li>' ).text( 'No sanitized excerpt available.' )
			);
		}
	...
	function initGraph( elements ) {
		if ( typeof cytoscape !== 'function' ) {
			throw new Error(
				'Cytoscape.js failed to load. Check network access and retry.'
			);
		}
		...
		state.cy = cytoscape( {
			...
			style: [
				...
				{
					selector:
						'node[analysis_status = "queued"], node[analysis_status = "processing"]',
					style: {
				...
				{
					selector:
						'node[analysis_status = "done"], node[analysis_status = "fallback_done"]',
					style: {
		...
		state.cy.on( 'tap', 'node', function ( event ) {
			...
			handleNodeClick( node.data() ).catch( ( error ) => {
				setStatus(
					error.message || 'Failed to load node details.',
					true
				);
			} );
		} );
	...
	async function ensureNodeChildrenLoaded( nodeData ) {
		...
		const hasChildren =
			nodeData.type !== 'file' &&
			Number( nodeData.children_count || 0 ) > 0;
		if ( ! hasChildren ) {
			return false;
		}
	...
	async function expandBranchFromNode( nodeData, levels = 2 ) {
		...
		const levelLimit = Math.max(
			1,
			Math.min( 4, parseInt( levels, 10 ) || 2 )
		);
		...
		while ( queue.length > 0 ) {
			...
			const hasChildren =
				Number( nodePayload.children_count || 0 ) > 0 &&
				nodePayload.type !== 'file';
			if ( ! hasChildren ) {
				continue;
			}
			...
			state.cy
				.edges()
				.filter( ( edge ) => edge.data( 'source' ) === current.nodeId )
				.forEach( ( edge ) => {
					const targetId = edge.data( 'target' );
					if ( ! targetId ) {
						return;
	...
	async function refreshSelectedNodeMeta() {
		...
		if ( state.cy ) {
			const selectedNode = state.cy
				.nodes()
				.filter(
					( node ) =>
						( node.data( 'path' ) || '' ) === state.selectedPath
				)
				.first();
	...
	function pollAiStatus( params = {} ) {
		...
		const pollParams = jobId ? { job_id: jobId } : { domain, path };
		if ( jobId ) {
			state.aiJobId = jobId;
		}
			...
			try {
				...
				const statusValue =
					statusPayload && statusPayload.status
						...
						: 'unknown';
				const modeValue =
					statusPayload && statusPayload.mode
						? String( statusPayload.mode ).toLowerCase()
						: '';
				const message =
					statusPayload && statusPayload.message
					...
					const mappedStatus =
						statusValue === 'completed'
							? modeValue === 'fallback'
								...
								: 'done'
							: statusValue;
					$( '#cc-node-ai-status' ).text(
						formatAiStatus( mappedStatus, modeValue )
					);
				}
				...
				if ( statusValue === 'completed' || statusValue === 'failed' ) {
					...
					if ( state.cy && state.selectedPath === path ) {
						...
						if ( graphNode && graphNode.length ) {
							...
							graphNode.data(
								'status',
								statusValue === 'failed' ? 'error' : 'ready'
							);
						}
					}
				}
	...
	function renderBatchProgress( payload ) {
		const status = String(
			payload && payload.status ? payload.status : 'queued'
		);
		const totalJobs = Number(
			payload && payload.total_jobs ? payload.total_jobs : 0
		);
		const processedJobs = Number(
			payload && payload.processed_jobs ? payload.processed_jobs : 0
		);
		const progressPercent = Number(
			payload && payload.progress_percent ? payload.progress_percent : 0
		);
		const counts = payload && payload.counts ? payload.counts : {};
		const completed =
			Number( counts.completed_ai || 0 ) +
			Number( counts.completed_fallback || 0 );
			...
			try {
				...
				if ( done ) {
					...
					if ( status === 'completed_with_failures' ) {
						setStatus(
							'Branch AI rerun finished with failures. Review branch status details.',
							true
						);
					} else {
				...
				$aiBranchStatus.text( '' );
				setStatus(
					error.message || 'Branch AI status polling failed.',
					true
				);
			}
		};
	...
	async function handleNodeClick( nodeData ) {
		...
		if ( canExpand && ! state.loadedChildren.has( nodeData.id ) ) {
			setStatus(
				`Loading children for ${ nodeData.label || nodeData.id }...`
			);
		...
		updateInspectorMeta( nodeData, detail );
		const detailStatus =
			detail && detail.node && detail.node.status
				? detail.node.status
				: {};
		const aiState = detailStatus.analysis
			? String( detailStatus.analysis ).toLowerCase()
			: '';
		if (
			state.selectedType === 'page' &&
			( aiState === 'queued' || aiState === 'processing' )
		) {
			pollAiStatus( {
		...
		const previewEligible =
			nodeData.type === 'page' || nodeData.type === 'section';
		if ( ! previewEligible ) {
			$contentPreview.addClass( 'cc-hidden' );
			dbvc_cc_set_content_context_status(
				'Content context is available for page/section nodes only.'
			);
			$contentContextJson.text( '' );
		...
		dbvc_cc_load_content_context( path ).catch( ( error ) => {
			dbvc_cc_set_content_context_status(
				error.message || 'Failed to load content context.',
				true
			);
			$contentContextJson.text( '' );
		} );
	...
	async function loadTree() {
		...
		const depth = Math.max(
			1,
			Math.min( 5, parseInt( $depth.val(), 10 ) || defaults.depth || 2 )
		);
		const maxNodes = Math.max(
			100,
			Math.min(
				2000,
				parseInt( $maxNodes.val(), 10 ) || defaults.max_nodes || 600
			)
		);
		...
		const elements = [];
		( payload.cytoscape && payload.cytoscape.nodes
			? payload.cytoscape.nodes
			: []
		).forEach( ( node ) => elements.push( node ) );
		( payload.cytoscape && payload.cytoscape.edges
			? payload.cytoscape.edges
			: []
		).forEach( ( edge ) => elements.push( edge ) );
		...
		setStatus(
			`Loaded ${ elements.length } graph elements. Pages: ${
				totals.pages || 0
			}, Media: ${ totals.media_files || 0 }, Cache: ${
				payload.scan_mode || 'fresh'
			}.`
		);
	}
	...
	async function loadDomains() {
		...
		domains.forEach( ( dbvc_cc_domain_record ) => {
			...
			state.dbvc_cc_domain_ai_health_by_key[ dbvc_cc_domain_key ] =
				dbvc_cc_health;
		} );
		...
		if ( domains.length === 0 ) {
			$domain.append(
				'<option value="">No crawl domains found</option>'
			);
			$domainAiWarning.addClass( 'cc-hidden' ).text( '' );
			setStatus(
				'No crawl domains found. Run a sitemap crawl first.',
				true
			);
			return;
		}
		...
		domains.forEach( ( domain ) => {
			...
			const dbvc_cc_option_label =
				dbvc_cc_build_domain_option_label( domain );
			$domain.append(
	...
	async function runExport( scope ) {
		...
		const aiStatus =
			response.ai && response.ai.status ? response.ai.status : 'unknown';
		const pages =
			response.totals && response.totals.pages
				? response.totals.pages
				: 0;

		if ( downloadUrl ) {
			setExportStatus(
				`Export complete. Pages: ${ pages }. AI status: ${ aiStatus }. <a href="${ escapeHtml(
					downloadUrl
				) }" target="_blank" rel="noopener noreferrer">Download ZIP</a>`,
				false,
				true
			);
			return;
		...
		setExportStatus(
			`Export completed but no download URL was returned. Job: ${ escapeHtml(
				response.job_id || ''
			) }`,
			true
		);
	...
	async function rerunAiForSelectedPath() {
		...
		if ( ! ( state.selectedType === 'page' && state.selectedHasJson ) ) {
			setStatus(
				'Rerun AI is only available for page nodes with JSON artifacts.',
				true
			);
			return;
		}
	...
	async function rerunAiForSelectedBranch() {
		...
		if ( state.selectedType === 'file' ) {
			setStatus(
				'Branch AI rerun is not available for file nodes.',
				true
			);
			return;
		}
		...
		const batchId =
			response && response.batch_id ? String( response.batch_id ) : '';
		$rerunAiBranchBtn.prop( 'disabled', true );
		$aiBranchStatus.text(
			response && response.message
				? String( response.message )
				: 'Branch AI rerun queued.'
		);
		setStatus(
			`Branch rerun queued${ batchId ? ` (${ batchId })` : '' }.`
		);
		if ( batchId ) {
	...
	function fitSelectedBranch() {
		...
		setStatus(
			`Fitted branch for ${ node.data( 'label' ) || node.id() }.`
		);
	}
	...
	function isolateSelectedBranch() {
		...
		if ( String( node.data( 'type' ) || '' ) === 'file' ) {
			setStatus(
				'Branch isolation is only available for directory/page nodes.',
				true
			);
			return;
		}
		...
		updateNodeActionButtons();
		setStatus(
			`Isolated branch for ${ node.data( 'label' ) || node.id() }.`
		);
	}
	...
	async function expandSelectedBranch() {
		...
		const levels = Math.max(
			1,
			Math.min( 4, parseInt( $nodeExpandLevels.val(), 10 ) || 2 )
		);
		...
		if ( result.stoppedEarly ) {
			setStatus(
				`Expanded ${ result.expanded } nodes (request limit reached). Refine branch and retry.`,
				true
			);
		} else {
			setStatus(
				`Expanded ${ result.expanded } node(s) from ${
					node.data( 'label' ) || node.id()
				}.`
			);
		}
	...
	function toggleSelectedBranch() {
		...
		setStatus(
			`${ nextCollapsedState ? 'Collapsed' : 'Expanded' } branch for ${
				node.data( 'label' ) || node.id()
			}.`
		);
	}
	...
	function openMappingWorkbenchForSelection() {
		...
		if ( ! state.domain || ! state.selectedPath ) {
			setStatus(
				'Select a page or section node before opening mapping.',
				true
			);
			return;
		}
	...
	$( '#cc-explorer-refresh' ).on( 'click', function () {
		loadTree().catch( ( error ) => {
			setStatus(
				error.message || 'Failed to refresh explorer tree.',
				true
			);
		} );
	} );
	...
	$( '#cc-view-apply' ).on( 'click', function () {
		...
		applyViewState( true );
		const searchState = state.view.search
			? `, Search: "${
					state.view.search
			  }" (${ getVisibleSearchMatchCount() } matches)`
			: ', Search: none';
		setStatus(
			`View applied. Layout: ${ state.view.layout }, Type: ${
				state.view.nodeType
			}, CPT: ${ state.view.cpt }, Depth: ${
				state.view.collapseLevel || 'all'
			}${ searchState }.`
		);
	} );
		...
		searchInputTimer = setTimeout( () => {
			...
			if ( state.view.search ) {
				setStatus(
					`Search "${
						state.view.search
					}" matched ${ getVisibleSearchMatchCount() } node(s).`
				);
			} else {
	...
	$rerunAiBranchBtn.on( 'click', function () {
		...
		rerunAiForSelectedBranch().catch( ( error ) => {
			setStatus(
				error.message || 'Branch AI rerun request failed.',
				true
			);
			$rerunAiBranchBtn.prop( 'disabled', false );
		} );
	...
	$contentMode.on( 'change', function () {
		...
		Promise.all( [
			...
			.then( ( [ preview, audit ] ) => {
				...
				updateAuditTrail( audit );
				setStatus(
					`Loaded ${ $contentMode.val() || 'raw' } preview for ${
						state.selectedPath
					}.`
				);
			} )
			.catch( ( error ) => {
				setStatus(
					error.message || 'Failed to load preview mode.',
					true
				);
			} );
	} );
	...
	$contentContextScrubPreviewBtn.on( 'click', function () {
		...
		dbvc_cc_load_scrub_policy_preview( state.selectedPath ).catch(
			( error ) => {
				dbvc_cc_set_content_context_status(
					error.message || 'Failed to load scrub preview.',
					true
				);
				$contentContextJson.text( '' );
			}
	...
	$contentContextScrubApproveBtn.on( 'click', function () {
		...
		dbvc_cc_approve_scrub_policy_suggestions( state.selectedPath ).catch(
			( error ) => {
				dbvc_cc_set_content_context_status(
					error.message || 'Failed to approve scrub suggestions.',
					true
				);
				$contentContextJson.text( '' );
			}
	...
	$contentContextScrubStatusBtn.on( 'click', function () {
		dbvc_cc_load_scrub_policy_approval_status(
			state.selectedPath || ''
		).catch( ( error ) => {
			dbvc_cc_set_content_context_status(
				error.message || 'Failed to load scrub approval status.',
				true
			);
			$contentContextJson.text( '' );
		} );
	...
	loadDomains()
		...
		.catch( ( error ) => {
			setStatus(
				error.message || 'Failed to initialize explorer.',
				true
			);
		} );
} );
