jQuery( document ).ready( function ( $ ) {
	...
	function getCollectOverrides() {
		return {
			...
			request_timeout:
				parseInt( $( '#request_timeout' ).val(), 10 ) || 30,
			...
			capture_include_attribute_context: $(
				...
				? 1
				: 0,
			capture_include_dom_path: $( '#capture_include_dom_path' ).is(
				':checked'
			)
				? 1
				: 0,
			capture_max_elements_per_page:
				parseInt( $( '#capture_max_elements_per_page' ).val(), 10 ) ||
				2000,
			capture_max_chars_per_element:
				parseInt( $( '#capture_max_chars_per_element' ).val(), 10 ) ||
				1000,
			context_enable_boilerplate_detection: $(
				...
				? 1
				: 0,
			context_enable_entity_hints: $( '#context_enable_entity_hints' ).is(
				':checked'
			)
				? 1
				: 0,
			ai_enable_section_typing: $( '#ai_enable_section_typing' ).is(
				':checked'
			)
				? 1
				: 0,
			ai_section_typing_confidence_threshold:
				parseFloat(
					$( '#ai_section_typing_confidence_threshold' ).val()
				) || 0.65,
			scrub_policy_enabled: $( '#scrub_policy_enabled' ).is( ':checked' )
				? 1
				: 0,
			scrub_profile_mode: String(
				$( '#scrub_profile_mode' ).val() || 'deterministic-default'
			),
			scrub_attr_action_class: String(
				$( '#scrub_attr_action_class' ).val() || 'tokenize'
			),
			scrub_attr_action_id: String(
				$( '#scrub_attr_action_id' ).val() || 'hash'
			),
			scrub_attr_action_data: String(
				$( '#scrub_attr_action_data' ).val() || 'tokenize'
			),
			scrub_attr_action_style: String(
				$( '#scrub_attr_action_style' ).val() || 'drop'
			),
			scrub_attr_action_aria: String(
				$( '#scrub_attr_action_aria' ).val() || 'keep'
			),
			scrub_custom_allowlist: String(
				$( '#scrub_custom_allowlist' ).val() || ''
			),
			scrub_custom_denylist: String(
				$( '#scrub_custom_denylist' ).val() || ''
			),
			scrub_ai_suggestion_enabled: $( '#scrub_ai_suggestion_enabled' ).is(
				':checked'
			)
				? 1
				: 0,
			scrub_preview_sample_size:
				parseInt( $( '#scrub_preview_sample_size' ).val(), 10 ) || 20,
		};
	}
		...
		$.post( ajax_url, {
			...
			.done( function ( response ) {
				if ( response.success ) {
					...
					dbvc_cc_active_crawl_domain = dbvc_cc_resolve_domain_key(
						response.data.domain,
						urlsToProcess,
						sitemapUrl
					);
					logMessage(
						`SUCCESS: Found ${ totalUrls } URLs to process.`,
						'success'
					);
					if ( totalUrls > 0 ) {
						processNextUrl();
						...
						logMessage(
							'No URLs found. Process finished.',
							'info'
						);
						finishProcess();
					}
			...
			.fail( function () {
				logMessage(
					'ERROR: An unknown AJAX error occurred while fetching the sitemap.',
					'error'
				);
				finishProcess();
			} );
	...
	$stopBtn.on( 'click', function () {
		...
		logMessage(
			'STOPPING: Crawl will halt after the current page finishes processing.',
			'info'
		);
	...
	$downloadLogBtn.on( 'click', function () {
		...
		link.download = `dbvc-content-migration-log-${ new Date()
			.toISOString()
			.slice( 0, 10 ) }.txt`;
		link.click();
	...
	function processNextUrl() {
		...
		logMessage(
			`[${
				processedCount + failedCount + 1
			}/${ totalUrls }] Processing: ${ url }`
		);
		...
		$.post(
			...
			.fail( function () {
				failedCount++;
				logMessage(
					`  -> ERROR: Unknown AJAX error for ${ url }`,
					'error'
				);
			} )
			.always( function () {
				updateProgressBar();
				const delay =
					parseInt( activeCrawlOverrides.request_delay, 10 ) || 500;
	...
	function updateProgressBar() {
		...
		const percentage =
			totalUrls > 0 ? ( processedTotal / totalUrls ) * 100 : 0;
		$progressBar
	...
	function dbvc_cc_trigger_domain_ai_refresh_if_needed() {
		const dbvc_cc_refresh_action =
			actions && actions.trigger_domain_ai_refresh
				? String( actions.trigger_domain_ai_refresh )
				: '';
		if (
			dbvc_cc_domain_ai_refresh_requested ||
			! dbvc_cc_refresh_action ||
			! dbvc_cc_active_crawl_domain
		) {
			if ( ! dbvc_cc_active_crawl_domain ) {
				logMessage(
					'INFO: Domain AI refresh skipped because no crawl domain was detected.',
					'info'
				);
			}
			return;
		...
		logMessage(
			`Queueing full-domain AI refresh for ${ dbvc_cc_active_crawl_domain }...`,
			'info'
		);
		...
		$.post(
			...
			.done( function ( dbvc_cc_response ) {
				if ( ! dbvc_cc_response || ! dbvc_cc_response.success ) {
					const dbvc_cc_error_message =
						dbvc_cc_response &&
						dbvc_cc_response.data &&
						dbvc_cc_response.data.message
				...
				const dbvc_cc_batch_id = dbvc_cc_payload.batch_id
					? String( dbvc_cc_payload.batch_id )
					: '';
				const dbvc_cc_total_jobs = Number(
					dbvc_cc_payload.total_jobs || 0
				);
				const dbvc_cc_failed_jobs = Number(
					dbvc_cc_payload.failed_jobs || 0
				);
				...
				if ( dbvc_cc_batch_id ) {
					logMessage(
						`INFO: AI refresh batch ${ dbvc_cc_batch_id } queued (${ dbvc_cc_total_jobs } jobs, ${ dbvc_cc_failed_jobs } queue errors).`,
						'info'
					);
				}

				if (
					dbvc_cc_payload.warning_badge &&
					dbvc_cc_payload.warning_message
				) {
					logMessage(
						`WARNING: ${ String(
							dbvc_cc_payload.warning_message
			...
			.fail( function () {
				logMessage(
					'ERROR: Failed to queue domain AI refresh.',
					'error'
				);
			} );
	}
	...
	function logMessage( message, type = 'info' ) {
		...
		$log.append(
			`<div class="log-entry log-${ type }" style="color:${
				colorMap[ type ] || '#333'
			};">${ message }</div>`
		);
