...
final class DBVC_CC_Media_Decision_Service
{
    /**
     * @var DBVC_CC_Media_Decision_Service|null
     */
    private static $instance = null;

    /**
     * @return DBVC_CC_Media_Decision_Service
     */
    ...
    public function get_decision($domain, $path)
    {
        ...
        return [
            ...
            'path' => $context['path'],
            'decision_file' => $context['decision_file'],
            'media_decision' => $this->build_empty_record($context, ''),
        ];
    }
    ...
    public function save_decision($domain, $path, array $payload, $user_id)
    {
        ...
        $record['approved'] = $this->sanitize_rows(isset($payload['approved']) && is_array($payload['approved']) ? $payload['approved'] : []);
        $record['overrides'] = $this->sanitize_rows(isset($payload['overrides']) && is_array($payload['overrides']) ? $payload['overrides'] : []);
        $record['ignored'] = $this->sanitize_rows(isset($payload['ignored']) && is_array($payload['ignored']) ? $payload['ignored'] : []);
        $record['conflicts'] = $this->sanitize_rows(isset($payload['conflicts']) && is_array($payload['conflicts']) ? $payload['conflicts'] : []);
        ...
        if ($record['decision_status'] === 'pending' && ! empty($record['approved']) && empty($record['conflicts'])) {
            $record['decision_status'] = 'approved';
        }

        if (! DBVC_CC_Artifact_Manager::write_json_file($context['decision_file'], $record)) {
            return new WP_Error(
                'dbvc_cc_media_decision_write_failed',
                __('Could not persist media decision artifact.', 'dbvc'),
                ['status' => 500]
            );
        }
        ...
        DBVC_CC_Artifact_Manager::log_event(
            [
                'stage' => 'mapping_media_decision',
                'status' => 'saved',
                'page_url' => $context['source_url'],
                'path' => $context['path'],
                'message' => 'Media decision artifact saved.',
            ]
        );
    ...
    private function resolve_page_context($domain, $path)
    {
        ...
        if ($domain_key === '') {
            return new WP_Error(
                'dbvc_cc_media_decision_domain_invalid',
        ...
        if ($relative_path === '') {
            return new WP_Error(
                'dbvc_cc_media_decision_path_invalid',
        ...
        if (! is_string($base_dir) || $base_dir === '' || ! is_dir($base_dir)) {
            return new WP_Error(
                'dbvc_cc_media_decision_storage_missing',
        ...
        if (! is_string($base_real)) {
            return new WP_Error(
                'dbvc_cc_media_decision_storage_invalid',
        ...
        if (! is_dir($domain_dir)) {
            return new WP_Error(
                'dbvc_cc_media_decision_domain_missing',
        ...
        if (! is_dir($page_dir)) {
            return new WP_Error(
                'dbvc_cc_media_decision_path_missing',
        ...
        $decision_file = trailingslashit($page_dir) . $slug . DBVC_CC_Contracts::STORAGE_MEDIA_DECISIONS_V1_SUFFIX;
        if (! dbvc_cc_path_is_within($decision_file, $base_real)) {
            return new WP_Error(
                'dbvc_cc_media_decision_file_invalid',
                __('Media decision file path is invalid.', 'dbvc'),
                ['status' => 500]
            );
        }
