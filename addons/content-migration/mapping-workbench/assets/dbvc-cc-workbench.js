jQuery( function ( $ ) {
	...
	const dbvc_cc_post_type_options = Array.isArray( config.post_types )
		? config.post_types
		: [];

	const dbvc_cc_mapping_catalog_bridge_enabled =
		!! capabilities.mapping_catalog_bridge;
	const dbvc_cc_media_mapping_bridge_enabled =
		!! capabilities.media_mapping_bridge;
	const dbvc_cc_mapping_bridge_enabled =
		dbvc_cc_mapping_catalog_bridge_enabled;
	const dbvc_cc_import_executor_enabled =
		typeof capabilities.import_executor === 'boolean'
	...
	const $mappingMediaDecisionSummary = $(
		'#dbvc-cc-mapping-media-decision-summary'
	);
	const $mappingHandoffSummary = $( '#dbvc-cc-mapping-handoff-summary' );
	const $mappingDefaultEntitySummary = $(
		'#dbvc-cc-mapping-default-entity-summary'
	);
	const $mappingPhase4ContextSummary = $(
		'#dbvc-cc-mapping-phase4-context-summary'
	);
	const $mappingImportPlanSummary = $(
		'#dbvc-cc-mapping-import-plan-summary'
	);
	const $mappingImportExecutorSummary = $(
		'#dbvc-cc-mapping-import-executor-summary'
	);
	const $mappingImportApprovalSummary = $(
		'#dbvc-cc-mapping-import-approval-summary'
	);
	const $mappingImportExecuteSummary = $(
		'#dbvc-cc-mapping-import-execute-summary'
	);
	const $mappingImportRecoverySummary = $(
		'#dbvc-cc-mapping-import-recovery-summary'
	);
	const $mappingRunHistorySummary = $(
		'#dbvc-cc-mapping-run-history-summary'
	);
	const $mappingHandoffReviewSummary = $(
		'#dbvc-cc-mapping-handoff-review-summary'
	);
	const $mappingHandoffReviewList = $(
		'#dbvc-cc-mapping-handoff-review-list'
	);
	const $mappingSectionsTableBody = $(
		'#dbvc-cc-mapping-sections-table tbody'
	);
	const $mappingMediaTableBody = $( '#dbvc-cc-mapping-media-table tbody' );
	const $mappingRunHistoryTableBody = $(
		'#dbvc-cc-mapping-run-history-table tbody'
	);
	...
	const $mappingRunDetailSourceUrl = $(
		'#dbvc-cc-mapping-run-detail-source-url'
	);
	const $mappingRunDetailStatus = $( '#dbvc-cc-mapping-run-detail-status' );
	const $mappingRunDetailRollback = $(
		'#dbvc-cc-mapping-run-detail-rollback'
	);
	...
	const $mappingRunActionsTableBody = $(
		'#dbvc-cc-mapping-run-actions-table tbody'
	);
	const $mappingRunFilterStage = $( '#dbvc-cc-mapping-run-filter-stage' );
	const $mappingRunFilterExecution = $(
		'#dbvc-cc-mapping-run-filter-execution'
	);
	const $mappingRunFilterRollback = $(
		'#dbvc-cc-mapping-run-filter-rollback'
	);
	const $mappingRunFilterFailedOnly = $(
		'#dbvc-cc-mapping-run-filter-failed-only'
	);
	const $mappingRunActionDetailEmpty = $(
		'#dbvc-cc-mapping-run-action-detail-empty'
	);
	const $mappingRunActionDetailContent = $(
		'#dbvc-cc-mapping-run-action-detail-content'
	);
	const $mappingRunActionDetailSummary = $(
		'#dbvc-cc-mapping-run-action-detail-summary'
	);
	const $mappingRunActionBeforeState = $(
		'#dbvc-cc-mapping-run-action-before-state'
	);
	const $mappingRunActionAfterState = $(
		'#dbvc-cc-mapping-run-action-after-state'
	);
	...
	const $mappingImportExecutorJson = $(
		'#dbvc-cc-mapping-import-executor-json'
	);
	const $mappingImportApprovalJson = $(
		'#dbvc-cc-mapping-import-approval-json'
	);
	const $mappingImportExecuteJson = $(
		'#dbvc-cc-mapping-import-execute-json'
	);
	const $mappingImportRecoveryJson = $(
		'#dbvc-cc-mapping-import-recovery-json'
	);
	...
	const $mappingMediaDecisionStatus = $(
		'#dbvc-cc-mapping-media-decision-status'
	);
	...
	function dbvc_cc_close_mapping_help_modal() {
		...
		if (
			dbvc_cc_last_focused_element &&
			typeof dbvc_cc_last_focused_element.focus === 'function'
		) {
	...
	function dbvc_cc_probe_mapping_interactivity() {
		...
		controlSelectors.forEach( ( selector ) => {
			...
			if (
				$topElement.closest( '#dbvc-cc-mapping-help-modal' ).length > 0
			) {
		...
		if ( overlayDetected ) {
			dbvc_cc_set_mapping_status(
				`Mapper controls are currently blocked by ${ overlayLabel }.`,
				true
			);
		}
	}
	...
	function dbvc_cc_get_catalog_post_type_label( slug ) {
		...
		const catalog =
			mappingState.catalog && typeof mappingState.catalog === 'object'
				? mappingState.catalog
				: null;
		const cptCatalog =
			catalog &&
			catalog.cpt_catalog &&
			typeof catalog.cpt_catalog === 'object'
				? catalog.cpt_catalog
				: {};
		const catalogEntry =
			cptCatalog[ normalizedSlug ] &&
			typeof cptCatalog[ normalizedSlug ] === 'object'
				? cptCatalog[ normalizedSlug ]
				: null;
	...
	function dbvc_cc_build_fallback_target_ref_meta( targetRef ) {
		...
		if ( source === 'acf' ) {
			...
			return {
				...
				source: 'acf',
				displayLabel: `ACF: ${ groupTitle || groupKey || 'Group' } -> ${
					fieldLabel || fieldKey || 'Field'
				}`,
				detailLabel: `key: ${ fieldKey || 'n/a' } | group: ${
					groupKey || 'n/a'
				} | ref: ${ normalizedTargetRef }`,
				groupKey,
				groupTitle,
				fieldKey,
				fieldName: '',
		...
		if ( source === 'meta' ) {
			...
			const subtypeLabel = dbvc_cc_get_catalog_post_type_label(
				subtype || objectType
			);
			return {
				targetRef: normalizedTargetRef,
				...
				displayLabel: `Meta: ${
					subtypeLabel ||
					dbvc_cc_humanize_identifier( subtype || objectType ) ||
					'Field'
				} -> ${
					dbvc_cc_humanize_identifier( fieldKey ) ||
					fieldKey ||
					'Field'
				}`,
				detailLabel: `meta key: ${
					fieldKey || 'n/a'
				} | ref: ${ normalizedTargetRef }`,
				groupKey: '',
				groupTitle: '',
	...
	function dbvc_cc_build_target_ref_index() {
		...
		const metaCatalog =
			...
			typeof catalog.meta_catalog === 'object'
				? catalog.meta_catalog
				: {};
		Object.keys( metaCatalog ).forEach( ( objectType ) => {
			...
			Object.keys( subtypes ).forEach( ( subtype ) => {
				...
				Object.keys( entries ).forEach( ( fieldKey ) => {
					const entry =
						entries[ fieldKey ] &&
						typeof entries[ fieldKey ] === 'object'
							? entries[ fieldKey ]
							: {};
					const targetRef = `meta:${ objectType }:${ subtype }:${ fieldKey }`;
					const fieldLabel =
						dbvc_cc_humanize_identifier( fieldKey ) || fieldKey;
		...
		const acfGroups =
			...
			typeof catalog.acf_catalog.groups === 'object'
				? catalog.acf_catalog.groups
				: {};
		Object.keys( acfGroups ).forEach( ( groupKey ) => {
			...
			const groupTitle =
				String( groupEntry.title || '' ).trim() ||
				dbvc_cc_humanize_identifier( groupKey ) ||
				groupKey;
			const fields =
				groupEntry.fields && typeof groupEntry.fields === 'object'
					? groupEntry.fields
					: {};
	...
	function dbvc_cc_get_target_ref_index() {
		const fingerprint =
			mappingState.catalog && mappingState.catalog.catalog_fingerprint
				? String( mappingState.catalog.catalog_fingerprint )
				: '';

		if (
			mappingState.targetRefIndex &&
	...
	function dbvc_cc_get_target_ref_meta( targetRef ) {
		...
		if (
			index[ normalizedTargetRef ] &&
			typeof index[ normalizedTargetRef ] === 'object'
		) {
	...
	function dbvc_cc_build_target_option_markup( targetRef, option = {} ) {
		...
		const optionLabel = String(
			option.label ||
				targetMeta.displayLabel ||
				normalizedTargetRef ||
				'Unknown target'
		);
		return `<option ${ dbvc_cc_build_html_attributes(
			attributes
		) }>${ dbvc_cc_escape_html( optionLabel ) }</option>`;
	}
	...
	function dbvc_cc_sync_target_select_titles( $container = null ) {
		...
		$scope
			.find( '.dbvc-cc-map-section-target, .dbvc-cc-map-media-target' )
			.each( function () {
				const $select = $( this );
				const $selectedOption = $select.find( 'option:selected' );
				const title = String(
					$selectedOption.attr( 'title' ) ||
						$selectedOption.data( 'target-label' ) ||
						$select.val() ||
						''
				).trim();
	...
	function dbvc_cc_build_state_pill_markup( value, options = {} ) {
		...
		return `<span class="dbvc-cc-state-pill ${ cssClass }">${ dbvc_cc_escape_html(
	...
	function dbvc_cc_get_run_summary_counts( run ) {
		...
		const entityCounts =
			summary &&
			typeof summary.entity_counts === 'object' &&
			summary.entity_counts
				? summary.entity_counts
				: {};
		const fieldCounts =
			summary &&
			typeof summary.field_counts === 'object' &&
			summary.field_counts
				? summary.field_counts
				: {};
		const mediaCounts =
			summary &&
			typeof summary.media_counts === 'object' &&
			summary.media_counts
				? summary.media_counts
				: {};
		const preparedCounts =
			summary &&
			typeof summary.prepared_counts === 'object' &&
			summary.prepared_counts
				? summary.prepared_counts
				: {};
		const executionFailures = Array.isArray( summary.execution_failures )
	...
	function dbvc_cc_normalize_for_diff( value ) {
		...
		if ( value && typeof value === 'object' ) {
			...
			Object.keys( value )
				.sort()
				.forEach( ( key ) => {
					normalized[ key ] = dbvc_cc_normalize_for_diff(
						value[ key ]
					);
				} );
			return normalized;
	...
	function dbvc_cc_collect_diff_keys( beforeValue, afterValue, prefix = '' ) {
		...
		const beforeIsObject =
			...
			! Array.isArray( beforeNormalized );
		const afterIsObject =
			afterNormalized &&
		...
		if ( beforeIsObject && afterIsObject ) {
			...
			keys.forEach( ( key ) => {
				...
				changes = changes.concat(
					dbvc_cc_collect_diff_keys(
						beforeNormalized[ key ],
						afterNormalized[ key ],
						nextPrefix
					)
				);
	...
	function dbvc_cc_get_filtered_run_actions( actions = [] ) {
		...
		return ( Array.isArray( actions ) ? actions : [] ).filter(
			( action ) => {
				...
				if (
					filters.execution &&
					filters.execution !== executionStatus
				) {
					return false;
				}
	...
	function dbvc_cc_get_selected_run_action( actions = [] ) {
		...
		const selectedActionId = Number(
			mappingState.importSelectedActionId || 0
		);
		if ( selectedActionId > 0 ) {
			const selectedAction = filteredActions.find(
				( action ) =>
					dbvc_cc_normalize_action_id( action ) === selectedActionId
			);
			if ( selectedAction ) {
	...
	function dbvc_cc_get_summary_issue_hint( payload, preferredKeys = [] ) {
		...
		for ( let index = 0; index < collectionKeys.length; index += 1 ) {
			...
			const firstScalar = collection.find(
				( item ) =>
			...
			const firstMatch = collection.find(
			...
			if (
				Array.isArray( firstMatch.review_reasons ) &&
				firstMatch.review_reasons.length > 0
			) {
				const reviewHint = dbvc_cc_get_summary_issue_hint(
					{ review_reasons: firstMatch.review_reasons },
					[ 'review_reasons' ]
				);
				if ( reviewHint ) {
					return reviewHint;
			...
			const code = String(
				firstMatch.code || firstMatch.deferred_reason_code || ''
			).trim();
	...
	function dbvc_cc_render_handoff_review() {
		const handoffPayload =
			...
			typeof mappingState.handoffPayload === 'object'
				? mappingState.handoffPayload
				: null;
		const review =
			handoffPayload &&
			handoffPayload.review &&
			typeof handoffPayload.review === 'object'
				? handoffPayload.review
				: {};
		...
		if ( ! handoffPayload ) {
			$mappingHandoffReviewSummary.text(
				'Preview Dry-Run Handoff to inspect review blockers for the selected domain/path.'
			);
			$mappingHandoffReviewList.append(
				'<li>No handoff review loaded yet.</li>'
			);
			return;
		}
		...
		if ( reasons.length === 0 ) {
			...
			$mappingHandoffReviewList.append(
				'<li>No handoff review blockers remain.</li>'
			);
			return;
		}

		const blockingCount = reasons.filter(
			( reason ) => reason && reason.blocking
		).length;
		$mappingHandoffReviewSummary.text(
			`${ reasons.length } review blocker(s) loaded for this domain/path. Blocking:${ blockingCount }.`
		);
		...
		reasons.forEach( ( reason ) => {
			...
			const context =
				reason && typeof reason.context === 'object' && reason.context
					? reason.context
					: {};
			const countText =
				context && Number( context.count || 0 ) > 0
					? ` (${ Number( context.count ) } item(s))`
					: '';
			const chipClass =
				reason && reason.blocking
					? 'dbvc-cc-map-chip-danger'
					: 'dbvc-cc-map-chip-muted';
			const chipLabel = reason && reason.blocking ? 'blocking' : 'note';
			const heading = code ? code.replace( /_/g, ' ' ) : 'review reason';

			$mappingHandoffReviewList.append( `
                <li>
                    <span class="dbvc-cc-map-chip ${ chipClass }">${ dbvc_cc_escape_html(
						chipLabel
					) }</span>
                    <strong>${ dbvc_cc_escape_html( heading ) }</strong>
                    <span>${ dbvc_cc_escape_html(
						message || 'Review this item before continuing.'
					) }${ dbvc_cc_escape_html( countText ) }</span>
                </li>
            ` );
		} );
	}
	...
	function dbvc_cc_render_import_run_history() {
		...
		const runHistory =
			...
			typeof mappingState.importRunHistory === 'object'
				? mappingState.importRunHistory
				: {};
		...
		if ( ! target.valid ) {
			$mappingRunHistoryTableBody.append(
				'<tr><td colspan="5">Select a domain/path to review import runs.</td></tr>'
			);
			return;
		}
		...
		if ( runHistory.load_error ) {
			$mappingRunHistoryTableBody.append(
				`<tr><td colspan="5">${ dbvc_cc_escape_html(
					String( runHistory.load_error )
				) }</td></tr>`
			);
			return;
		}
		...
		if ( runs.length === 0 ) {
			$mappingRunHistoryTableBody.append(
				'<tr><td colspan="5">No import runs were found for this domain/path yet.</td></tr>'
			);
			return;
		}
		...
		runs.forEach( ( run ) => {
			...
			const isActive =
				runId > 0 &&
				runId ===
					dbvc_cc_normalize_run_id(
						mappingState.importSelectedRunId
					);
			const rowClass = isActive ? ' class="is-active"' : '';
			const shortRunUuid = String(
				run && run.run_uuid ? run.run_uuid : ''
			).substring( 0, 18 );
			const createdLabel = dbvc_cc_format_datetime(
				run && run.created_at ? run.created_at : ''
			);
			const rollbackFinishedLabel = dbvc_cc_format_datetime(
				run && run.rollback_finished_at ? run.rollback_finished_at : ''
			);
			const writesSummary = dbvc_cc_build_run_counts_text( run );
			const failureCount =
				dbvc_cc_get_run_summary_counts( run ).executionFailureCount;
			const errorSummary = String(
				run && run.error_summary ? run.error_summary : ''
			).trim();

			$mappingRunHistoryTableBody.append( `
                <tr data-run-id="${ dbvc_cc_escape_html(
					runId
				) }"${ rowClass }>
                    <td>
                        <strong>#${ dbvc_cc_escape_html(
							runId || '0'
						) }</strong>
                        <div class="dbvc-cc-map-meta">${ dbvc_cc_escape_html(
							shortRunUuid || 'no uuid'
						) }</div>
                    </td>
                    <td>
                        ${ dbvc_cc_build_state_pill_markup(
							run && run.status ? run.status : 'unknown'
						) }
                        ${
							errorSummary
								? `<div class="dbvc-cc-map-meta">${ dbvc_cc_escape_html(
                    ...
                    <td>
                        ...
                        <div class="dbvc-cc-map-meta">approved: ${ dbvc_cc_escape_html(
							dbvc_cc_format_datetime(
								run && run.approved_at ? run.approved_at : ''
							)
						) }</div>
                    </td>
                    <td>
                        ${ dbvc_cc_build_state_pill_markup(
							run && run.rollback_status
								? run.rollback_status
								: 'unknown',
							{ type: 'rollback' }
						) }
                        <div class="dbvc-cc-map-meta">${ dbvc_cc_escape_html(
							rollbackFinishedLabel
						) }</div>
                    </td>
                    <td>
                        <div>${ dbvc_cc_escape_html( writesSummary ) }</div>
                        <div class="dbvc-cc-map-meta">failures: ${ dbvc_cc_escape_html(
							failureCount
						) }</div>
                    </td>
                </tr>
	...
	function dbvc_cc_render_run_banner( run ) {
		if ( ! run || typeof run !== 'object' ) {
			$mappingRunDetailBanner
				.addClass( 'dbvc-cc-hidden' )
		...
		if ( status === 'rolled_back_after_failure' ) {
			bannerText =
				'Automatic rollback completed after an execution failure. Review the selected actions below to confirm the restored state.';
		} else if ( status === 'rollback_failed_after_failure' ) {
			bannerText =
				'Automatic rollback failed after an execution failure. Review the failed actions below and use manual rollback if it is still available.';
		...
		$mappingRunDetailBanner
			.removeClass( 'dbvc-cc-hidden is-error is-success' )
	...
	function dbvc_cc_render_selected_run_action_detail( action ) {
		...
		const beforeState =
			action.before_state && typeof action.before_state === 'object'
				? action.before_state
				: action.before_state || null;
		const afterState =
			action.after_state && typeof action.after_state === 'object'
				? action.after_state
				: action.after_state || null;
		const changedKeys = dbvc_cc_collect_diff_keys(
			beforeState,
			afterState
		);
		const summaryParts = [
			`action #${ Number( action.action_order || 0 ) }`,
			`${ String( action.stage || 'unknown' ) } / ${ String(
		...
		if ( changedKeys.length > 0 ) {
			summaryParts.push(
				`changed ${ changedKeys.slice( 0, 8 ).join( ', ' ) }`
			);
		} else {
	...
	function dbvc_cc_render_import_run_detail() {
		const detail =
			...
			typeof mappingState.importRunDetail === 'object'
				? mappingState.importRunDetail
				: null;
		const run =
			detail && detail.run && typeof detail.run === 'object'
				? detail.run
				: null;
		const actions =
			detail && Array.isArray( detail.actions ) ? detail.actions : [];
		...
		if ( ! run || dbvc_cc_normalize_run_id( run.id ) <= 0 ) {
			...
			$mappingRunActionsTableBody.append(
				'<tr><td colspan="6">No journaled actions were found for this run.</td></tr>'
			);
		...
		if ( selectedAction ) {
			mappingState.importSelectedActionId =
				dbvc_cc_normalize_action_id( selectedAction );
		} else {
			mappingState.importSelectedActionId = 0;
		}
		...
		$mappingRunDetailId.text(
			`#${ dbvc_cc_normalize_run_id( run.id ) } | ${
				String( run.run_uuid || '' ).substring( 0, 24 ) || 'no uuid'
			}`
		);
		if ( sourceUrl ) {
			$mappingRunDetailSourceUrl
				.text( sourceUrl )
				.attr( 'href', sourceUrl );
		} else {
			$mappingRunDetailSourceUrl.text( 'n/a' ).attr( 'href', '#' );
		}
		$mappingRunDetailStatus.html(
			dbvc_cc_build_state_pill_markup( run.status || 'unknown' )
		);
		$mappingRunDetailRollback.html(
			dbvc_cc_build_state_pill_markup( run.rollback_status || 'unknown', {
				type: 'rollback',
			} )
		);
		...
		if ( filteredActions.length === 0 ) {
			$mappingRunActionsTableBody.append(
				'<tr><td colspan="6">No journaled actions match the current filters.</td></tr>'
			);
			dbvc_cc_render_selected_run_action_detail( null );
			return;
		}
		...
		filteredActions.forEach( ( action ) => {
			const actionOrder = Number(
				action && action.action_order ? action.action_order : 0
			);
			const actionId = dbvc_cc_normalize_action_id( action );
			const stageLabel = `${ String(
				action && action.stage ? action.stage : 'unknown'
			) } / ${ String(
				action && action.action_type ? action.action_type : 'unknown'
			) }`;
			const notes = [
				action && action.execution_error
					? `execute: ${ String( action.execution_error ) }`
					: '',
				action && action.rollback_error
					? `rollback: ${ String( action.rollback_error ) }`
					: '',
			]
				.filter( Boolean )
				.join( ' | ' );
			const isActive =
				actionId > 0 &&
				actionId === Number( mappingState.importSelectedActionId || 0 );

			$mappingRunActionsTableBody.append( `
                <tr data-action-id="${ dbvc_cc_escape_html( actionId ) }"${
					isActive ? ' class="is-active"' : ''
				}>
                    <td>${ dbvc_cc_escape_html( actionOrder || 0 ) }</td>
                    <td>${ dbvc_cc_escape_html( stageLabel ) }</td>
                    <td>${ dbvc_cc_escape_html(
						dbvc_cc_build_run_target_label( action )
					) }</td>
                    <td>${ dbvc_cc_build_state_pill_markup(
						action && action.execution_status
							? action.execution_status
							: 'unknown'
					) }</td>
                    <td>${ dbvc_cc_build_state_pill_markup(
						action && action.rollback_status
	...
	function dbvc_cc_ensure_object_post_type_option(
		postTypeValue,
		postTypeLabel = ''
	) {
		...
		if (
			! normalizedPostType ||
			dbvc_cc_has_select_option(
				$mappingObjectPostType,
				normalizedPostType
			)
		) {
			return;
		...
		$mappingObjectPostType.append(
			`<option value="${ dbvc_cc_escape_html(
				normalizedPostType
			) }">${ dbvc_cc_escape_html( label ) }</option>`
		);
	}
	...
	function dbvc_cc_populate_object_post_type_options() {
		...
		$mappingObjectPostType.append(
			'<option value="">Auto (Mapping + AI)</option>'
		);
		...
		normalizedOptions.forEach( ( option ) => {
			dbvc_cc_ensure_object_post_type_option(
				option.value,
				option.label
			);
		} );
	...
	function dbvc_cc_set_object_post_type_value(
		postTypeValue,
		postTypeLabel = ''
	) {
		const normalizedPostType = dbvc_cc_normalize_post_type( postTypeValue );
	...
	function dbvc_cc_append_domain_option( $select, domainKey, label = '' ) {
		...
		if (
			! normalizedDomain ||
			dbvc_cc_has_domain_option( $select, normalizedDomain )
		) {
			return;
		}
		...
		const optionLabel = String(
			label ||
				dbvc_cc_domain_labels_by_key[ normalizedDomain ] ||
				normalizedDomain
		);
		$select.append(
			`<option value="${ dbvc_cc_escape_html(
	...
	function dbvc_cc_domain_has_ai_warning( domainKey ) {
		...
		const dbvc_cc_health =
			dbvc_cc_domain_ai_health_by_key[ normalizedDomain ];
	...
	function dbvc_cc_build_domain_option_label( domainKey, label ) {
		...
		if (
			! normalizedDomain ||
			! dbvc_cc_domain_has_ai_warning( normalizedDomain )
		) {
	...
	function dbvc_cc_render_domain_ai_warning() {
		...
		if ( dbvc_cc_active_targets.length === 1 ) {
			...
			const dbvc_cc_warning_message =
				dbvc_cc_get_domain_ai_warning_message( dbvc_cc_domain_key );
			$domainAiWarning
				...
				.attr(
					'title',
					`${
						dbvc_cc_warning_message ||
						'AI pass errors detected. Newer content may be missing.'
					} Click to run a full-domain AI refresh.`
				);
			return;
		}
		...
		$domainAiWarning
			.removeClass( 'dbvc-cc-hidden' )
			.text(
				`AI Warning: Refresh ${ dbvc_cc_active_targets.length } Domains`
			)
			.prop( 'disabled', false )
			.attr(
				'title',
				`${ dbvc_cc_active_targets.length } domains have AI pass errors. Click to queue full-domain AI refresh for each warning domain.`
			);
	}
	...
	function dbvc_cc_get_warning_domains() {
		return Object.keys( dbvc_cc_domain_ai_health_by_key )
			.filter( ( domainKey ) =>
				dbvc_cc_domain_has_ai_warning( domainKey )
			)
	...
	function dbvc_cc_get_active_warning_domains() {
		...
		if ( normalizedDomain ) {
			return dbvc_cc_domain_has_ai_warning( normalizedDomain )
				? [ normalizedDomain ]
				: [];
		}
	...
	async function dbvc_cc_queue_domain_ai_refresh(
		dbvc_cc_domain_key,
		dbvc_cc_max_jobs = 0,
		dbvc_cc_offset = 0
	) {
	...
	async function dbvc_cc_queue_domain_ai_refresh_in_chunks(
		dbvc_cc_domain_key,
		dbvc_cc_chunk_size = 50
	) {
		const dbvc_cc_batches = [];
		const dbvc_cc_safe_chunk_size = Math.max(
			1,
			Math.min( 500, Number( dbvc_cc_chunk_size || 50 ) )
		);
		...
		while ( dbvc_cc_guard < 500 ) {
			dbvc_cc_guard += 1;
			const dbvc_cc_queue_response =
				await dbvc_cc_queue_domain_ai_refresh(
					dbvc_cc_domain_key,
					dbvc_cc_safe_chunk_size,
					dbvc_cc_offset
				);

			const dbvc_cc_batch_id = String(
				dbvc_cc_queue_response && dbvc_cc_queue_response.batch_id
					? dbvc_cc_queue_response.batch_id
					: ''
			);
			const dbvc_cc_total_jobs = Number(
				dbvc_cc_queue_response && dbvc_cc_queue_response.total_jobs
					? dbvc_cc_queue_response.total_jobs
					: 0
			);
			...
			const dbvc_cc_total_candidate_jobs = Number(
				dbvc_cc_queue_response &&
					dbvc_cc_queue_response.total_candidate_jobs
					? dbvc_cc_queue_response.total_candidate_jobs
					: dbvc_cc_next_offset
			);
			const dbvc_cc_was_truncated = !! (
				dbvc_cc_queue_response && dbvc_cc_queue_response.was_truncated
			);

			if ( dbvc_cc_batch_id !== '' ) {
				const dbvc_cc_chunk_start = dbvc_cc_offset + 1;
				const dbvc_cc_chunk_end = Math.max(
					dbvc_cc_offset,
					dbvc_cc_next_offset
				);
			...
			if (
				! dbvc_cc_was_truncated ||
				dbvc_cc_total_jobs <= 0 ||
				dbvc_cc_next_offset <= dbvc_cc_offset ||
				dbvc_cc_next_offset >= dbvc_cc_total_candidate_jobs
			) {
				break;
			}
		...
		if ( dbvc_cc_guard >= 500 ) {
			throw new Error(
				`AI refresh chunking exceeded guard limit for ${ dbvc_cc_domain_key }.`
			);
		}
	...
	async function dbvc_cc_wait_for_ai_batch_completion(
		dbvc_cc_batch_id,
		dbvc_cc_options = {}
	) {
		const dbvc_cc_poll_delay_ms = Number(
			dbvc_cc_options.pollDelayMs || 1500
		);
		const dbvc_cc_max_polls = Number( dbvc_cc_options.maxPolls || 240 );
		const dbvc_cc_batch_label = String(
			dbvc_cc_options.batchLabel || dbvc_cc_batch_id
		);

		let dbvc_cc_polls = 0;
		while ( dbvc_cc_polls < dbvc_cc_max_polls ) {
			dbvc_cc_polls += 1;
			const dbvc_cc_status_payload =
				await dbvc_cc_fetch_ai_batch_status( dbvc_cc_batch_id );
			const dbvc_cc_status = String(
				dbvc_cc_status_payload && dbvc_cc_status_payload.status
					? dbvc_cc_status_payload.status
					: 'queued'
			);
			const dbvc_cc_processed_jobs = Number(
				dbvc_cc_status_payload && dbvc_cc_status_payload.processed_jobs
					? dbvc_cc_status_payload.processed_jobs
					: 0
			);
			const dbvc_cc_total_jobs = Number(
				dbvc_cc_status_payload && dbvc_cc_status_payload.total_jobs
					? dbvc_cc_status_payload.total_jobs
					: 0
			);

			if (
				dbvc_cc_status === 'completed' ||
				dbvc_cc_status === 'completed_with_failures'
			) {
				return dbvc_cc_status_payload;
			}

			setStatus(
				`AI processing ${ dbvc_cc_batch_label }: ${ dbvc_cc_processed_jobs }/${ dbvc_cc_total_jobs } item(s)...`
			);
			dbvc_cc_set_ai_refresh_note(
				`AI chunk progress: ${ dbvc_cc_batch_label } (${ dbvc_cc_processed_jobs }/${ dbvc_cc_total_jobs })`
			);
			await new Promise( ( resolve ) =>
				window.setTimeout( resolve, dbvc_cc_poll_delay_ms )
			);
		}

		const dbvc_cc_timeout_error = new Error(
			`AI refresh polling timed out for ${ dbvc_cc_batch_label }.`
		);
		dbvc_cc_timeout_error.code = 'dbvc_cc_ai_poll_timeout';
		throw dbvc_cc_timeout_error;
	}

	async function dbvc_cc_wait_for_ai_batch_completion_with_retry(
		dbvc_cc_batch_id,
		dbvc_cc_options = {}
	) {
		const dbvc_cc_retry_limit = Math.max(
			0,
			Number( dbvc_cc_options.retryLimit || 1 )
		);
		const dbvc_cc_batch_label = String(
			dbvc_cc_options.batchLabel || dbvc_cc_batch_id
		);
		...
		while ( dbvc_cc_attempt <= dbvc_cc_retry_limit ) {
			try {
				return await dbvc_cc_wait_for_ai_batch_completion(
					dbvc_cc_batch_id,
					dbvc_cc_options
				);
			} catch ( dbvc_cc_error ) {
				dbvc_cc_last_error = dbvc_cc_error;
				const dbvc_cc_error_code = String(
					dbvc_cc_error && dbvc_cc_error.code
						? dbvc_cc_error.code
						: ''
				);
				const dbvc_cc_is_timeout =
					dbvc_cc_error_code === 'dbvc_cc_ai_poll_timeout';
				if (
					! dbvc_cc_is_timeout ||
					dbvc_cc_attempt >= dbvc_cc_retry_limit
				) {
					throw dbvc_cc_error;
				}
				...
				dbvc_cc_attempt += 1;
				setStatus(
					`AI polling timed out for ${ dbvc_cc_batch_label }. Retrying (${ dbvc_cc_attempt }/${ dbvc_cc_retry_limit })...`,
					true
				);
				dbvc_cc_set_ai_refresh_note(
					`Retrying timed-out AI chunk: ${ dbvc_cc_batch_label } (${ dbvc_cc_attempt }/${ dbvc_cc_retry_limit })`,
					true
				);
			}
		}
		...
		throw (
			dbvc_cc_last_error ||
			new Error(
				`AI refresh polling failed for ${ dbvc_cc_batch_label }.`
			)
		);
	}
	...
	async function dbvc_cc_run_warning_domain_refreshes() {
		...
		const dbvc_cc_previous_workbench_domain = dbvc_cc_normalize_domain(
			$domain.val()
		);
		const dbvc_cc_previous_mapping_domain = dbvc_cc_normalize_domain(
			$mappingDomain.val()
		);

		$domainAiWarning.prop( 'disabled', true ).text( 'Refreshing AI...' );
		setStatus(
			`Queueing full-domain AI refresh for ${ dbvc_cc_targets.length } domain(s) in chunks of ${ dbvc_cc_chunk_size }...`
		);
		dbvc_cc_set_ai_refresh_note(
			`Preparing AI chunk queue (${ dbvc_cc_chunk_size } items per batch)...`
		);
		...
		try {
			for ( const dbvc_cc_domain_key of dbvc_cc_targets ) {
				try {
					const dbvc_cc_domain_batches =
						await dbvc_cc_queue_domain_ai_refresh_in_chunks(
							dbvc_cc_domain_key,
							dbvc_cc_chunk_size
						);
					if ( dbvc_cc_domain_batches.length > 0 ) {
						dbvc_cc_queued_batches.push(
							...dbvc_cc_domain_batches
						);
						dbvc_cc_set_ai_refresh_note(
							`Queued ${ dbvc_cc_domain_batches.length } chunk batch(es) for ${ dbvc_cc_domain_key }.`
						);
			...
			if ( dbvc_cc_queued_batches.length > 0 ) {
				...
				for ( const dbvc_cc_batch of dbvc_cc_queued_batches ) {
					const dbvc_cc_batch_domain = String(
						dbvc_cc_batch && dbvc_cc_batch.domain
							? dbvc_cc_batch.domain
							: 'domain'
					);
					const dbvc_cc_chunk_start = Number(
						dbvc_cc_batch && dbvc_cc_batch.chunk_start
							? dbvc_cc_batch.chunk_start
							: 0
					);
					const dbvc_cc_chunk_end = Number(
						dbvc_cc_batch && dbvc_cc_batch.chunk_end
							? dbvc_cc_batch.chunk_end
							: 0
					);
					const dbvc_cc_batch_label = `${ dbvc_cc_batch_domain } [${ dbvc_cc_chunk_start }-${ dbvc_cc_chunk_end }]`;
					try {
						const dbvc_cc_batch_status =
							await dbvc_cc_wait_for_ai_batch_completion_with_retry(
						...
						dbvc_cc_set_ai_refresh_note(
							`AI chunks completed: ${ dbvc_cc_completed_batches }/${ dbvc_cc_total_batches } (${ dbvc_cc_batch_label })`
						);
						if (
							String(
								dbvc_cc_batch_status &&
									dbvc_cc_batch_status.status
									? dbvc_cc_batch_status.status
									: ''
						...
						dbvc_cc_completed_batches += 1;
						dbvc_cc_set_ai_refresh_note(
							`AI chunk failed: ${ dbvc_cc_batch_label } (${ dbvc_cc_completed_batches }/${ dbvc_cc_total_batches })`,
							true
						);
						dbvc_cc_failure_count++;
					}
			...
			if ( dbvc_cc_previous_workbench_domain ) {
				dbvc_cc_set_domain_value(
					$domain,
					dbvc_cc_previous_workbench_domain
				);
			}
			if ( dbvc_cc_previous_mapping_domain ) {
				dbvc_cc_set_domain_value(
					$mappingDomain,
					dbvc_cc_previous_mapping_domain
				);
			}
			dbvc_cc_render_domain_ai_warning();
			...
			if ( dbvc_cc_failure_count > 0 ) {
				setStatus(
					`AI refresh completed with issues for ${ dbvc_cc_success_count }/${ dbvc_cc_targets.length } domain(s). Queue auto-refreshed.`,
					true
				);
				dbvc_cc_set_ai_refresh_note(
					'AI refresh completed with some failed chunks. Queue was auto-refreshed.',
					true
				);
				return;
			}
			...
			setStatus(
				`AI refresh completed for ${ dbvc_cc_success_count } domain(s). Queue auto-refreshed.`
			);
			dbvc_cc_set_ai_refresh_note(
				'AI refresh completed successfully. Queue was auto-refreshed.'
			);
		} finally {
	...
	function dbvc_cc_populate_domain_selects( domains ) {
		...
		domains.forEach( ( domainRecord ) => {
			const normalizedDomain = dbvc_cc_normalize_domain(
				domainRecord && domainRecord.key ? domainRecord.key : ''
			);
			if ( ! normalizedDomain ) {
				return;
			}
			...
			const label = String(
				domainRecord && domainRecord.label
					? domainRecord.label
					: normalizedDomain
			);
			const dbvc_cc_health =
			...
			const dbvc_cc_option_label = dbvc_cc_build_domain_option_label(
				normalizedDomain,
				label
			);
			dbvc_cc_append_domain_option(
				$domain,
				normalizedDomain,
				dbvc_cc_option_label
			);
			dbvc_cc_append_domain_option(
				$mappingDomain,
				normalizedDomain,
	...
	async function dbvc_cc_load_available_domains() {
		try {
			...
			const domains = Array.isArray( payload && payload.domains )
				? payload.domains
				: [];
	...
	if ( prefill.path ) {
		$mappingPath.val(
			dbvc_cc_normalize_mapping_path( String( prefill.path ) )
		);
	}
	...
	function dbvc_cc_get_mapping_target() {
		...
		const path = dbvc_cc_normalize_mapping_path(
			String( $mappingPath.val() || '' )
		);
	...
	function dbvc_cc_update_execute_controls() {
		...
		const hasCompletedDryRun = !! (
			...
			String( mappingState.importExecutorDryRun.status || '' ) ===
				'completed'
		);
		...
		const hasRollbackableRun = !! (
			...
			String(
				mappingState.importExecuteSkeleton.rollback_status || ''
			) === 'available'
		);
		const hasRollbackableSelectedRun = !! (
			mappingState.importRunDetail &&
			mappingState.importRunDetail.run &&
			dbvc_cc_normalize_run_id( mappingState.importRunDetail.run.id ) >
				0 &&
			String( mappingState.importRunDetail.run.rollback_status || '' ) ===
				'available'
		);

		$( '#dbvc-cc-mapping-approve-import' ).prop(
			'disabled',
			! dbvc_cc_import_executor_enabled ||
	...
	function dbvc_cc_update_mapping_availability() {
		const baseSelector =
			'#dbvc-cc-mapping-domain, #dbvc-cc-mapping-path, #dbvc-cc-mapping-object-post-type, #dbvc-cc-mapping-load-package, #dbvc-cc-mapping-build-catalog, #dbvc-cc-mapping-refresh-catalog, #dbvc-cc-mapping-rebuild-domain, #dbvc-cc-mapping-load-decisions, #dbvc-cc-mapping-load-handoff, #dbvc-cc-mapping-generate-dry-run-plan, #dbvc-cc-mapping-save-decision, #dbvc-cc-mapping-save-media-decision, #dbvc-cc-mapping-refresh-run-history';
		const executorSelector =
			'#dbvc-cc-mapping-run-executor-dry-run, #dbvc-cc-mapping-approve-import, #dbvc-cc-mapping-run-execute-skeleton, #dbvc-cc-mapping-rollback-run, #dbvc-cc-mapping-rollback-selected-run, #dbvc-cc-mapping-download-run-report';
		const mediaSelector =
			'#dbvc-cc-mapping-save-media-decision, .dbvc-cc-map-media-target, .dbvc-cc-map-media-override, .dbvc-cc-map-media-ignore';
	...
	if ( ! dbvc_cc_import_executor_enabled ) {
		$(
			'#dbvc-cc-mapping-run-executor-dry-run, #dbvc-cc-mapping-approve-import, #dbvc-cc-mapping-run-execute-skeleton, #dbvc-cc-mapping-rollback-run, #dbvc-cc-mapping-download-run-report'
		).prop( 'disabled', true );
	}
	...
	function dbvc_cc_get_queue_item_mapping_health( item ) {
		return item &&
			item.mapping_health &&
			typeof item.mapping_health === 'object'
			? item.mapping_health
			: null;
	}

	function dbvc_cc_render_queue_state_cell( item, index ) {
		const dbvc_cc_mapping_health =
			dbvc_cc_get_queue_item_mapping_health( item );
		if ( ! dbvc_cc_mapping_health ) {
			return '<span class="dbvc-cc-state-pill dbvc-cc-state-pill-ok">Ready</span>';
		}

		const dbvc_cc_reasons = Array.isArray( dbvc_cc_mapping_health.reasons )
			? dbvc_cc_mapping_health.reasons
			: [];
		const dbvc_cc_title =
			dbvc_cc_reasons.length > 0
				? escapeHtml( dbvc_cc_reasons.join( ', ' ) )
				: '';
	...
	async function dbvc_cc_rebuild_queue_item_mapping( item ) {
		if ( ! item || ! item.domain || ! item.path ) {
			setStatus(
				'Select a valid queue item to rebuild mapping artifacts.',
				true
			);
			return;
		}
		...
		setStatus(
			`Rebuilding mapping artifacts for ${ item.domain }/${ item.path }...`
		);
		...
		try {
			...
			await loadQueue();
			setStatus(
				`Rebuilt mapping artifacts for ${ item.domain }/${ item.path }.`
			);
		} catch ( error ) {
			setStatus(
				error.message ||
					'Failed to rebuild mapping artifacts for queue item.',
				true
			);
		}
	}
	...
	function renderQueue() {
		...
		if ( queueItems.length === 0 ) {
			$tbody.append(
				'<tr><td colspan="7">No review items found.</td></tr>'
			);
			return;
		}

		queueItems.forEach( ( item, index ) => {
			const reasons = Array.isArray( item.review && item.review.reasons )
				? item.review.reasons.join( ', ' )
				: '';
			const decision =
				item.decision && item.decision.status
					? item.decision.status
					: 'pending';
			const confidence =
				item.suggestion_summary &&
				typeof item.suggestion_summary.post_type_confidence === 'number'
					? item.suggestion_summary.post_type_confidence.toFixed( 2 )
					: '0.00';

			const row = `
                <tr data-index="${ index }" class="${
					selected && selected.index === index ? 'is-active' : ''
				}">
                    <td>${ escapeHtml( item.domain || '' ) }</td>
                    <td>${ escapeHtml( item.path || '' ) }</td>
                    <td>${ escapeHtml(
						( item.suggestion_summary &&
							item.suggestion_summary.post_type ) ||
							''
					) }</td>
	...
	async function loadQueue() {
		...
		params.set(
			'include_decided',
			$includeDecided.is( ':checked' ) ? '1' : '0'
		);
		params.set( 'min_confidence', String( $minConfidence.val() || 0.75 ) );

		try {
			const payload = await apiRequest(
				`workbench/review-queue?${ params.toString() }`
			);
	...
	async function loadDetail( item, index ) {
		...
		try {
			const payload = await apiRequest(
				`workbench/suggestions?${ params.toString() }`
			);
			const suggestionPayload =
				payload && payload.suggestions ? payload.suggestions : {};
			const decision =
			...
			const sourceUrl =
				suggestionPayload && suggestionPayload.source_url
					? String( suggestionPayload.source_url )
					: '';
			if ( sourceUrl ) {
			...
			setStatus(
				error.message || 'Failed to load suggestion detail.',
				true
			);
		}
	}
	...
	function dbvc_cc_render_sections_table() {
		...
		const rows =
			mappingState.sectionCandidates &&
			Array.isArray( mappingState.sectionCandidates.sections )
				? mappingState.sectionCandidates.sections
				: [];

		if ( rows.length === 0 ) {
			$mappingSectionsTableBody.append(
				'<tr><td colspan="5">No section candidates were found for this node.</td></tr>'
			);
			return;
		}
		...
		rows.forEach( ( section ) => {
			...
			const candidates = Array.isArray( section.deterministic_candidates )
				? section.deterministic_candidates
				: [];
			const unresolvedCount = Array.isArray( section.unresolved_fields )
				? section.unresolved_fields.length
				: 0;
			const collectedPreview = String(
				section.collected_value_preview || ''
			).trim();
			const sectionDisplayLabel = sectionLabel || sectionId || 'section';
			const sectionIdMeta =
				sectionId && sectionDisplayLabel !== sectionId
					? `<div class="dbvc-cc-map-meta">id: ${ dbvc_cc_escape_html(
							sectionId
					  ) }</div>`
					: '';
			const collectedMeta = collectedPreview
				? `<div class="dbvc-cc-map-meta dbvc-cc-map-meta-sample">collected: ${ dbvc_cc_escape_html(
						collectedPreview
				  ) }</div>`
				: '';
			...
			candidates.forEach( ( candidate ) => {
				...
				const confidence = dbvc_cc_format_confidence(
					candidate.confidence || 0
				);
				const reason = String( candidate.reason || 'deterministic' );
				const targetMeta = dbvc_cc_get_target_ref_meta( targetRef );
				const label = `${
					targetMeta.displayLabel || targetRef
				} (${ confidence })`;
				options.push(
					dbvc_cc_build_target_option_markup( targetRef, {
						label,
						attributes: {
			...
			const rowHtml = `
                <tr data-section-id="${ dbvc_cc_escape_html( sectionId ) }">
                    <td>
                        ...
                        ${ sectionIdMeta }
                        ${ collectedMeta }
                        <div class="dbvc-cc-map-meta">${
							candidates.length
						} candidate(s), ${ unresolvedCount } unresolved</div>
                    </td>
	...
	function dbvc_cc_build_media_target_options( mediaItem ) {
		...
		const roleCandidates = Array.isArray( mediaItem.role_candidates )
			? mediaItem.role_candidates
			: [];
	...
	function dbvc_cc_render_media_preview( mediaItem ) {
		...
		if ( kind === 'image' && /^https?:\/\//i.test( previewRef ) ) {
			return `<img src="${ dbvc_cc_escape_html(
				previewRef
			) }" alt="preview" class="dbvc-cc-map-preview-image" loading="lazy" />`;
		}

		const fallbackLabel = previewStatus
			? `${ kind.toUpperCase() } (${ previewStatus })`
			: kind.toUpperCase();
		return `<span class="dbvc-cc-map-preview-fallback">${ dbvc_cc_escape_html(
			fallbackLabel
		) }</span>`;
	}
	...
	function dbvc_cc_get_media_write_state_index() {
		...
		const writePreparation =
			...
			mappingState.importExecuteSkeleton.write_preparation
				...
				: null;
		const mediaWrites =
			writePreparation && Array.isArray( writePreparation.media_writes )
	...
	function dbvc_cc_render_media_write_chip( mediaWrite ) {
		...
		const deferredGroup = String(
			mediaWrite.deferred_reason_group || ''
		).trim();
		...
		return `<span class="dbvc-cc-map-chip dbvc-cc-map-chip-muted">${ dbvc_cc_escape_html(
			writeStatus || 'pending-dry-run'
		) }</span>`;
	}
	...
	function dbvc_cc_render_media_table() {
		...
		if ( ! dbvc_cc_media_mapping_bridge_enabled ) {
			$mappingMediaTableBody.append(
				'<tr><td colspan="5">Media mapping bridge is disabled for this site. Text-only mapping remains available.</td></tr>'
			);
			return;
		}

		const items =
			mappingState.mediaCandidates &&
			Array.isArray( mappingState.mediaCandidates.media_items )
				? mappingState.mediaCandidates.media_items
				: [];
		if ( items.length === 0 ) {
			$mappingMediaTableBody.append(
				'<tr><td colspan="5">No media candidates were found for this node.</td></tr>'
			);
			return;
		}
		...
		items.forEach( ( mediaItem ) => {
			...
			const roleCandidates = Array.isArray( mediaItem.role_candidates )
				? mediaItem.role_candidates
				: [];
			const roleLabel = roleCandidates.length
				? roleCandidates.join( ', ' )
				: 'none';
			const previewStatus = String( mediaItem.preview_status || '' );
			const policyTrace = mediaItem.policy_trace || {};
			const domainPolicy = String(
				policyTrace.dbvc_cc_domain_policy || ''
			);
			const mimeAllowed =
				policyTrace.dbvc_cc_mime_allowed === false
					? 'blocked-mime'
					: 'allowed-mime';

			const rowHtml = `
                <tr data-media-id="${ dbvc_cc_escape_html(
					mediaId
				) }" data-media-kind="${ dbvc_cc_escape_html(
					mediaKind
				) }" data-source-url="${ dbvc_cc_escape_html( sourceUrl ) }">
                    ...
                    <td>
                        ...
                        <div class="dbvc-cc-map-meta">${ dbvc_cc_escape_html(
							mediaKind
						) } | roles: ${ dbvc_cc_escape_html( roleLabel ) }</div>
                        <div class="dbvc-cc-map-meta">${ dbvc_cc_escape_html(
							previewStatus || 'preview_status_unknown'
						) } | ${ dbvc_cc_escape_html(
							domainPolicy || 'domain_policy_unknown'
						) } | ${ dbvc_cc_escape_html( mimeAllowed ) }</div>
                        <div class="dbvc-cc-map-meta dbvc-cc-map-media-write-message"></div>
                        ${
							sourceUrl
								? `<a href="${ dbvc_cc_escape_html(
										sourceUrl
	...
	function dbvc_cc_update_media_write_annotations() {
		...
		$mappingMediaTableBody.find( 'tr[data-media-id]' ).each( function () {
			...
			const message =
				mediaWrite && mediaWrite.message
					? String( mediaWrite.message || '' )
					: '';
			$row.find( '.dbvc-cc-map-media-write-status' ).html(
				dbvc_cc_render_media_write_chip( mediaWrite )
			);
	...
	function dbvc_cc_update_mapping_summaries() {
		if ( mappingState.catalog ) {
			const fingerprint = String(
				mappingState.catalog.catalog_fingerprint || ''
			);
			const generatedAt = String(
				mappingState.catalog.generated_at || ''
			);
			$mappingCatalogSummary.text(
			...
			$mappingCatalogJson.text(
				JSON.stringify( mappingState.catalog, null, 2 )
			);
		}
		...
		if ( mappingState.sectionCandidates ) {
			...
			const unresolvedCount = Number(
				stats.unresolved_section_count || 0
			);
			const sectionStatus = mappingState.sectionCandidatesStatus || {};
			const staleText = sectionStatus.stale
				? `, stale (${ String(
						sectionStatus.stale_reason || 'unknown'
				  ) })`
				: '';
			$mappingSectionsSummary.text(
				`${ sectionCount } section(s), ${ unresolvedCount } unresolved${ staleText }`
			);
			$mappingSectionsJson.text(
				JSON.stringify( mappingState.sectionCandidates, null, 2 )
			);
		}
		...
		if ( mappingState.mediaCandidates ) {
			...
			const suppressedCount = Number(
				stats.suppressed_preview_count || 0
			);
			const policy = mappingState.mediaCandidates.policy || {};
			const previewMode =
				Number(
					policy.dbvc_cc_media_preview_thumbnail_enabled || 0
				) === 1
					? 'preview:on'
					: 'preview:off';
			const privateHostMode =
				Number( policy.dbvc_cc_media_block_private_hosts || 0 ) === 1
					? 'private-host-block:on'
					: 'private-host-block:off';
			const mediaStatus = mappingState.mediaCandidatesStatus || {};
			const staleText = mediaStatus.stale
				? `, stale (${ String(
						mediaStatus.stale_reason || 'unknown'
				  ) })`
				: '';
			$mappingMediaSummary.text(
				`${ total } media candidate(s), ${ previewCount } preview(s), blocked: ${ blockedCount }, suppressed: ${ suppressedCount }, ${ previewMode }, ${ privateHostMode }${ staleText }`
			);
			$mappingMediaJson.text(
				JSON.stringify( mappingState.mediaCandidates, null, 2 )
			);
		} else if ( ! dbvc_cc_media_mapping_bridge_enabled ) {
			$mappingMediaSummary.text(
				'disabled (text-only mapping remains available)'
			);
			$mappingMediaJson.text(
				JSON.stringify(
					{
		...
		const decisionSummary = mappingState.mappingDecision
			? `${ String(
					mappingState.mappingDecision.decision_status || 'pending'
			  ) } (${
					Array.isArray(
						mappingState.mappingDecision.approved_mappings
					)
						? mappingState.mappingDecision.approved_mappings.length
						: 0
			  } approved)`
			: 'n/a';
		...
		const mediaDecisionSummary = mappingState.mediaDecision
			? `${ String(
					mappingState.mediaDecision.decision_status || 'pending'
			  ) } (${
					Array.isArray( mappingState.mediaDecision.approved )
						? mappingState.mediaDecision.approved.length
						: 0
			  } approved)`
			: 'n/a';
		$mappingMediaDecisionSummary.text( mediaDecisionSummary );

		const handoffReviewHint =
			mappingState.handoffPayload &&
			mappingState.handoffPayload.review &&
			Array.isArray( mappingState.handoffPayload.review.reasons )
				? dbvc_cc_get_summary_issue_hint(
						{
							review_reasons:
								mappingState.handoffPayload.review.reasons,
						},
						[ 'review_reasons' ]
				  )
				: '';
		const handoffReviewCount =
			mappingState.handoffPayload && mappingState.handoffPayload.review
				? Number( mappingState.handoffPayload.review.reason_count || 0 )
				: 0;
		const handoffSummary = mappingState.handoffPayload
			? `${ String(
					mappingState.handoffPayload.status || 'needs_review'
			  ) } (${
					Array.isArray( mappingState.handoffPayload.warnings )
						? mappingState.handoffPayload.warnings.length
						: 0
			  } warning(s), review:${ handoffReviewCount }${
					handoffReviewHint ? `, reason:${ handoffReviewHint }` : ''
			  })`
			: 'n/a';
		...
		if (
			mappingState.handoffPayload &&
			mappingState.handoffPayload.phase4_input &&
			mappingState.handoffPayload.phase4_input.object_hints
		) {
			const dbvc_cc_hints =
				mappingState.handoffPayload.phase4_input.object_hints;
			defaultEntityReason = String(
				dbvc_cc_hints.default_entity_reason || ''
			);
			defaultEntityOverridePostType = String(
				dbvc_cc_hints.override_post_type || ''
			);
		}
		if (
			mappingState.importExecutorDryRun &&
			mappingState.importExecutorDryRun.trace &&
			mappingState.importExecutorDryRun.trace.default_entity_key
		) {
			defaultEntityKey = String(
				mappingState.importExecutorDryRun.trace.default_entity_key
			);
		} else if (
			mappingState.importPlanDryRun &&
			mappingState.importPlanDryRun.phase4_input &&
			mappingState.importPlanDryRun.phase4_input.default_entity_key
		) {
			defaultEntityKey = String(
				mappingState.importPlanDryRun.phase4_input.default_entity_key
			);
			...
			mappingState.handoffPayload &&
			mappingState.handoffPayload.phase4_input &&
			mappingState.handoffPayload.phase4_input.default_entity_key
		) {
			defaultEntityKey = String(
				mappingState.handoffPayload.phase4_input.default_entity_key
			);
		...
		if ( defaultEntityReason ) {
			dbvc_cc_default_entity_summary_parts.push(
				defaultEntityReason.replace( /_/g, ' ' )
			);
		}
		if ( defaultEntityOverridePostType ) {
			dbvc_cc_default_entity_summary_parts.push(
				`override:${ defaultEntityOverridePostType }`
			);
		}
		$mappingDefaultEntitySummary.text(
			dbvc_cc_default_entity_summary_parts.length > 0
				? dbvc_cc_default_entity_summary_parts.join( ' | ' )
				: 'n/a'
		);
		...
		if (
			mappingState.importExecuteSkeleton &&
			mappingState.importExecuteSkeleton.phase4_context
		) {
			dbvc_cc_phase4_context =
				mappingState.importExecuteSkeleton.phase4_context;
		} else if (
			mappingState.importExecutorDryRun &&
			mappingState.importExecutorDryRun.phase4_context
		) {
			dbvc_cc_phase4_context =
				mappingState.importExecutorDryRun.phase4_context;
		}
		...
		if ( ! dbvc_cc_phase4_context && mappingState.importPlanDryRun ) {
			dbvc_cc_phase4_context = {
				handoff_schema_version: String(
					mappingState.importPlanDryRun.handoff_schema_version || ''
				),
				handoff_generated_at: String(
					mappingState.importPlanDryRun.handoff_generated_at || ''
				),
				...
				suggested_post_type:
					...
					mappingState.handoffPayload.phase4_input.object_hints
						? String(
								mappingState.handoffPayload.phase4_input
									.object_hints.suggested_post_type || ''
						  )
						: '',
			};
		}
		...
		if (
			dbvc_cc_phase4_context &&
			dbvc_cc_phase4_context.handoff_schema_version
		) {
			dbvc_cc_phase4_context_parts.push(
				`handoff:${ String(
					dbvc_cc_phase4_context.handoff_schema_version
				) }`
			);
		...
		if (
			dbvc_cc_phase4_context &&
			dbvc_cc_phase4_context.default_entity_reason
		) {
			dbvc_cc_phase4_context_parts.push(
				`reason:${ String(
					dbvc_cc_phase4_context.default_entity_reason
		...
		if (
			dbvc_cc_phase4_context &&
			dbvc_cc_phase4_context.override_post_type
		) {
			dbvc_cc_phase4_context_parts.push(
				`override:${ String(
					dbvc_cc_phase4_context.override_post_type
		...
		if (
			dbvc_cc_phase4_context &&
			dbvc_cc_phase4_context.suggested_post_type
		) {
			dbvc_cc_phase4_context_parts.push(
				`suggested:${ String(
					dbvc_cc_phase4_context.suggested_post_type
		...
		$mappingPhase4ContextSummary.text(
			dbvc_cc_phase4_context_parts.length > 0
				? dbvc_cc_phase4_context_parts.join( ' | ' )
				: 'n/a'
		);

		const importPlanIssueHint = mappingState.importPlanDryRun
			? dbvc_cc_get_summary_issue_hint( mappingState.importPlanDryRun, [
					'issues',
			  ] )
			: '';
		const importPlanSummary = mappingState.importPlanDryRun
			? `${ String(
					mappingState.importPlanDryRun.status || 'blocked'
			  ) } (${ Number(
					mappingState.importPlanDryRun.blocking_issue_count || 0
			  ) } blocking issue(s)${
					importPlanIssueHint
						? `, reason:${ importPlanIssueHint }`
						: ''
			  })`
			: 'n/a';
		...
		const importExecutorIssueHint = mappingState.importExecutorDryRun
			? dbvc_cc_get_summary_issue_hint(
					mappingState.importExecutorDryRun,
					[ 'issues', 'write_barriers' ]
			  )
			: '';
		const importExecutorSummary = mappingState.importExecutorDryRun
			? `${ String(
					mappingState.importExecutorDryRun.status || 'blocked'
			  ) } (${ Number(
					mappingState.importExecutorDryRun.operation_counts &&
						mappingState.importExecutorDryRun.operation_counts
							.total_simulated
						? mappingState.importExecutorDryRun.operation_counts
								.total_simulated
					...
					mappingState.importExecutorDryRun.operation_counts &&
						...
						? mappingState.importExecutorDryRun.operation_counts
								.write_barrier_count
						: 0
			  ) }, deferred media:${ Number(
					mappingState.importExecutorDryRun.deferred_media_count ||
						( mappingState.importExecutorDryRun.operation_counts &&
						mappingState.importExecutorDryRun.operation_counts
							.deferred_media_count
							? mappingState.importExecutorDryRun.operation_counts
									.deferred_media_count
							: 0 )
			  ) }, ${ Number(
					mappingState.importExecutorDryRun.blocking_issue_count || 0
			  ) } blocking issue(s)${
					importExecutorIssueHint
		...
		const importApprovalSummary = mappingState.importPreflightApproval
			? `${ String(
					mappingState.importPreflightApproval.status || 'blocked'
			  ) } (eligible:${
					mappingState.importPreflightApproval.approval_eligible
						? 'yes'
						: 'no'
			  }, valid:${
					mappingState.importPreflightApproval.approval_valid
					...
					mappingState.importPreflightApproval.deferred_media_count ||
						( mappingState.importPreflightApproval
							.operation_counts &&
						mappingState.importPreflightApproval.operation_counts
							.deferred_media_count
							? mappingState.importPreflightApproval
									.operation_counts.deferred_media_count
							: 0 )
			  ) }, execute:${
					mappingState.importPreflightApproval.approval_valid
						? 'continues without deferred media'
						: 'not armed'
			  }, expires:${ String(
					mappingState.importPreflightApproval.approval &&
						mappingState.importPreflightApproval.approval.expires_at
						? mappingState.importPreflightApproval.approval
								.expires_at
						: 'n/a'
			  ) }${
					importApprovalIssueHint
						? `, reason:${ importApprovalIssueHint }`
						: ''
			  })`
			: 'approval required';
		$mappingImportApprovalSummary.text( importApprovalSummary );
		const executeAutoRollbackSummary =
			mappingState.importExecuteSkeleton &&
			mappingState.importExecuteSkeleton.auto_rollback &&
			mappingState.importExecuteSkeleton.auto_rollback.attempted
				? `, auto rollback:${ String(
						mappingState.importExecuteSkeleton.auto_rollback
							.status || 'unknown'
				  ) }, restored:${ Number(
						mappingState.importExecuteSkeleton.auto_rollback
		...
		const importExecuteIssueHint = mappingState.importExecuteSkeleton
			? dbvc_cc_get_summary_issue_hint(
					mappingState.importExecuteSkeleton,
					[ 'guard_failures', 'write_barriers', 'execution_failures' ]
			  )
			: '';
		const importExecuteSummary = mappingState.importExecuteSkeleton
			? `${ String(
					mappingState.importExecuteSkeleton.status || 'blocked'
			  ) } (run:${ Number(
					mappingState.importExecuteSkeleton.run_id || 0
			  ) }, rollback:${ String(
					mappingState.importExecuteSkeleton.rollback_status || 'n/a'
			  ) }, guards:${ Number(
					mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.guard_failure_count
						? mappingState.importExecuteSkeleton.operation_counts
								.guard_failure_count
					...
					mappingState.importExecuteSkeleton.operation_counts &&
						...
						? mappingState.importExecuteSkeleton.operation_counts
								.write_barrier_count
						: 0
			  ) }, executed entity:${ Number(
					mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.executed_entity_writes
						? mappingState.importExecuteSkeleton.operation_counts
								.executed_entity_writes
						: 0
			  ) }, executed field:${ Number(
					mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.executed_field_writes
						? mappingState.importExecuteSkeleton.operation_counts
								.executed_field_writes
						: 0
			  ) }, executed media:${ Number(
					mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.executed_media_writes
						? mappingState.importExecuteSkeleton.operation_counts
								.executed_media_writes
						: 0
			  ) }, failed entity:${ Number(
					mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.failed_entity_writes
						? mappingState.importExecuteSkeleton.operation_counts
								.failed_entity_writes
						: 0
			  ) }, failed field:${ Number(
					...
					mappingState.importExecuteSkeleton.deferred_media_count ||
						( mappingState.importExecuteSkeleton.operation_counts &&
						mappingState.importExecuteSkeleton.operation_counts
							.deferred_media_writes
							? mappingState.importExecuteSkeleton
									.operation_counts.deferred_media_writes
							: 0 )
			  ) }${
					importExecuteIssueHint
						? `, reason:${ importExecuteIssueHint }`
						: ''
			  }${ executeAutoRollbackSummary })`
			: 'n/a';
		$mappingImportExecuteSummary.text( importExecuteSummary );
		const importRecoverySummary = mappingState.importRecovery
			? `${ String(
					mappingState.importRecovery.status || 'unknown'
			  ) } (run:${ Number(
					mappingState.importRecovery.run &&
						mappingState.importRecovery.run.id
						? mappingState.importRecovery.run.id
						: 0
			  ) }, restored:${ Number(
					mappingState.importRecovery.restored_count || 0
			  ) }, failed:${ Number(
					mappingState.importRecovery.failed_count || 0
			  ) })`
			: mappingState.importExecuteSkeleton &&
			  Number( mappingState.importExecuteSkeleton.run_id || 0 ) > 0
			? `${ String(
		...
		const runs = Array.isArray( runHistory.runs ) ? runHistory.runs : [];
		const selectedRun =
			mappingState.importRunDetail && mappingState.importRunDetail.run
				? mappingState.importRunDetail.run
				: null;
		const runHistorySummary = ! target.valid
			...
			: `${ runs.length } run(s) loaded${
					selectedRun && selectedRun.id
						? `, selected #${ Number( selectedRun.id || 0 ) }`
						: ''
			  }`;
		$mappingRunHistorySummary.text( runHistorySummary );

		$mappingDecisionsJson.text(
			JSON.stringify(
				{
		...
		$mappingHandoffJson.text(
			JSON.stringify( mappingState.handoffPayload, null, 2 )
		);
		$mappingImportPlanJson.text(
			JSON.stringify( mappingState.importPlanDryRun, null, 2 )
		);
		$mappingImportExecutorJson.text(
			JSON.stringify( mappingState.importExecutorDryRun, null, 2 )
		);
		$mappingImportApprovalJson.text(
			JSON.stringify( mappingState.importPreflightApproval, null, 2 )
		);
		$mappingImportExecuteJson.text(
			JSON.stringify( mappingState.importExecuteSkeleton, null, 2 )
		);
		$mappingImportRecoveryJson.text(
			JSON.stringify( mappingState.importRecovery, null, 2 )
		);
		$mappingRunHistoryJson.text(
			JSON.stringify( mappingState.importRunHistory, null, 2 )
		);
		$mappingRunDetailJson.text(
			JSON.stringify( mappingState.importRunDetail, null, 2 )
		);
		dbvc_cc_update_media_write_annotations();
	...
	async function dbvc_cc_load_section_candidates( forceRebuild = false ) {
		...
		const response = await apiRequest(
			`mapping/candidates?${ params.toString() }`
		);
		mappingState.sectionCandidates =
			response && response.section_candidates
				? response.section_candidates
				: null;
		mappingState.sectionCandidatesStatus = {
			...
			stale_reason:
				response && response.stale_reason
					? String( response.stale_reason )
					: '',
		};
	...
	async function dbvc_cc_load_media_candidates( forceRebuild = false ) {
		...
		const response = await apiRequest(
			`mapping/media/candidates?${ params.toString() }`
		);
		mappingState.mediaCandidates =
			response && response.media_candidates
				? response.media_candidates
				: null;
		mappingState.mediaCandidatesStatus = {
			status:
				response && response.status
	...
	function dbvc_cc_apply_loaded_decisions_to_form() {
		...
		if ( mappingDecision ) {
			const approved = Array.isArray( mappingDecision.approved_mappings )
				? mappingDecision.approved_mappings
				: [];
			const overrides = Array.isArray( mappingDecision.overrides )
				? mappingDecision.overrides
				: [];
			const rejections = Array.isArray( mappingDecision.rejections )
				? mappingDecision.rejections
				: [];
			...
			approved.forEach( ( row ) => {
				...
				approvedBySection[ String( row.section_id ) ] = String(
					row.target_ref || ''
				);
			} );
			...
			overrides.forEach( ( row ) => {
				...
				overrideBySection[ String( row.section_id ) ] = String(
					row.override_target_ref || row.target_ref || ''
				);
			} );
			...
			$mappingSectionsTableBody
				.find( 'tr[data-section-id]' )
				.each( function () {
					const $row = $( this );
					...
					$row.find( '.dbvc-cc-map-section-override' ).val(
						overrideRef
					);
					$row.find( '.dbvc-cc-map-section-ignore' ).prop(
						'checked',
						isRejected
					);
				} );
			...
			if ( mappingDecision.decision_status ) {
				$mappingDecisionStatus.val(
					String( mappingDecision.decision_status )
				);
			}

			const mappingObjectHints =
				mappingDecision.object_hints &&
				typeof mappingDecision.object_hints === 'object'
					? mappingDecision.object_hints
					: null;
			if ( mappingObjectHints && mappingObjectHints.override_post_type ) {
				dbvc_cc_set_object_post_type_value(
					String( mappingObjectHints.override_post_type )
				);
			} else {
				dbvc_cc_set_object_post_type_value( '' );
			}
		...
		if ( mediaDecision ) {
			const approved = Array.isArray( mediaDecision.approved )
				? mediaDecision.approved
				: [];
			const overrides = Array.isArray( mediaDecision.overrides )
				? mediaDecision.overrides
				: [];
			const ignored = Array.isArray( mediaDecision.ignored )
				? mediaDecision.ignored
				: [];
			...
			approved.forEach( ( row ) => {
				...
				approvedByMedia[ String( row.media_id ) ] = String(
					row.target_ref || ''
				);
			} );
			...
			overrides.forEach( ( row ) => {
				...
				overrideByMedia[ String( row.media_id ) ] = String(
					row.override_target_ref || row.target_ref || ''
				);
			} );
			...
			$mappingMediaTableBody
				.find( 'tr[data-media-id]' )
				.each( function () {
					const $row = $( this );
					const mediaId = String( $row.data( 'media-id' ) || '' );
					...
					$row.find( '.dbvc-cc-map-media-override' ).val(
						overrideRef
					);
					$row.find( '.dbvc-cc-map-media-ignore' ).prop(
						'checked',
						isIgnored
					);
				} );
			...
			if ( mediaDecision.decision_status ) {
				$mappingMediaDecisionStatus.val(
					String( mediaDecision.decision_status )
				);
			}
		}
	}
	...
	async function dbvc_cc_load_decisions() {
		...
		const [ mappingDecisionResponse, mediaDecisionResponse ] =
			await Promise.all( [
				apiRequest( `mapping/decision?${ mappingParams.toString() }` ),
				apiRequest(
					`mapping/media/decision?${ mappingParams.toString() }`
				),
			] );

		mappingState.mappingDecision =
			mappingDecisionResponse && mappingDecisionResponse.mapping_decision
				? mappingDecisionResponse.mapping_decision
				: null;
		mappingState.mediaDecision =
			mediaDecisionResponse && mediaDecisionResponse.media_decision
	...
	async function dbvc_cc_load_handoff_payload() {
		...
		const response = await apiRequest(
			`mapping/handoff?${ params.toString() }`
		);
		...
		if (
			! dbvc_cc_has_mapping_override &&
			response &&
			response.phase4_input &&
			response.phase4_input.object_hints
		) {
			dbvc_cc_set_object_post_type_value(
				String(
					response.phase4_input.object_hints.override_post_type || ''
				)
			);
		}
		dbvc_cc_update_mapping_summaries();
	...
	async function dbvc_cc_load_import_plan_dry_run() {
		...
		const response = await apiRequest(
			`import-plan/dry-run?${ params.toString() }`
		);
	...
	async function dbvc_cc_load_import_executor_dry_run() {
		...
		const response = await apiRequest(
			`import-executor/dry-run?${ params.toString() }`
		);
		dbvc_cc_reset_import_execution_state();
	...
	async function dbvc_cc_load_import_run_detail( runId, options = {} ) {
		...
		const response = await apiRequest(
			`import-executor/run?${ params.toString() }`
		);
	...
	async function dbvc_cc_load_import_run_history( options = {} ) {
		...
		const response = await apiRequest(
			`import-executor/runs?${ params.toString() }`
		);
		const runs =
			response && Array.isArray( response.runs ) ? response.runs : [];
		const requestedRunId = dbvc_cc_normalize_run_id( options.selectRunId );
		const preservedRunId = dbvc_cc_normalize_run_id(
			options.preserveSelection ? mappingState.importSelectedRunId : 0
		);
		const fallbackRunId =
			runs.length > 0 ? dbvc_cc_normalize_run_id( runs[ 0 ].id || 0 ) : 0;
		...
		if (
			...
			! runs.some(
				( run ) =>
					dbvc_cc_normalize_run_id( run && run.id ? run.id : 0 ) ===
					nextSelectedRunId
			)
		) {
			nextSelectedRunId = 0;
			...
			mappingState.importRunHistory = {
				...
				load_error:
					error && error.message
						...
						: 'Failed to load import run history.',
			};
			mappingState.importRunDetail = null;
			mappingState.importSelectedRunId = 0;
			dbvc_cc_update_mapping_summaries();
	...
	async function dbvc_cc_run_import_rollback( runId = 0 ) {
		...
		if ( mappingState.importExecuteSkeleton ) {
			const executeRunId = dbvc_cc_normalize_run_id(
				mappingState.importExecuteSkeleton.run_id || 0
			);
			if ( executeRunId === normalizedRunId ) {
				mappingState.importExecuteSkeleton.rollback_available = false;
				mappingState.importExecuteSkeleton.rollback_status =
					response && response.run && response.run.rollback_status
		...
		await dbvc_cc_refresh_import_run_history_safely( {
			selectRunId:
				response && response.run && response.run.id
					? response.run.id
					: normalizedRunId,
			silent: true,
		} );
	...
	function dbvc_cc_build_mapping_decision_payload() {
		...
		$mappingSectionsTableBody
			...
			.each( function () {
				const $row = $( this );
				const sectionId = String( $row.data( 'section-id' ) || '' );
				const $targetSelect = $row.find(
					'.dbvc-cc-map-section-target'
				);
				const selectedTarget = String(
					$targetSelect.val() || ''
				).trim();
				const overrideTarget = String(
					$row.find( '.dbvc-cc-map-section-override' ).val() || ''
				).trim();
				const ignored = $row
					.find( '.dbvc-cc-map-section-ignore' )
					.is( ':checked' );
				const selectedOption = $targetSelect.find( 'option:selected' );
				const candidateId = String(
					selectedOption.data( 'candidate-id' ) || ''
				);
				const candidateConfidence = Number(
					selectedOption.data( 'confidence' ) || 0
				);
				const reason = String(
					selectedOption.data( 'reason' ) || 'deterministic'
				);
		...
		const catalogFingerprint =
			mappingState.sectionCandidates &&
			mappingState.sectionCandidates.catalog_fingerprint
				? String( mappingState.sectionCandidates.catalog_fingerprint )
				: mappingState.catalog &&
				  mappingState.catalog.catalog_fingerprint
				? String( mappingState.catalog.catalog_fingerprint )
				: '';
		const objectPostTypeOverride = dbvc_cc_normalize_post_type(
			$mappingObjectPostType.val() || ''
		);
		...
		return {
			...
			decision_status: String(
				$mappingDecisionStatus.val() || 'pending'
			),
	...
	function dbvc_cc_build_media_decision_payload() {
		...
		$mappingMediaTableBody.find( 'tr[data-media-id]' ).each( function () {
			...
			const selectedTarget = String(
				$row.find( '.dbvc-cc-map-media-target' ).val() || ''
			).trim();
			const overrideTarget = String(
				$row.find( '.dbvc-cc-map-media-override' ).val() || ''
			).trim();
			const isIgnored = $row
				.find( '.dbvc-cc-map-media-ignore' )
				.is( ':checked' );
			const resolvedTarget = overrideTarget || selectedTarget;
		...
		const catalogFingerprint =
			mappingState.mediaCandidates &&
			mappingState.mediaCandidates.catalog_fingerprint
		...
		return {
			...
			catalog_fingerprint: catalogFingerprint,
			decision_status: String(
				$mappingMediaDecisionStatus.val() || 'pending'
			),
			approved,
			overrides,
			ignored,
	...
	async function dbvc_cc_load_mapping_package() {
		if ( ! dbvc_cc_mapping_bridge_enabled ) {
			dbvc_cc_set_mapping_status(
				'Mapping bridge feature flags are disabled.',
				true
			);
			return;
		}
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to load mapping package.',
				true
			);
		}
	}
	...
	async function dbvc_cc_save_mapping_decision() {
		...
		try {
			...
			mappingState.mappingDecision =
				response && response.mapping_decision
					? response.mapping_decision
					: null;
			dbvc_cc_reset_import_execution_state( {
				clearDryRun: true,
				clearPlan: true,
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to save mapping decision.',
				true
			);
		}
	}
	...
	async function dbvc_cc_save_media_decision() {
		if ( ! dbvc_cc_media_mapping_bridge_enabled ) {
			dbvc_cc_set_mapping_status(
				'Media mapping bridge is disabled. Text-only mapping remains available.',
				true
			);
			return;
		}
		...
		try {
			...
			mappingState.mediaDecision =
				response && response.media_decision
					? response.media_decision
					: null;
			dbvc_cc_reset_import_execution_state( {
				clearDryRun: true,
				clearPlan: true,
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to save media decision.',
				true
			);
		}
	}
	...
	async function dbvc_cc_fetch_mapping_rebuild_batch_status( batchId ) {
		...
		return apiRequest(
			`mapping/domain/rebuild/status?${ params.toString() }`
		);
	}
	...
	async function dbvc_cc_poll_mapping_rebuild_batch( batchId, options = {} ) {
		...
		while ( polls < maxPolls ) {
			...
			const statusResponse =
				await dbvc_cc_fetch_mapping_rebuild_batch_status( batchId );
			const totalJobs = Number(
				statusResponse && statusResponse.total_jobs
					? statusResponse.total_jobs
					: 0
			);
			const processedJobs = Number(
				statusResponse && statusResponse.processed_jobs
					? statusResponse.processed_jobs
					: 0
			);
			const failedJobs = Number(
				statusResponse && statusResponse.failed_jobs
					? statusResponse.failed_jobs
					: 0
			);
			const progressPercent = Number(
				statusResponse && statusResponse.progress_percent
					? statusResponse.progress_percent
					: 0
			);
			const status = String(
			...
			if ( status === 'completed_with_failures' ) {
				dbvc_cc_set_mapping_status(
					`Domain rebuild completed with issues (${ processedJobs }/${ totalJobs } jobs, ${ failedJobs } failed).`,
					true
				);
			...
			if ( status === 'completed' ) {
				dbvc_cc_set_mapping_status(
					`Domain rebuild completed (${ processedJobs }/${ totalJobs } jobs).`
				);
				return statusResponse;
			}
			...
			if ( status === 'failed' ) {
				dbvc_cc_set_mapping_status(
					`Domain rebuild failed after ${ processedJobs }/${ totalJobs } jobs.`,
					true
				);
				return statusResponse;
			}
			...
			dbvc_cc_set_mapping_status(
				`Rebuilding mapping artifacts for domain... ${ progressPercent }% (${ processedJobs }/${ totalJobs })`
			);
			await new Promise( ( resolve ) =>
		...
		throw new Error(
			'Domain rebuild polling timed out. Check batch status and retry.'
		);
	}
	...
	async function dbvc_cc_rebuild_domain_mapping_artifacts() {
		...
		const dbvc_cc_domain = dbvc_cc_normalize_domain(
			$mappingDomain.val() || $domain.val()
		);
		if ( ! dbvc_cc_domain ) {
			dbvc_cc_set_mapping_status(
				'Select a domain before rebuilding mapping artifacts.',
				true
			);
			return;
		}
		...
		const originalLabel = String(
			$button.text() || 'Rebuild Domain Mapping Artifacts'
		);
		$button.prop( 'disabled', true ).text( 'Rebuilding...' );

		dbvc_cc_set_mapping_status(
			`Queueing mapping rebuild for ${ dbvc_cc_domain }...`
		);
		...
		try {
			...
			const batchId = String(
				response && response.batch_id ? response.batch_id : ''
			);
			if ( ! batchId ) {
			...
			try {
				...
				dbvc_cc_set_mapping_status(
					`Domain rebuild completed. Mapping preview refresh failed: ${
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to rebuild domain mapping artifacts.',
				true
			);
		} finally {
	...
	$domain.on( 'change', function () {
		const dbvc_cc_selected_domain = dbvc_cc_normalize_domain(
			$domain.val()
		);
		dbvc_cc_render_domain_ai_warning();
		...
		const dbvc_cc_current_mapping_domain = dbvc_cc_normalize_domain(
			$mappingDomain.val()
		);
	...
	$( '#dbvc-cc-workbench-open-mapping' ).on( 'click', function () {
		...
		const mappingSection = document.querySelector(
			'.dbvc-cc-mapping-bridge'
		);
		if (
			mappingSection &&
			typeof mappingSection.scrollIntoView === 'function'
		) {
	...
	$( document ).on( 'keydown', function ( event ) {
		...
		if (
			$mappingHelpModal.length === 0 ||
			$mappingHelpModal.hasClass( 'dbvc-cc-hidden' )
		) {
			return;
		}
	...
	$( '#dbvc-cc-mapping-refresh-run-history' ).on( 'click', async function () {
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to load import run history.',
				true
			);
		}
	} );

	$mappingRunHistoryTableBody.on(
		'click',
		'tr[data-run-id]',
		async function () {
			const runId = dbvc_cc_normalize_run_id(
			...
			try {
				...
				dbvc_cc_set_mapping_status(
					error.message || 'Failed to load import run detail.',
					true
				);
			}
		}
	...
	$mappingRunFilterStage.on( 'change', function () {
		mappingState.importRunActionFilters.stage = String(
			$( this ).val() || ''
		);
		mappingState.importSelectedActionId = 0;
		dbvc_cc_render_import_run_detail();
	...
	$mappingRunFilterExecution.on( 'change', function () {
		mappingState.importRunActionFilters.execution = String(
			$( this ).val() || ''
		);
		mappingState.importSelectedActionId = 0;
		dbvc_cc_render_import_run_detail();
	...
	$mappingRunFilterRollback.on( 'change', function () {
		mappingState.importRunActionFilters.rollback = String(
			$( this ).val() || ''
		);
		mappingState.importSelectedActionId = 0;
		dbvc_cc_render_import_run_detail();
	...
	$mappingRunFilterFailedOnly.on( 'change', function () {
		mappingState.importRunActionFilters.failedOnly =
			$( this ).is( ':checked' );
		mappingState.importSelectedActionId = 0;
		dbvc_cc_render_import_run_detail();
	} );
	...
	$( '#dbvc-cc-mapping-download-run-report' ).on( 'click', function () {
		...
		if ( ! run || dbvc_cc_normalize_run_id( run.id ) <= 0 ) {
			dbvc_cc_set_mapping_status(
				'Select an import run before downloading a report.',
				true
			);
			return;
		}
		...
		link.download = `dbvc-cc-import-run-${ dbvc_cc_normalize_run_id(
			run.id
		) }.json`;
		...
		dbvc_cc_set_mapping_status(
			`Downloaded JSON report for import run #${ dbvc_cc_normalize_run_id(
				run.id
			) }.`
		);
	} );
	...
	$( '#dbvc-cc-mapping-build-catalog' ).on( 'click', async function () {
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to build catalog.',
				true
			);
		}
	} );
	...
	$( '#dbvc-cc-mapping-refresh-catalog' ).on( 'click', async function () {
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to refresh catalog.',
				true
			);
		}
	} );
	...
	$( '#dbvc-cc-mapping-load-handoff' ).on( 'click', async function () {
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to load handoff payload.',
				true
			);
		}
	} );

	$( '#dbvc-cc-mapping-generate-dry-run-plan' ).on(
		'click',
		async function () {
			...
			try {
				...
				dbvc_cc_set_mapping_status(
					error.message || 'Failed to generate dry-run import plan.',
					true
				);
			}
		}
	...
	$( '#dbvc-cc-mapping-run-executor-dry-run' ).on(
		'click',
		async function () {
			if ( ! dbvc_cc_import_executor_enabled ) {
				dbvc_cc_set_mapping_status(
					'Import executor dry-run is currently disabled by feature flags.',
					true
				);
				return;
			}
			...
			try {
				...
				dbvc_cc_set_mapping_status(
					'Import executor dry-run completed.'
				);
			} catch ( error ) {
				dbvc_cc_set_mapping_status(
					error.message || 'Failed to run import executor dry-run.',
					true
				);
			}
		}
	...
	$( '#dbvc-cc-mapping-approve-import' ).on( 'click', async function () {
		if ( ! dbvc_cc_import_executor_enabled ) {
			dbvc_cc_set_mapping_status(
				'Import execute approval is currently disabled by feature flags.',
				true
			);
			return;
		}
		...
		try {
			...
			dbvc_cc_set_mapping_status(
				'Preflight approval granted for the current dry-run fingerprint.'
			);
		} catch ( error ) {
			dbvc_cc_set_mapping_status(
				error.message ||
					'Failed to approve the current dry-run for execute.',
				true
			);
		}
	} );

	$( '#dbvc-cc-mapping-run-execute-skeleton' ).on(
		'click',
		async function () {
			...
			try {
				...
				const dbvc_cc_execute_status =
					mappingState.importExecuteSkeleton &&
					mappingState.importExecuteSkeleton.status
						? String( mappingState.importExecuteSkeleton.status )
						: 'unknown';
				dbvc_cc_set_mapping_status(
					`Phase 4 execute finished with status: ${ dbvc_cc_execute_status }.`
				);
			} catch ( error ) {
				dbvc_cc_set_mapping_status(
					error.message || 'Failed to run execute.',
					true
				);
			}
		}
		...
		try {
			...
			const rollbackStatus =
				response && response.status
					...
					: 'unknown';
			dbvc_cc_set_mapping_status(
				`Rollback finished with status: ${ rollbackStatus }.`
			);
		} catch ( error ) {
			dbvc_cc_set_mapping_status(
				error.message || 'Failed to roll back the latest import run.',
				true
			);
		}
	} );

	$( '#dbvc-cc-mapping-rollback-selected-run' ).on(
		'click',
		async function () {
			const runId = dbvc_cc_normalize_run_id(
				mappingState.importRunDetail &&
			...
			if ( runId <= 0 ) {
				dbvc_cc_set_mapping_status(
					'Select a run before attempting rollback.',
					true
				);
				return;
			}
			...
			dbvc_cc_set_mapping_status(
				`Rolling back selected import run #${ runId }...`
			);
			try {
				const response = await dbvc_cc_run_import_rollback( runId );
				const rollbackStatus =
					response && response.status
						...
						: 'unknown';
				dbvc_cc_set_mapping_status(
					`Selected run rollback finished with status: ${ rollbackStatus }.`
				);
			} catch ( error ) {
				dbvc_cc_set_mapping_status(
					error.message ||
						'Failed to roll back the selected import run.',
					true
				);
			}
		}
	...
	$mappingMediaTableBody.on(
		'change',
		'.dbvc-cc-map-media-target',
		function () {
			dbvc_cc_sync_target_select_titles( $( this ).closest( 'tr' ) );
		}
	);
