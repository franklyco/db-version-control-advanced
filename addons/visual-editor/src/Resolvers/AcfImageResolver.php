...
final class AcfImageResolver extends AbstractAcfResolver
{
    /**
     * @return string
     */
    ...
    public function supports(EditableDescriptor $descriptor)
    {
        return $this->supportsAcfSource($descriptor)
            && ($descriptor->source['field_type'] ?? '') === 'image';
    }
    ...
    {
        return $this->normalizeImageValue($this->getRawAcfValue($descriptor), $descriptor);
    }
    ...
    public function validate(EditableDescriptor $descriptor, $value)
    {
        ...
        if (! is_array($value)) {
            return [
                'ok' => false,
                'message' => __('This image field expects a Media Library attachment selection or a local media URL fallback.', 'dbvc'),
            ];
        }
    ...
    public function save(EditableDescriptor $descriptor, $value)
    {
        ...
        $result = $this->writeAcfValue($descriptor, $attachment_id > 0 ? $attachment_id : 0);
        if (empty($result['ok'])) {
            return $result;
        }
