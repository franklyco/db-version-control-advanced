...
| `R2` | Implemented through R2-F Slice 3 + R2-G; R2-H Phase 1 in progress (Slices 1-2 done) | Per-field remediation with expected-empty precondition, shared audited mutation pipeline, journaling, cache invalidation, and no-reload DOM patch. Persistent Media Index (R2-H) landing incrementally. |
...
| `R5.later-perf` | PAUSED 2026-09-07 | Instrumentation (Task 2) landed 2026-09-06; measurement recipe (Task 3) landed 2026-09-06; sweep (Task 4) surfaced uniform 3.7s LocalWP TTFB uncorrelated with Xdebug / `WP_DEBUG_LOG` / autoload / object cache — investigation deferred. Instrumentation stays gated OFF via URL param. |
