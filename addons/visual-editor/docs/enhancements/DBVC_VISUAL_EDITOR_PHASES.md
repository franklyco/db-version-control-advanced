...
| `P1` | Post-owned linked-term collection editing in loop cards | Dedicated branch is WIP for Bricks term roots and native taxonomy/terms elements. Browser QA found two distinct `Category Terms` markers on the contractors vertical, but the active LocalWP asset collapsed the visible container badge to the first card; the worktree now keys related term badges by public source group when compact descriptors omit source metadata. | Retest against an active plugin build, then finish term search, no-reload save, optional reload, and rendered chip update QA for each concrete owner card. | `DBVC_VISUAL_EDITOR_COLLECTION_EDITOR_PLAN.md` |
