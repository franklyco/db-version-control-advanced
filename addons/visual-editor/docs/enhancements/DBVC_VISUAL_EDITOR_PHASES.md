...
| `R1` | COMPLETE — signed off (R1-A…R1-E; authenticated-runtime, real-AT and real-Safari evidence carried as the D-049 residual on R2) | Policy/catalog, bounded scan/snapshot, protected active-mode lifecycle/list/group routes, safe search/filter/sort/cursor projection, single-row current-state revalidation, the feature-gated shell, frontend lifecycle/query controller, server-driven laptop/desktop safe-results table, and lazy one-row field-status expansion are implemented. R1-E now covers focus continuity, native keyboard disclosure, reduced motion, named semantic regions/loading/live announcements, Chromium/Firefox/WebKit engine coverage, targeted lint, no-auto-scan proof, and synthetic 100/500/2,000-group snapshot/read/payload measurements; authenticated runtime, complete candidate-scan scale, real AT, real Safari, and aggregate lint completion remain. D-036 preserves the proven responsive baseline but tables additional mobile-friendly layouts, responsive cards/slide-overs, touch refinements, real-device optimization, and mobile-specific mockup/QA work across the remaining releases until explicitly reauthorized. Descriptor issuance, Media Library actions, and content mutation remain absent from R1. |
| `R2` | COMPLETE through R2-F Slice 4 + R2-G + R2-H Phase 1 (Slices 1–5); residual = real-browser/AT Media Manager QA on an authenticated runtime (D-049) and the unscheduled Media Index Phase 2 (cron reconciliation) | Per-field remediation with expected-empty precondition, shared audited mutation pipeline, journaling, cache invalidation, and no-reload DOM patch. Persistent Media Index (R2-H) landing incrementally. |
| `R3` | COMPLETE | Provider-agnostic control registry (R3-A) + Shared Globals popover (R3-B) + Brand Control Center drawer bridge (R3-C-1/-2) all shipped. Zero new write authority; every save routes through the existing R2 mutation contract. |
| `R4` | COMPLETE + defensively hardened by R4-D-3 | R4-A backend (search/family filter + `POST /value-summaries` batch endpoint + lazy IntersectionObserver hydration); R4-B mockup translation; R4-C production drawer; R4-D-1 save-status-strip; R4-D-2 coexistence + long-label calibration; R4-D-3 label overflow-wrap defense. |
| `R5.1`-`R5.5` | COMPLETE | R5.1 text/textarea/url/email/number; R5.2 choice/link/wysiwyg + `+color_picker`; R5.3 image; R5.4 relationship/post_object; R5.5 date_picker; R5.6 hardening; true_false. Every family has its own resolver + descriptor factory + value-summary builder + panel controller. |
| `R5.7` | COMPLETE | Repeater/flexible inline tree — parent rows with disclosure triangles + child-count chips + synthetic per-row controls. |
| `R5.later-a / .b / .y / .y-2 / .y-3` | COMPLETE | Palette grouping (curator opt-in) + palette parent overview panel + bulk-edit popover + inline swatch editing + pre-minted leaf descriptors for 1-round-trip first save. |
| `R5.later-tests` | LANDED 2026-09-05 | Overlay-app.js jsdom scaffolding — 15-test suite covering `renderEditorPanel` dispatch per family + palette controllers + `handleSave` guard + bulk popover copy paths + `handleOpenControlRequest` routing. See E-134. |
| `R5.later-toolbar-skin (.a/.b/.c)` | COMPLETE 2026-09-11 | Structural reskin into one unified pill + light-glassy → dark color scheme + aria-expanded orange split. Full mockup match. See E-135 / E-136 / E-141. |
| `R5.later-darkmode (.a/.b/.c)` | COMPLETE 2026-09-10 | System `prefers-color-scheme: dark` support via component-scoped tokens (.a/.b) + token-level global override (.c). Every VE surface adapts. See E-137 / E-138 / E-139. |
| `R5.later-c` | LANDED 2026-09-10 | Drawer inline current-value display — R4-C-1b's chip moves from action cell to label cell (after `.__meta`); every R5.x family renders inline; palette/repeater parents show compact aggregate. See E-140. |
| `R5.later-perf` | **perf-c LANDED 2026-09-17** (E-163: record-pass memo + one-round-trip session attach — palette open 13.9 s → 4.5 s in the browser, value-summary lookups 560 ms → 0.3 ms per batch; no switch). Next: perf-c-2 (control-record cache, audit F1) / perf-d (client). Earlier: SWEEP S1–S6 COMPLETE 2026-09-17 (E-162, `qa/R5-LATER-PERF-AUDIT-REPORT.md`): every wait is server-side — ACF store re-materialised per control-center request (2.7 s), unmemoised `collectValidRecords()`, 20 serial session attaches per palette open, under-filled value-summary batches; next = perf-c (server) / perf-d (client) decision, S7/S8 maintainer-only. Earlier: RESUMED 2026-09-17 — TTFB root cause found (E-159); **R5.later-perf-a landed** (E-160: fingerprint-pinned opt-in shim + upstream patch, −1.5 s/request) and **R5.later-perf-b landed** (E-161: fingerprinted, event-invalidated, cron-verified cache of Bricks' ACF field schema, −2 s/request); both default off; with both on `init` ≈ 0.5 s here. Next: maintainer enables + measures; then the sweep; perf-c (ACF-store warm-up) deferred | Instrumentation (Task 2) landed 2026-09-06; measurement recipe (Task 3) landed 2026-09-06; sweep (Task 4) surfaced uniform 3.7s LocalWP TTFB uncorrelated with Xdebug / `WP_DEBUG_LOG` / autoload / object cache — investigation deferred. Instrumentation stays gated OFF via URL param. |
| `R5.later-cache` | PLANNED | Session-persistent client-side scan cache. 7 correctness invariants + localStorage LRU + kill-switch default OFF. Sequenced after perf resumes. |
| `R6` | Code complete + live-QA'd 2026-09-15 (E-142–E-152); maintainer sign-off pending (3 checklist rows) | Persistent laptop/desktop Frontend Site Manager Workspace integrating existing tools; it must not duplicate Media Manager or Go To Object logic. R6-A shipped the shared `ObjectNavigationReadModel` + paginated/typed/honest-route `object-search` contract; R6-B froze the runtime contract (`releases/R6-WORKSPACE-STATE-CONTRACT.md`); R6-C delivered the accepted static mockup (`docs/ui-mockups/dbvc-visual-editor/r6-site-manager-workspace/`, D-074–D-077). R6-D-1 shipped the flag-gated production shell (`dbvc_visual_editor_workspace_enabled`, default off); R6-D-2 the Navigate panel against the R6-A read model; R6-D-3 the overlay-app hooks (Review Fields routing, panel clamp inset) with headless integration QA against the real BCC script. R6-E added permission/perf PHPUnit, fallback + polish coverage, axe/keyboard evidence, release notes + rollback (`releases/SITE-MANAGER-WORKSPACE-RELEASE-NOTES-AND-ROLLBACK.md`) and the maintainer checklist (`qa/R6-SITE-MANAGER-WORKSPACE-QA-CHECKLIST.md`). Default-off flag `dbvc_visual_editor_workspace_enabled`. Additional responsive/mobile variants remain tabled by D-058. |
| `VE-prefs-1` | Landed 2026-09-15 (E-147) | Toolbar Preferences popover with a System/Light/Dark appearance override for Visual Editor surfaces (per-viewer localStorage; attribute-driven dark blocks; no host-site leak). First slice of a `VE-prefs` umbrella for future per-viewer preferences. |
| `VE-prefs-2` | Landed 2026-09-16 (E-158, D-081) | "Site Manager on page load" preference (Remember / Always open / Always closed) in the Preferences popover; preferences declared in a registry so further viewer preferences are definitions, not code paths. |
| `R5.later-darkmode.d` | Landed 2026-09-15 (E-151) | Residual dark-mode tokenisation: control-surface + emphasis tokens replace every un-flipped surface/secondary/light usage across all Visual Editor stylesheets; the darkmode arc (.a/.b/.c/.d) is closed. |
| `R6.1` | Code complete + live-QA'd 2026-09-16 (E-155–E-157); maintainer sign-off over checklist rows 21–26 pending | Site Manager Workspace sort-by (recent / title / newest / oldest / relevance) + Posts vs Taxonomies kind filter; spec `releases/R6.1-WORKSPACE-SORT-AND-KIND-FILTER.md`. |
