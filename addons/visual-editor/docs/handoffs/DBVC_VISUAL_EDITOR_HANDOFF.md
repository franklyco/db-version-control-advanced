...
- **R3-B — Shared Globals compatibility provider** — adapt `SharedGlobalFieldsController`'s configured relationship/post_object fields into `ControlRecord`s via the R3-A registry, preserving current settings and existing edit behavior. Compat gate: no regression when the registry-backed center is disabled. See `releases/R3-REGISTRY-BACKED-BRAND-CONTROL-CENTER.md` §R3-B.
- **R3-C — Minimal center UI + open route** — smallest production interface that lists registered controls (loading/empty/error/inspect-only/unsupported/unavailable states) and opens a control by its `publicId` through a fresh server-side descriptor resolution. Reuses the existing main panel and popover patterns; no new toolbar button unless discovery proves it necessary.
