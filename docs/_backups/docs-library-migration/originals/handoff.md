...
- `POST /proposals/{id}/entities/{vf_object_uid}/snapshot` — capture a current snapshot for the entity. If the incoming UID does not exist locally, the server falls back to manifest identity matching (post_id/post_type/post_name) and logs a debug message when `WP_DEBUG` is enabled.
...
- `GET /proposals/{id}/entities`: `{ "proposal_id": "...", "items": [{ "vf_object_uid": "21651", "post_id": 21651, "post_type": "alternative", "post_title": "SquareSpace...", "path": "...json", "hash": "...", "content_hash": "...", "media_refs": {...}, "resolver": { "status": "needs_review", "summary": { "total": 2, "resolved": 1, "unresolved": 1, "conflicts": 0, "unknown": 0 }, "attachments": [...] }, "decision_summary": { "accepted": 3, "kept": 1, "total": 4, "has_accept": true } }], "resolver": { "metrics": {...} }, "decision_summary": { "accepted": 12, "kept": 4, "total": 16, "entities_reviewed": 5, "entities_with_accept": 3 } }`
  ...
  - UI: Entity drawer now supports a "View All" mode to list all meta fields (including unchanged values). Entity list badges include a "UID mismatch" filter for items whose local UID differs from the proposal UID. The entities panel now shows a totals summary (proposed/current breakdown by posts/terms/media + filtered count).
...
- Placeholder UI (`src/admin-app/index.js`) currently renders proposal table sourced from new REST endpoints, with the compact uploader panel + capped (three-row) scrollable list + per-row delete actions for non-current proposals already in place—replace with production components once design is ready while preserving that footprint.
