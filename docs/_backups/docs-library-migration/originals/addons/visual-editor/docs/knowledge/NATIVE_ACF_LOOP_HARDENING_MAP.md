...
| One-level `parent_native_query` context was too thin for deeper owner-loop descendants | [`src/Bricks/LoopContextResolver.php`](../../src/Bricks/LoopContextResolver.php), [`src/Resolvers/ResolverRegistry.php`](../../src/Resolvers/ResolverRegistry.php), [`src/Bricks/ElementInstrumentationService.php`](../../src/Bricks/ElementInstrumentationService.php), [`src/Presentation/DescriptorSummaryBuilder.php`](../../src/Presentation/DescriptorSummaryBuilder.php), [`src/Save/MutationContractService.php`](../../src/Save/MutationContractService.php) | `LoopContextResolver::buildNativeAcfQueryAncestry()`, `exportNativeAcfQueryAncestry()`, `ResolverRegistry::normalizeNativeQueryAncestry()`, `ElementInstrumentationService::normalizeNativeQueryAncestry()`, `buildNativeQueryAncestryLabels()`, `DescriptorSummaryBuilder::normalizeNativeQueryAncestry()`, `MutationContractService::resolveMutationList()` | Carries the full outer-to-inner native ancestor chain into source hashing, path summaries, and contract detail, which is a better fit for upcoming `relationship/post_object -> repeater/flexible` and later mixed nesting than a single parent hint | Keep full native ancestry as the canonical descriptor contract for nested owner-loop descendants so future widening does not need to rediscover owner chains one level at a time |
