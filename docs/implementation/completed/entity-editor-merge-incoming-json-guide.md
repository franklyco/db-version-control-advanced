...
The implementation adds a small reusable merge service rather than placing merge logic directly in REST callbacks.
...
Implemented routes under the existing Entity Editor namespace:
...
| Phase | Status | Goal |
|---|---|---|
| P0 | `CLOSED` | Confirm existing helpers, current JSON shapes, and safest service boundaries |
| P1 | `CLOSED` | Add reusable merge preview service |
| P2 | `CLOSED` | Add REST preview/save endpoints |
| P3 | `CLOSED` | Add modal UI, identity toolbar, preview notes, and confirmation gating |
| P4 | `CLOSED` | Wire save-only and save-plus-partial-import actions through existing Entity Editor paths |
| P5 | `CLOSED` | Add Bricks template soft flags for condition/scope/preview reference mismatches |
| P6 | `CLOSED` | Add focused PHPUnit and UI smoke coverage |
