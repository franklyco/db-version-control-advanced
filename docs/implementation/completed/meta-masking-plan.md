...
This plan outlines the implementation steps for masking meta fields inside live proposal reviews. It builds on the reference copy/links in `docs/reference/meta-masking.md` and adds concrete UI + backend work needed before coding.
...
1. **Toolbar entry point**
   ...
   - Tooltip content comes directly from `docs/reference/meta-masking.md#live-proposal-masking` (help link text “Masking guide”).
...
2. **Masking panel**
   ...
   - When `Override` is chosen, override value + optional note inputs appear. Tooltip copy links to `docs/reference/meta-masking.md#override-masked-value`.
...
All tooltip triggers pull copy from `docs/reference/meta-masking.md`:
