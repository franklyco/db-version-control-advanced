...
Current phases: `P5-P7`
Status legend: `OPEN` | `WIP` | `CLOSED` | `DEFERRED`

## Current Answer

The registry-backed DBVC configuration portability workflow is implemented and active, but runtime hardening, environment adaptation, and acceptance work remain.

The current administrator surface provides registered setting domains, package export/download, package upload and validation, staged diff review, environment decisions, confirmed apply with backup, and rollback. It is loaded from the plugin bootstrap and covered by registry, package, import-session, apply, and rollback tests.

Phases 3 and 4 are complete. Phases 5-7 remain active for runtime refresh and failure reporting, environment presets, browser/runtime QA, and user/developer documentation. No same-checkout browser apply/rollback claim is made by this guide.

Phase 8 continues to defer writer-capable WP-CLI parity. A separately bounded metadata-only `wp dbvc config domains|status` inspection surface may report provider schema counts, but it must not read current provider values or invoke export, upload, diff, apply, backup, or rollback services.
...
| `P3` | `DONE` | Implement import validation, sessions, and diff engine |
| `P4` | `DONE` | Add the administrator review UI; REST remains unnecessary for the current server-rendered contract |
...
### 2026-08-14 dependency preflight tranche notes

- Added provider-owned import dependency declarations without changing the exported package schema.
- Import-session review now surfaces unsupported incoming fields and missing domains, and blocks selected fields whose required post type, taxonomy, ACF options group, or add-on class is unavailable.
- Apply preflight rechecks dependencies before backup or settings writes.
- Added focused checksum-failure, missing-domain, unsupported-field, missing-dependency, and target-non-mutation coverage.

### 2026-08-14 review draft tranche notes

- Added seven-day, fingerprint-bound review drafts inside the existing import-session file lifecycle.
- Drafts restore keep/replace choices and ordinary replacement values, but never store or rehydrate credential-like replacement text.
- Drafts become unusable when expired or when their staged package/diff fingerprint changes, and successful apply removes the draft before the session is saved.
- Added administrator, nonce, invalid-session, stale-fingerprint, expiry, recovery, and no-settings-write coverage.
...
- [x] Add dependency checks for post types, taxonomies, ACF option groups, and add-on classes.
- [x] Add diff service.
- [x] Add status model and warnings.
- [x] Add draft decision persistence.
- [x] Add tests for checksum failures, missing domains, unsupported fields, and target dependency warnings.
...
### 2026-08-14 review filters tranche notes

- Added server-rendered domain, group, field, status, and unresolved-only filters using GET parameters.
- Rehydrated active session draft choices in the existing environment-decision form without adding REST or JavaScript.
- Added a nonce-protected administrator draft-save action that writes only the owning import-session file and never destination configuration.
...
- [x] Keep REST omitted because the completed server-rendered review contract does not require it.
...
- [x] Add domain/group/field/status and unresolved-only filters.
- [x] Add environment replacement inputs.
- [x] Add draft decisions.
- [x] Add apply confirmation checkbox.
- [x] Add backup/rollback controls.
- [x] Keep Phase 4 server-rendered; no client-side JS is required for current filtering or environment prompts.
...
The remaining P3-P7 work is organized into the following continuation phases in `docs/implementation/completed/dbvc-agent-reference-library.md`:

| Continuation phase | Existing guide coverage | Boundary |
|---|---|---|
| Phase 36 — Documentation Authority Cleanup (`COMPLETE`) | Guide/manifest/roadmap maintenance | Documentation and generated-authority corrections only |
| Phase 37 — Import Dependency Preflight (`COMPLETE`) | P3 validation, dependency warnings, and missing/unsupported-input tests | Non-writing upload/session review |
| Phase 38 — Review Drafts And Filters (`COMPLETE`) | P3/P4 draft decisions, filters, and minimal progressive UI | Session-state writes only; no destination configuration writes |
| Phase 39 — Apply And Runtime Hardening | P5 runtime refresh, activity logs, and partial-failure reporting | Explicitly approved configuration writer/rollback boundary |
| Phase 40 — Environment Adaptation | P6 presets, replacements, policy overrides, and mismatch warnings | Preview is non-writing; apply remains governed by Phase 39 |
| Phase 41 — Acceptance And Documentation | P7 tests, runtime QA, user/developer/schema/QA docs, and changelog | Writer QA requires separate authorization and disposable fixtures |

Sequence authority: Phases 36-38 are complete. Stop for an explicit writer-boundary review before Phase 39. Phase 40 preview design can proceed independently, but live application evidence depends on Phase 39. Phase 41 closes the workflow only after applicable source, test, runtime, cleanup, and documentation evidence agree.

These continuation phases organize existing work; they do not silently close P3-P7 checkboxes or authorize the deferred Phase 8 capabilities below.
...
- Writer-capable WP-CLI commands:
  - `wp dbvc config export`
  - `wp dbvc config import`
  - `wp dbvc config apply`
  - `wp dbvc config rollback`
- Any CLI output that reads or returns current provider values, secrets, environment-specific values, package contents, import sessions, or backups.
...
- The implemented v1 uses the dedicated `DBVC -> Configuration Portability` submenu at `admin.php?page=dbvc-configuration-portability`; the earlier Configure-subtab-versus-submenu question is closed.
...
Completed in the initial implementation tranches:

1. Registry with the initial three providers:
   - Visual Editor activation
   - Logging toggles
   - Media handling settings
...
This proved the architecture before Bricks, AI Package, and Content Collector providers with nested arrays and secret policies were added. Remaining work is governed by P3-P7 and continuation Phases 37-41.
