...
(function () {
  ...
  var root = document.querySelector('.dbvc-ve-media-manager');
  if (!root) {
    return;
  }

  var announcer = root.querySelector('[data-mockup-live="announcer"]');
  var tbody = root.querySelector('[data-mockup-region="rows"]');
  var emptyState = root.querySelector('[data-mockup-region="empty"]');
  var results = root.querySelector('[data-mockup-region="results"]');
  var reopen = document.querySelector('[data-mockup-action="reopen"]');
  ...
  function collapseAll(except) {
    var toggles = root.querySelectorAll('[data-mockup-action="toggle-row"]');
    Array.prototype.forEach.call(toggles, function (toggle) {
      if (toggle === except) {
        return;
      }
      var expansion = expansionFor(toggle);
      toggle.setAttribute('aria-expanded', 'false');
      toggle.closest('.dbvc-ve-media-manager__row').classList.remove('is-expanded');
      if (expansion) {
  ...
  function toggleRow(toggle) {
    var row = toggle.closest('.dbvc-ve-media-manager__row');
    var expansion = expansionFor(toggle);
    var wasExpanded = toggle.getAttribute('aria-expanded') === 'true';
    var label = row.querySelector('.dbvc-ve-media-manager__entity-label');
    var name = label ? label.textContent.trim() : 'entity';
    ...
    if (!panel || panel.classList.contains('is-pending')) {
      announce('Expanded ' + name + '. Requesting field detail. Still pending.');
    } else {
  ...
  function revalidatedSummary(panel) {
    var status = panel.getAttribute('data-expansion-status') || '';
    var fields = panel.querySelectorAll('.dbvc-ve-media-manager__field').length;
    var wording = {
    ...
    return (wording[status] || 'Revalidated.') + ' ' + count(fields, 'field', 'fields') + ' listed.';
  }
   ...
   */
  function readQuery() {
    var search = root.querySelector('[data-mockup-input="search"]');
    var entity = root.querySelector('[data-mockup-input="entity-family"]:checked');
    var field = root.querySelector('[data-mockup-input="field-family"]:checked');
    var sort = root.querySelector('[data-mockup-input="sort"]');
  ...
  function applyQuery() {
    ...
    rows.forEach(function (row) {
      var families = (row.getAttribute('data-field-families') || '').split(' ');
      var label = row.getAttribute('data-entity-label') || '';
      var matches =
        (query.search === '' || label.indexOf(query.search) !== -1) &&
        (query.entity === 'all' || row.getAttribute('data-entity-family') === query.entity) &&
        (query.field === 'all' || families.indexOf(query.field) !== -1);

      row.hidden = !matches;

      var toggle = row.querySelector('[data-mockup-action="toggle-row"]');
      var expansion = toggle ? expansionFor(toggle) : null;
    ...
    announce(
      visible === 0
        ...
        : 'Showing ' + count(visible, 'entity', 'entities') + ' on this page.'
    );
  }
  ...
  function reorder(rows, sort) {
    ...
    if (sort === 'missing_desc') {
      sorted.sort(function (a, b) {
        return Number(b.getAttribute('data-missing')) - Number(a.getAttribute('data-missing'));
      });
    } else if (sort === 'scanned_desc') {
      sorted.sort(function (a, b) {
        return Number(b.getAttribute('data-scanned')) - Number(a.getAttribute('data-scanned'));
      });
    } else {
      sorted.sort(function (a, b) {
        return (a.getAttribute('data-entity-label') || '').localeCompare(
          b.getAttribute('data-entity-label') || ''
        );
      });
    }
  ...
  function markSortColumn(sort) {
    var headers = root.querySelectorAll('[data-mockup-sortcol]');
    Array.prototype.forEach.call(headers, function (header) {
      if (header.getAttribute('data-mockup-sortcol') === sort) {
        header.setAttribute('aria-sort', sort === 'entity_asc' ? 'ascending' : 'descending');
      } else {
  ...
  root.addEventListener('click', function (event) {
    var trigger = event.target.closest('[data-mockup-action]');
    if (!trigger || !root.contains(trigger)) {
      return;
    }

    var action = trigger.getAttribute('data-mockup-action');
    ...
    if (action === 'refresh') {
      ...
      announce('Refresh requested. In production this starts a new scan and reports progress.');
      return;
    }
  ...
  document.addEventListener('keydown', function (event) {
    ...
    var close = root.querySelector('[data-mockup-action="close"]');
    if (close) {
      close.click();
    }
  });
