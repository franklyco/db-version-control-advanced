...
| R3 | Registry-backed Brand Control Center | In development (Slice R3-A done; R3-BX curation tool landed) | Codex R3-A + R3-BX / 2026-08-23..24 | `releases/R3-REGISTRY-BACKED-BRAND-CONTROL-CENTER.md`; `addons/visual-editor/src/Registry/{ControlProvider,ControlRecord,ControlRegistry}.php`; `addons/visual-editor/src/Curation/{FieldCandidateProvider,FieldCurationRecommender,CurationStore,CurationExporter}.php`; `addons/visual-editor/src/Admin/CurationPage.php`; `tests/phpunit/VisualEditorControlRegistryTest.php`; `tests/phpunit/VisualEditorCuration*Test.php`; E-088; E-089 | **R3-A (2026-08-23)**: provider-agnostic control registry foundation — `ControlProvider` interface, `ControlRecord` value object with whitelisted normalization + safe list projection (no `source` bag on the wire), `ControlRegistry` collects/validates/dedupes/filters-by-user-visibility with deterministic ordering. Duplicate providers, duplicate local ids within a provider, and malformed records are all rejected observably via `dbvc_visual_editor_control_registry_invalid`. `getVisibleRecord($publicId)` is the internal bridge R3-C will use for the open route. **No REST, no UI, no wiring in `Addon::register` yet** (nothing to serve until R3-B registers Shared Globals as the first provider). 11 PHP tests / 44 assertions; full suite 810 with six inherited failures + one pre-existing dirty-tree `ProposalDiffContractTest` failure (not R3-A caused); agent docs 54/432/0. Remaining: **R3-B** (Shared Globals compat provider), **R3-C** (minimal center UI + open route), **R3-D** (production hardening) |
...
- [ ] Shared Globals compatibility provider (R3-B)
- [ ] Minimal center (R3-C)
- [ ] Fresh descriptor opening (R3-C)
- [ ] Existing main panel/save behavior (R3-C)
- [ ] Diagnostics/tests/fallback (R3-D)
