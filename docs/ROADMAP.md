...
|---|---|---|
| Agent capability reference library | Self-updating 53-record/405-surface inventory, eight bounded read-only recipes, capability/Bricks/Entity Editor/Content Migration/proposal inspection CLIs, resolver dry-run, explicitly confirmed trusted stale-decision pruning in Proposal Review, non-creating snapshot- and backup-read barriers, and non-mutating current-state identity inspection, machine-readable opportunity boundaries, plus an administrator-only verification-ledger Capability Landscape UI. | `docs/agents/README.md`; `docs/implementation/completed/dbvc-agent-reference-library.md` |
